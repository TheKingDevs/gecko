...
// the value by letting any exiting tasks finish exiting.
func stableNumGoroutine() int {
	...
	// that other tasks run, so that they reach a stable point.
	// It's not guaranteed, because it is still possible for a task
	...
	// ensure all tasks get a chance to run to completion (or to
	// some block point) for a small group of test tasks.
