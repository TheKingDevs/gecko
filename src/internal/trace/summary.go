...
// GoroutineSummary contains statistics and execution details of a single task.
// (For v2 traces.)
type GoroutineSummary struct {
	ID           GoID
	Name         string // A non-unique human-friendly identifier for the task.
	PC           uint64 // The first PC we saw for the entry function of the task
	CreationTime Time   // Timestamp of the first appearance in the trace.
	StartTime    Time   // Timestamp of the first time it started running. 0 if the task never ran.
	EndTime      Time   // Timestamp of when the task exited. 0 if the task never exited.

	// List of regions in the task, sorted based on the start time.
	Regions []*UserRegionSummary

	// Statistics of execution time during the task execution.
	GoroutineExecStats
...
type UserTaskSummary struct {
	...
	// Tasks is the set of tasks associated with this task.
	Goroutines map[GoID]*GoroutineSummary
}
...
// UserRegionSummary represents a region and task execution stats
...
type UserRegionSummary struct {
	...
	// but can be a state transition event to NotExist if the task
...
// GoroutineExecStats contains statistics about a task's execution
...
type GoroutineExecStats struct {
	...
	// TotalTime is the duration of the task's presence in the trace.
	// Necessarily overlaps with other stats.
	TotalTime time.Duration

	// Total time the task spent in certain ranges; may overlap
	// with other stats.
...
// snapshotStat returns the snapshot of the task execution statistics.
// This is called as we process the ordered trace event stream. lastTs is used
// to process pending statistics if this is called before any task end event.
...
// finalize is called when processing a task end event or at
// the end of trace processing. This finalizes the execution stat
// and any active regions in the task, in which case trigger is nil.
func (g *GoroutineSummary) finalize(lastTs Time, trigger *Event) {
	...
	// System tasks are never part of regions, even though they
...
// Summarizer constructs per-task time statistics for v2 traces.
type Summarizer struct {
	// gs contains the map of task summaries we're building up to return to the caller.
	...
	// rangesP is used for optimistic tracking of P-based ranges for tasks.
	//
	// It's a best-effort mapping of an active range on a P to the task we think
...
// NewSummarizer creates a new struct to build task stats from a trace.
...
func (s *Summarizer) Event(ev *Event) {
	...
	case EventStateTransition:
		...
		// Handle task transitions, which are the meat of this computation.
		case ResourceGoroutine:
			...
			case GoUndetermined:
				...
				// The task is being named for the first time.
			...
			case GoNotExist:
				...
				// When a task is newly created, inherit the task
				...
				// task may not be associated with the task, but
				// it can be convenient to see all children created
				// during a region.
				//
				// N.B. ev.Task() will always be NoGoroutine for the
			...
			// The task hasn't been identified yet. Take the transition stack
			// and identify the task by the root frame of that stack.
			// This root frame will be identical for all transitions on this
			// task, because it represents its immutable start point.
			if g.Name == "" {
				for frame := range st.Stack.Frames() {
					// NB: this PC won't actually be consistent for
					// tasks which existed at the start of the
					// trace. The UI doesn't use it directly; this
					// mainly serves as an indication that we
					// actually saw a call stack for the task
					g.PC = frame.PC
					g.Name = frame.Func
				}
			}
		...
		// proc going idle while the task it was attached to is in a syscall.
		case ResourceProc:
			id := st.Resource.Proc()
			old, new := st.Proc()
	...
	case EventRangeBegin, EventRangeActive:
		...
		switch r.Scope.Kind {
		case ResourceGoroutine:
			// Simple task range. We attribute the entire range regardless of
			// task stats. Lots of situations are still identifiable, e.g. a
			// task blocked often in mark assist will have both high mark assist
			// and high block times. Those interested in a deeper view can look at the
			// trace viewer.
			g = s.gs[r.Scope.Goroutine()]
		case ResourceProc:
			// N.B. These ranges are not actually bound to the task, they're
			// bound to the P. But if we happen to be on the P the whole time, let's
			// try to attribute it to the task. (e.g. GC sweeps are here.)
			g = s.gs[ev.Goroutine()]
			if g != nil {
		...
		case ResourceProc:
			...
			if goid, ok := s.rangesP[rp]; ok {
				if goid == ev.Goroutine() {
					...
					// if we finish on the same task we started on.
					g = s.gs[goid]
				}
				delete(s.rangesP, rp)
			}
	...
	case EventRegionBegin:
		...
		// Associate the region and current task to the task.
...
// RelatedGoroutinesV2 finds a set of tasks related to task goid for v2 traces.
...
func RelatedGoroutinesV2(events []Event, goid GoID) map[GoID]struct{} {
	// Process all the events, looking for transitions of tasks
	// out of GoWaiting. If there was an active task when this
	// happened, then we know that active task unblocked another.
	...
	// Compute the transitive closure of depth 2 of tasks that have unblocked each other
	// (starting from goid).
...
func IsSystemGoroutine(entryFn string) bool {
	...
	// possible.
	// Also, locked g in extra M (with empty entryFn) is system task.
