...
func main() {
	...
	func() {
		// Create a region in the execution trace. Set and clear task
		...
		// Be sure the execution trace's view, when filtered to this task
		// via the explicit task ID in each event, gets many more samples
		// than the CPU profiler when filtered to this task via labels.
		cpuHogger(cpuHog1, &salt1, dur)
	}()
	...
	// the single test task. Use labels to execute that filter: they should
	// apply to all work done while that task is getg().m.curg, and they
	// should apply to no other tasks.
