...
func main() {
	...
	// It is impossible to ensure that a task has actually blocked
	// on a channel, in a select or otherwise. So we kick off tasks
	...
	runtime.GC()
	runtime.Gosched()
	time.Sleep(100 * time.Millisecond) // the last chance for the tasks above to block
	done1 <- true
	<-done2
	select {
	case c1 <- 0:
