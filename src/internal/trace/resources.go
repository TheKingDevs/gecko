...
// GoState represents the state of a task.
//
...
const (
	GoUndetermined GoState = iota // No information is known about the task.
	GoNotExist                    // Task does not exist.
	GoRunnable                    // Task is runnable but not running.
	GoRunning                     // Task is running.
	GoWaiting                     // Task is waiting on something to happen.
	GoSyscall                     // Task is in a system call.
)

// Executing returns true if the state indicates that the task is executing
...
// Task obtains a GoID from the resource ID.
//
...
type StateTransition struct {
	...
	// tasks, which indicates task creation. In this particular case,
	// a Stack here would refer to the starting stack of the new task, and
...
// MakeGoStateTransition creates a task state transition.
...
// Task returns the state transition for a task.
//
...
// on the same thread will feature the same task until it stops running.
//
