...
func (v *Validator) Event(ev trace.Event) error {
	...
	case trace.EventStateTransition:
		...
		//
		// TODO(mknyszek): A lot of logic is duplicated between tasks and procs.
		...
		case trace.ResourceGoroutine:
			...
			if new.Executing() {
				...
				if tr.Stack != ev.Stack() {
					// This is a case where the transition is happening to a task that is also executing, so
