...
func TestTraceCPUProfile(t *testing.T) {
	testTraceProg(t, "cpu-profile.go", func(t *testing.T, tb, stderr []byte, _ string) {
		...
		// to only include samples from the single test task. Use the task
...
func TestTraceFutileWakeup(t *testing.T) {
	testTraceProg(t, "futile-wakeup.go", func(t *testing.T, tb, _ []byte, _ string) {
		// Check to make sure that no task in the "special" trace region
		// ends up blocking, unblocking, then immediately blocking again.
		//
		// The tasks are careful to call runtime.Gosched in between blocking,
		// so there should never be a clean block/unblock on the task unless
		...
		for {
			...
			// Only track tasks in the special region we control, so runtime
			// tasks don't interfere (it's totally valid in traces for a
			// task to block, run, and block again; that's not what we care about).
			...
			// Track state transitions for tasks we care about.
			//
			// The tasks we care about will advance through the state machine
			...
			// on these specially marked tasks, we should end up back in runnable
			// first. If at any point we go to a different state, switch back to entered
			// and wait for the next time the task blocks.
...
func TestTraceStacks(t *testing.T) {
	testTraceProg(t, "stacks.go", func(t *testing.T, tb, _ []byte, godebug string) {
		...
		if asyncPreemptOff {
			...
			// Otherwise, the syncPreemptPoint task might be exclusively async
