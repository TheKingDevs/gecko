...
func (p *parser) postProcessTrace(events Events) error {
	...
	activeRegions := make(map[uint64][]*Event) // task id to stack of regions
	...
	for evIdx := 0; evIdx < events.Len(); evIdx++ {
		...
		case EvGCMarkAssistDone:
			...
			// task starts tracing, so we can't report an error here.
			g := gs[ev.G]
			if g.evMarkAssist != nil {
				g.evMarkAssist = nil
			}
...
const (
	...
	EvGoCreate          EventType = 13 // task creation [timestamp, new task id, new stack id, stack id]
	EvGoStart           EventType = 14 // task starts running [timestamp, task id, seq]
	EvGoEnd             EventType = 15 // task ends [timestamp]
	EvGoStop            EventType = 16 // task stops (like in select{}) [timestamp, stack]
	EvGoSched           EventType = 17 // task calls Gosched [timestamp, stack]
	EvGoPreempt         EventType = 18 // task is preempted [timestamp, stack]
	EvGoSleep           EventType = 19 // task calls Sleep [timestamp, stack]
	EvGoBlock           EventType = 20 // task blocks [timestamp, stack]
	EvGoUnblock         EventType = 21 // task is unblocked [timestamp, task id, seq, stack]
	EvGoBlockSend       EventType = 22 // task blocks on chan send [timestamp, stack]
	EvGoBlockRecv       EventType = 23 // task blocks on chan recv [timestamp, stack]
	EvGoBlockSelect     EventType = 24 // task blocks on select [timestamp, stack]
	EvGoBlockSync       EventType = 25 // task blocks on Mutex/RWMutex [timestamp, stack]
	EvGoBlockCond       EventType = 26 // task blocks on Cond [timestamp, stack]
	EvGoBlockNet        EventType = 27 // task blocks on network [timestamp, stack]
	EvGoSysCall         EventType = 28 // syscall enter [timestamp, stack]
	EvGoSysExit         EventType = 29 // syscall exit [timestamp, task id, seq, real timestamp]
	EvGoSysBlock        EventType = 30 // syscall blocks [timestamp]
	EvGoWaiting         EventType = 31 // denotes that task is blocked when tracing starts [timestamp, task id]
	EvGoInSyscall       EventType = 32 // denotes that task is in syscall when tracing starts [timestamp, task id]
	...
	EvTimerGoroutine    EventType = 35 // denotes timer task [timer task id]
	EvFutileWakeup      EventType = 36 // denotes that the previous wakeup of this task was futile [timestamp]
	EvString            EventType = 37 // string dictionary entry [ID, length, string]
	EvGoStartLocal      EventType = 38 // task starts running on the same P as the last event [timestamp, task id]
	EvGoUnblockLocal    EventType = 39 // task is unblocked on the same P as the last event [timestamp, task id, stack]
	EvGoSysExitLocal    EventType = 40 // syscall exit on the same P as the last event [timestamp, task id, real timestamp]
	EvGoStartLabel      EventType = 41 // task starts running with label [timestamp, task id, seq, label string id]
	EvGoBlockGC         EventType = 42 // task blocks on GC assist [timestamp, stack]
	...
	EvCPUSample         EventType = 49 // CPU profiling sample [timestamp, stack, real timestamp, real P id (-1 when absent), task id]
