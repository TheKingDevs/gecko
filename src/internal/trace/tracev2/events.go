...
const (
	...
	EvCPUSample  // CPU profiling sample [timestamp, M ID, P ID, task ID, stack ID]
	...
	// Tasks.
	EvGoCreate            // task creation [timestamp, new task ID, new stack ID, stack ID]
	EvGoCreateSyscall     // task appears in syscall (cgo callback) [timestamp, new task ID]
	EvGoStart             // task starts running [timestamp, task ID, task seq]
	EvGoDestroy           // task ends [timestamp]
	EvGoDestroySyscall    // task ends in syscall (cgo callback) [timestamp]
	EvGoStop              // task yields its time, but is runnable [timestamp, reason, stack ID]
	EvGoBlock             // task blocks [timestamp, reason, stack ID]
	EvGoUnblock           // task is unblocked [timestamp, task ID, task seq, stack ID]
	...
	EvGoStatus            // task status at the start of a generation [timestamp, task ID, M ID, status]
	...
	EvGCMarkAssistActive // GC mark assist active [timestamp, task ID]
	...
	// Annotations.
	EvGoLabel         // apply string label to current running task [timestamp, label string ID]
	...
	EvGoSwitch        // task switch (coroswitch) [timestamp, task ID, task seq]
	EvGoSwitchDestroy // task switch and destroy [timestamp, task ID, task seq]
	EvGoCreateBlocked // task creation (starts blocked) [timestamp, new task ID, new stack ID, stack ID]

	// GoStatus with stack. Added in Go 1.23.
	EvGoStatusStack // task status at the start of a generation, with a stack [timestamp, task ID, M ID, status, stack ID]
...
// GoStatus is the status of a task.
//
// They correspond directly to the various task states.
type GoStatus uint8
