...
const (
	...
	// tasks) that have existed until that point have been enumerated.
	EventSync
	...
	// thread/proc/task was doing at a particular point in time via
	// its backtrace.
	//
	// Note: Samples should be considered a close approximation of
	// what a thread/proc/task was executing at a given point in time.
	...
	// range is globally scoped. That is, any task/proc/thread can start or
...
type Range struct {
	...
	//
	// For example, a ResourceGoroutine scope means that the same task
	// must have a start and end for the range, and that task can only
	// have one range of a particular name active at any given time. The
	// ID that this range is scoped to may be obtained via Event.Task.
	//
	// The ResourceNone scope means that the range is globally scoped. As a
	// result, any task/proc/thread may start or end the range, and only
...
type EventConfig[T EventDetails] struct {
	...
	// Task is the task ID of the event.
	Goroutine GoID
...
// Task returns the ID of the task that was executing when
// this event happened. It describes part of the execution context
// for this event.
//
// Note that for task state transitions this always refers to the
// state before the transition. For example, if a task is just
// starting to run on this thread and/or proc, then this will return
// NoGoroutine. In this case, the task starting to run will be
...
func (e Event) StateTransition() StateTransition {
	...
	case tracev2.EvGoUnblock, tracev2.EvGoSwitch, tracev2.EvGoSwitchDestroy:
		...
		// the first thing they both do is unblock the task they name,
