...
var orderingDispatch = [256]orderingHandleFunc{
	...
	// Experimental task stack events. Added in Go 1.23.
...
func (o *ordering) advanceProcStart(ev *baseEvent, evt *evTable, m ThreadID, gen uint64, curCtx schedCtx) (schedCtx, bool, error) {
	...
	//
	// We might have a task if a task is exiting a syscall.
...
func (o *ordering) advanceGoStatus(ev *baseEvent, evt *evTable, m ThreadID, gen uint64, curCtx schedCtx) (schedCtx, bool, error) {
	...
	case tracev2.GoRunning:
		// Bind the task to the new context, since it's running.
		newCtx.G = gid
	case tracev2.GoSyscall:
		...
		if ok {
			...
			// task go into a syscall on this thread at some point.
			if ms.g != gid {
			...
			// The M hasn't been seen yet. That means this task
...
func (o *ordering) advanceGoCreate(ev *baseEvent, evt *evTable, m ThreadID, gen uint64, curCtx schedCtx) (schedCtx, bool, error) {
	// Tasks must be created on a running P, but may or may not be created
	// by a running task.
	reqs := schedReqs{M: mustHave, P: mustHave, G: mayHave}
	if err := validateCtx(curCtx, reqs); err != nil {
		return curCtx, false, err
	}
	// If we have a task, it must be running.
	...
	// This task created another. Add a state for it.
...
func (o *ordering) advanceGoStopExec(ev *baseEvent, evt *evTable, m ThreadID, gen uint64, curCtx schedCtx) (schedCtx, bool, error) {
	// These are task events that all require an active running
	// task on some thread. They must *always* be advance-able,
	// since running tasks are bound to their M.
	...
	case tracev2.EvGoDestroy:
		// This task is exiting itself.
		delete(o.gStates, curCtx.G)
		newCtx.G = NoGoroutine
	case tracev2.EvGoStop:
		// Task stopped (yielded). It's runnable but not running on this M.
		state.status = tracev2.GoRunnable
		newCtx.G = NoGoroutine
	case tracev2.EvGoBlock:
		// Task blocked. It's waiting now and not running on this M.
...
func (o *ordering) advanceGoStart(ev *baseEvent, evt *evTable, m ThreadID, gen uint64, curCtx schedCtx) (schedCtx, bool, error) {
	...
	if !ok || state.status != tracev2.GoRunnable || !seq.succeeds(state.seq) {
		// We can't make an inference as to whether this is bad. We could just be seeing
		// a GoStart on a different M before the task was created, before it had its
		// state emitted, or before we got to the right point in the trace yet.
		return curCtx, false, nil
	}
	// We can advance this task. Check some invariants.
...
func (o *ordering) advanceGoUnblock(ev *baseEvent, evt *evTable, m ThreadID, gen uint64, curCtx schedCtx) (schedCtx, bool, error) {
	// N.B. These both reference the task to unblock, not the current task.
	gid := GoID(ev.args[0])
	seq := makeSeq(gen, ev.args[1])
	state, ok := o.gStates[gid]
	if !ok || state.status != tracev2.GoWaiting || !seq.succeeds(state.seq) {
		// We can't make an inference as to whether this is bad. We could just be seeing
		// a GoUnblock on a different M before the task was created and blocked itself,
...
func (o *ordering) advanceGoSwitch(ev *baseEvent, evt *evTable, m ThreadID, gen uint64, curCtx schedCtx) (schedCtx, bool, error) {
	...
	// - Unblock of the task to switch to.
	// - Block or destroy of the current task.
	// - Start executing the next task.
	//
	// Because it acts like a GoStart for the next task, we can
	// only advance it if the sequence numbers line up.
	//
	// The current task on the thread must be actively running.
	if err := validateCtx(curCtx, userGoReqs); err != nil {
		return curCtx, false, err
	}
	...
	if !ok || nextGState.status != tracev2.GoWaiting || !seq.succeeds(nextGState.seq) {
		// We can't make an inference as to whether this is bad. We could just be seeing
		// a GoSwitch on a different M before the task was created, before it had its
		// state emitted, or before we got to the right point in the trace yet.
		return curCtx, false, nil
	}
	o.queue.push(Event{table: evt, ctx: curCtx, base: *ev})

	// Update the state of the executing task and emit an event for it
	...
	// Update the state of the next task.
	nextGState.status = tracev2.GoRunning
	nextGState.seq = seq
	newCtx := curCtx
	newCtx.G = nextg

	// Queue an event for the next task starting to run.
	startCtx := curCtx
...
func (o *ordering) advanceGoSyscallBegin(ev *baseEvent, evt *evTable, m ThreadID, gen uint64, curCtx schedCtx) (schedCtx, bool, error) {
	// Entering a syscall requires an active running task with a
	...
	// Task entered a syscall. It's still running on this P and M.
	...
	//
	// We have a P sequence number for what is supposed to be a task event
...
func (o *ordering) advanceGoSyscallEnd(ev *baseEvent, evt *evTable, m ThreadID, gen uint64, curCtx schedCtx) (schedCtx, bool, error) {
	...
	// thread that EvGoSyscallStart happened, and the task can't leave
...
func (o *ordering) advanceGoCreateSyscall(ev *baseEvent, evt *evTable, m ThreadID, gen uint64, curCtx schedCtx) (schedCtx, bool, error) {
	// This event indicates that a task is effectively
	// being created out of a cgo callback. Such a task
	// is 'created' in the syscall state.
	if err := validateCtx(curCtx, schedReqs{M: mustHave, P: mayHave, G: mustNotHave}); err != nil {
		return curCtx, false, err
	}
	// This task is effectively being created. Add a state for it.
	newgid := GoID(ev.args[0])
	if _, ok := o.gStates[newgid]; ok {
	...
	// Task is executing. Bind it to the context.
	newCtx := curCtx
	newCtx.G = newgid
	o.queue.push(Event{table: evt, ctx: curCtx, base: *ev})
	return newCtx, true, nil
...
func (o *ordering) advanceGoDestroySyscall(ev *baseEvent, evt *evTable, m ThreadID, gen uint64, curCtx schedCtx) (schedCtx, bool, error) {
	// This event indicates that a task created for a
	...
	// Check to make sure the task exists in the right state.
	state, ok := o.gStates[curCtx.G]
	if !ok {
		return curCtx, false, fmt.Errorf("event %s for goroutine (%v) that doesn't exist", o.evName(ev.typ), curCtx.G)
	}
...
func (o *ordering) advanceGoRangeBegin(ev *baseEvent, evt *evTable, m ThreadID, gen uint64, curCtx schedCtx) (schedCtx, bool, error) {
	// Handle special task-bound event ranges.
	if err := validateCtx(curCtx, userGoReqs); err != nil {
		return curCtx, false, err
	}
	desc := stringID(0)
...
func (o *ordering) advanceGoRangeActive(ev *baseEvent, evt *evTable, m ThreadID, gen uint64, curCtx schedCtx) (schedCtx, bool, error) {
	...
	// reference a non-running task. Don't check anything about the
...
// gState is the state of a task at a point in the trace.
type gState struct {
	id     GoID
	status tracev2.GoStatus
	seq    seqCounter

	// regions are the active user regions for this task.
	regions []userRegion

	// rangeState is the state of special time ranges bound to this task.
	rangeState
}

// beginRegion starts a user region on the task.
func (s *gState) beginRegion(r userRegion) error {
	s.regions = append(s.regions, r)
	return nil
}

// endRegion ends a user region on the task.
...
type mState struct {
	g GoID   // Task bound to this M. (The task's state is Executing.)
...
// beginRange begins a special range in time on the task.
//
...
// activeRange marks special range in time on the task as active in the
...
// hasRange returns true if a special time range on the task as in progress.
func (s *rangeState) hasRange(typ rangeType) bool {
	return slices.Contains(s.inFlight, typ)
}

// endRange ends a special range in time on the task.
//
// This must line up with the start event type  of the range the task is currently in.
