...
func (it *traceV1Converter) convertEvent(ev *tracev1.Event) (OUT Event, ERR error) {
	...
	case tracev1.EvGomaxprocs:
		...
		if it.preInit {
			...
			// all tasks that already existed at trace begin.
			it.preInit = false
			for gid := range it.createdPreInit {
				// These are tasks that already existed when tracing started but for which we
				// received neither GoWaiting, GoInSyscall, or GoStart. These are tasks that are in
