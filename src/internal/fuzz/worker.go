...
// shared memory because it was closed and unmapped by another task. That
// can happen when worker.cleanup is called in the worker task while a
...
func (cr *contextReader) Read(b []byte) (int, error) {
	...
	// This task may stay blocked after Read returns because the underlying
	// read is blocked.
	var n int
	var err error
	go func() {
