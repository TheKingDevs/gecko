...
func TestAnd8(t *testing.T) {
	...
	// Clear array bit-by-bit in different tasks.
	done := make(chan bool)
	for i := 0; i < 8; i++ {
		m := ^uint8(1 << i)
		go func() {
...
func TestOr8(t *testing.T) {
	...
	// Set every bit in array bit-by-bit in different tasks.
	done := make(chan bool)
	for i := 0; i < 8; i++ {
		m := uint8(1 << i)
		go func() {
...
func TestBitwiseContended8(t *testing.T) {
	...
	// Set and then clear every bit in the array bit-by-bit in different tasks.
	done := make(chan bool)
	for i := 0; i < 8; i++ {
		m := uint8(1 << i)
		go func() {
