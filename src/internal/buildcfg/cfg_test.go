...
func TestConfigFlags(t *testing.T) {
	os.Setenv("GKAMD64", "v1")
	if goamd64() != 1 {
		t.Errorf("Wrong parsing of GKAMD64=v1")
	}
	os.Setenv("GKAMD64", "v4")
	if goamd64() != 4 {
		t.Errorf("Wrong parsing of GKAMD64=v4")
	}
	Error = nil
	os.Setenv("GKAMD64", "1")
	if goamd64(); Error == nil {
		t.Errorf("Wrong parsing of GKAMD64=1")
	}

	os.Setenv("GKRISCV64", "rva20u64")
	if goriscv64() != 20 {
		t.Errorf("Wrong parsing of RISCV64=rva20u64")
	}
	os.Setenv("GKRISCV64", "rva22u64")
	if goriscv64() != 22 {
		t.Errorf("Wrong parsing of RISCV64=rva22u64")
	}
	os.Setenv("GKRISCV64", "rva23u64")
	if goriscv64() != 23 {
		t.Errorf("Wrong parsing of RISCV64=rva23u64")
	}
	Error = nil
	os.Setenv("GKRISCV64", "rva22")
	if _ = goriscv64(); Error == nil {
		t.Errorf("Wrong parsing of RISCV64=rva22")
	}
	Error = nil
	os.Setenv("GKARM64", "v7.0")
	if _ = goarm64(); Error == nil {
		t.Errorf("Wrong parsing of GKARM64=7.0")
	}
	Error = nil
	os.Setenv("GKARM64", "8.0")
	if _ = goarm64(); Error == nil {
		t.Errorf("Wrong parsing of GKARM64=8.0")
	}
	Error = nil
	os.Setenv("GKARM64", "v8.0,lsb")
	if _ = goarm64(); Error == nil {
		t.Errorf("Wrong parsing of GKARM64=v8.0,lsb")
	}
	os.Setenv("GKARM64", "v8.0,lse")
	if goarm64().Version != "v8.0" || goarm64().LSE != true || goarm64().Crypto != false {
		t.Errorf("Wrong parsing of GKARM64=v8.0,lse")
	}
	os.Setenv("GKARM64", "v8.0,crypto")
	if goarm64().Version != "v8.0" || goarm64().LSE != false || goarm64().Crypto != true {
		t.Errorf("Wrong parsing of GKARM64=v8.0,crypto")
	}
	os.Setenv("GKARM64", "v8.0,crypto,lse")
	if goarm64().Version != "v8.0" || goarm64().LSE != true || goarm64().Crypto != true {
		t.Errorf("Wrong parsing of GKARM64=v8.0,crypto,lse")
	}
	os.Setenv("GKARM64", "v8.0,lse,crypto")
	if goarm64().Version != "v8.0" || goarm64().LSE != true || goarm64().Crypto != true {
		t.Errorf("Wrong parsing of GKARM64=v8.0,lse,crypto")
	}
	os.Setenv("GKARM64", "v9.0")
	if goarm64().Version != "v9.0" || goarm64().LSE != true || goarm64().Crypto != false {
		t.Errorf("Wrong parsing of GKARM64=v9.0")
	}
}
...
func TestGogoarchTags(t *testing.T) {
	...
	if len(tags) != len(want) {
		t.Errorf("Wrong number of tags for GKARM64=v9.5")
	} else {
		for i, v := range tags {
			if v != want[i] {
				t.Error("Wrong tags for GKARM64=v9.5")
				break
			}
		}
	}
