...
func TestNow(t *testing.T) {
	...
	synctest.Run(func() {
		...
		go func() {
			// New tasks see the same fake clock.
			if got, want := time.Now(), start; !got.Equal(want) {
...
// TestWait starts a collection of tasks.
// It checks that synctest.Wait waits for all tasks to exit before returning.
...
// TestSleepNondeterminism verifies that tasks sleeping to the same instant
...
func TestCond(t *testing.T) {
	synctest.Run(func() {
		...
		// Wait for cond.Broadcast in two tasks.
		waiterDone := false
		go func() {
			mu.Lock()
			cond.Wait()
...
func TestHappensBefore(t *testing.T) {
	// Use two parallel tasks accessing different vars to ensure that
	// we correctly account for multiple tasks in the bubble.
	var v1 int
	var v2 int
	synctest.Run(func() {
		v1++ // 1
		...
		// Wait returns after these tasks exit.
		go func() {
			v1++ // 2
		}()
		go func() {
		...
		// Wait returns after these tasks block.
		...
		// Wait returns after these timer tasks block.
		ch2 := make(chan struct{})
		time.AfterFunc(0, func() {
			v1++ // 8
			<-ch2
