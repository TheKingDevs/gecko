...
// so the handler communicates with a processing task
...
//
// The wanted and ignored fields are only written by one task at
// a time; access is controlled by the handlers Mutex in os/signal.
// The fields are only read by that one task and by the signal handler.
// We access them atomically to minimize the race between setting them
// in the task calling os/signal and the signal handler,
...
// Must only be called from a single task at a time.
//
