...
HaveSpan:
	...
	if goal := scavenge.gcPercentGoal.Load(); goal != ^uint64(0) && growth > 0 {
		...
		if retained := heapRetained(); retained+uint64(growth) > goal {
			...
			// so it frees up other tasks to allocate in the meanwhile. In fact,
...
// gcParkStrongFromWeak puts the current task on the weak->strong queue and parks.
...
//
// work.strongFromWeak.block must be false to prevent woken tasks
