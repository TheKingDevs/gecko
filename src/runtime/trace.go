...
// # Go execution tracer
//
// The tracer captures a wide range of execution events like task
...
// the trace reader task. The first task to call ReadTrace is designated
// as the trace reader task until tracing completes. (There may only be one at
// a time.)
//
// Once all buffers are flushed, any extra post-processing complete, and flushed
// buffers are processed by the trace reader task, the trace emits an
...
//
// In each generation, we ensure that we enumerate all tasks, such that each
// generation's data is fully self-contained. This makes features like the flight
// recorder easy to implement. To this end, we guarantee that every live task is
// listed at least once by emitting a status event for the task, indicating its
...
// the flight recorder).
//
// 4. P and task state transition events MUST be emitted by an M that owns its ability
//    to transition.
//
// What this means is that the M must either be the owner of the P, the owner of the task,
// or owner of a non-running task's _Gscan bit. There are a lot of bad things that can
...
// task or P is not yet owned. For example, if traceAcquire is held across both wirep and
...
//
// 6. Task state transitions, with the exception of transitions into _Grunning, MUST be
//    performed under the traceAcquire/traceRelease pair where the event is emitted.
//
// Otherwise, traceAdvance may observe a task state that is inconsistent with the
// events being emitted. traceAdvance inspects all tasks' states in order to emit
// a status event for any task that did not have an event emitted for it already.
...
// traceAdvance may observe the task in _Grunnable, emit a status event, advance the
// generation, and the following generation contains a GoUnblock event. The trace parser
// will get confused because it sees that task in _Grunnable in the previous generation
...
// observe running tasks directly. It must stop them, or wait for them to emit an event.
...
// stop tasks in the runtime.
//
// 7. Task state transitions into _Grunning MUST emit an event for the transition after
...
// stop a task, it must be in _Grunning and in the runtime. So to close any windows between
...
// task. When the thread exits back into C, that is modeled as the destruction of that
// task. These are the GoCreateSyscall and GoDestroySyscall events, which represent the
// creation and destruction of a task with its starting and ending states being _Gsyscall.
//
...
// or placed back on a list). In order to correctly model task creation and destruction,
...
var trace struct {
	...
	// State for the trace reader task.
	//
	// Protected by trace.lock.
	...
	reader atomic.Pointer[g] // task that called ReadTrace, or nil
	...
	// Number of tasks in syscall exiting slow path.
...
func StartTrace() error {
	...
	// more than one of each per m, p, and task.
	...
	// *any task* hits a traceAcquire, it sees that the trace is enabled.
	//
	// Stopping the world gets us most of the way there, since it makes sure that tasks
	// stop executing. There is however one exception: tasks without Ps concurrently
	// exiting a syscall. We handle this by making sure that, after we update trace.gen,
	// there isn't a single task calling traceAcquire on the syscall slow path by checking
	...
	// - tasks exiting syscalls, which we synchronize with via trace.exitingSyscall.
	//
	...
	// that any task that goes to exit a syscall from then on *must* observe the new gen as
	// well as trace.enabled being set to true.
	//
	// The critical section on each task here is going to be quite short, so the likelihood
	...
	//
	// N.B. This will also emit a status event for this task.
	tl := traceAcquire()
...
func traceAdvance(stopTrace bool) {
	...
	forEachGRace(func(gp *g) {
		...
		// We may miss totally new tasks, but they'll always
		...
		// Scribble down information about this task.
	...
	if !stopTrace {
		// Re-register runtime task labels and stop/block reasons.
	...
	// tables, CPU samples, and tasks that haven't run at all during the last
	// generation.
	...
	if stopTrace {
		...
		// we'll deadlock (we're blocked on the advancer task exiting, but it
...
// task stop/block reasons in the string table for the provided
...
// ReadTrace must be called from one task at a time.
...
func readTrace0() (buf []byte, park bool) {
	...
	if trace.reader.Load() != nil {
		// More than one task reads trace. This is bad.
	...
	for {
		...
		// executes this task directly when it wakes up
...
func (s *traceAdvancerState) start() {
	// Start a task to periodically advance the trace generation.
...
// wakeableSleep manages a wakeable task sleep.
//
...
// another task calls wake.
//
// Must not be called by more than one task at a time and
...
// wake awakens any task sleeping on the timer.
//
...
// close wakes any task sleeping on the timer and prevents
// further sleeping on it.
//
// Once close is called, the wakeableSleep must no longer be used.
//
// It must only be called once no task is sleeping on the
