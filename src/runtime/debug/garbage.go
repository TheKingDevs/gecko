...
// can be used by a single task stack.
// If any task exceeds this limit while growing its stack,
// the program crashes.
...
// tasks that enter an infinite recursion. It only limits future
// stack growth.
...
// of tasks. A Go program creates a new thread only when a task
// is ready to run but all the existing threads are blocked in system calls, cgo calls,
// or are locked to other tasks due to use of [runtime.LockOSThread].
//
...
// SetPanicOnFault applies only to the current task.
...
//
// WriteHeapDump suspends the execution of all tasks until the heap
...
// that the program prints all tasks when it crashes.
