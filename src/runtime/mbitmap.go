...
// to avoid being preempted; the GC must not stop the task
...
func heapSetTypeLarge(x, dataSize uintptr, typ *_type, span *mspan) uintptr {
	...
	//   1. It stops this task and scans its stack, or
	...
	// had to stop our task when it was preemptible during zeroing. For the
...
// or if the pointer is not in the heap. Used by task leak detection
...
func isMarkedOrNotInHeap(p unsafe.Pointer) bool {
	...
	// of runnable tasks. I don't think this is necessary, based on what we've seen, but
	// let's keep the option open in case the runtime evolves.
	// This will (naively) lead to quadratic blow-up for task leak detection,
...
//
// It is nosplit so it is safe for p to be a pointer to the current task's stack.
