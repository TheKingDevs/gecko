...
func testFreegc[T comparable](noscan bool) func(*testing.T) {
	...
	// to test longer or in some cases with more tasks.
	...
	return func(t *testing.T) {
		...
		if runtime.GOARCH == "wasm" {
			...
			// many tasks, which might be expected for wasm. This skip might no longer be
		...
		t.Run("duplicate-check", func(t *testing.T) {
			...
			// TODO: maybe add randomness? allow more live pointers? do across tasks?
		...
		t.Run("free-other-goroutine", func(t *testing.T) {
			// Use explicit free, but the free happens on a different task than the alloc.
		...
		t.Run("many-goroutines", func(t *testing.T) {
			// Allocate across multiple tasks, freeing on the same task.
		...
		t.Run("assist-credit", func(t *testing.T) {
			...
			// Use a background task to continuously run the GC.
...
func TestArenaHintChainGeneration(t *testing.T) {
	...
	if !RandomizeHeapBase {
		t.Skip("heap base randomization is disabled in this configuration")
	}
