...
such as functions to control tasks. It also includes the low-level type information
...
It is a comma-separated list of name=val pairs setting these named variables:
	...
	tasks, which makes it possible to traceback all tasks, and
	keeps their state close to the point of panic. Setting
	dontfreezetheworld=1 disables this preemption, allowing tasks to
	continue executing during panic processing. Note that tasks that
	...
	gcshrinkstackoff: setting gcshrinkstackoff=1 disables moving tasks
	onto smaller stacks. In this mode, a task's stack can only grow.
	...
	processors, threads and tasks.

	schedtrace: setting schedtrace=X causes the scheduler to emit a single line to standard
	error every X milliseconds, summarizing the scheduler state.

	tracebackancestors: setting tracebackancestors=N extends tracebacks with the stacks at
	which tasks were created, where N limits the number of ancestor tasks to
	...
	asynchronous task preemption. This makes some loops
	non-preemptible for long periods, which may delay GC and
	task scheduling. This is useful for debugging GC issues
	because it also disables the conservative stack scanning used
	for asynchronously preempted tasks.
...
By default, a failure prints a stack trace for the current task,
eliding functions internal to the run-time system, and then exits with exit code 2.
The failure prints stack traces for all tasks if there is no current task
or the failure is internal to the run-time.
GOTRACEBACK=none omits the task stack traces entirely.
GOTRACEBACK=single (the default) behaves as described above.
GOTRACEBACK=all adds stack traces for all user-created tasks.
GOTRACEBACK=system is like “all” but adds stack frames for run-time functions
and shows tasks created internally by the run-time.
...
things:
  ...
  - When a signal is received that terminates the program, or the program
    ...
    of GOTRACEBACK, the task stack, registers, and other memory related
...
// the calling task's stack. The argument skip is the number of stack frames
...
// on the calling task's stack. The argument skip is the number of stack frames
