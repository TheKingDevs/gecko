...
//  except for the case of an unbuffered channel with a single task
...
// only operations where one running task writes to the stack of
// another running task. The GC assumes that stack writes only
// happen when the task is running and are only done by that
// task. Using a write barrier is sufficient to make up for
...
func (q *waitq) dequeue() *sudog {
	for {
		...
		// if a task was put on this queue because of a
		// select, there is a small window between the task
		// being woken up by a different case and it grabbing the
		// channel locks. Once it has the lock
		// it removes itself from the queue, so we won't see it after that.
		// We use a flag in the G struct to tell us when someone
		// else has won the race to signal this task but the task
		// hasn't removed itself from the queue yet.
		if sgp.isSelect {
			if !sgp.g.selectDone.CompareAndSwap(0, 1) {
				// We lost the race to wake this task.
				continue
			}
		}
