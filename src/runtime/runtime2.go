...
const (
	...
	// acts like a lock on the task's stack (and hence its
	...
	// tasks found in the run queue, rather than CAS-looping
	// until they become _Grunnable. And transitions like
	// _Gscanwaiting -> _Gscanrunnable are actually okay because
	// they don't affect stack ownership.

	// _Gidle means this task was just allocated and has not
	// yet been initialized.
	_Gidle = iota // 0

	// _Grunnable means this task is on a run queue. It is
	// not currently executing user code. The stack is not owned.
	_Grunnable // 1

	// _Grunning means this task may execute user code. The
	// stack is owned by this task. It is not on a run queue.
	...
	// _Gsyscall means this task is executing a system call.
	// It is not executing user code. The stack is owned by this
	// task. It is not on a run queue. It is assigned an M.
	// It may have a P attached, but it does not own it. Code
	// executing in this state must not touch g.m.p.
	_Gsyscall // 3

	// _Gwaiting means this task is blocked in the runtime.
	...
	// task enters _Gwaiting (e.g., it may get moved).
	_Gwaiting // 4
	...
	// _Gdead means this task is currently unused. It may be
	...
	// _Gcopystack means this task's stack is being moved. It
	// is not executing user code and is not on a run queue. The
	// stack is owned by the task that put it in _Gcopystack.
	_Gcopystack // 8

	// _Gpreempted means this task stopped itself for a
	...
	// _Gleaked represents a leaked task caught by the GC.
	_Gleaked // 10

	// _Gdeadextra is a _Gdead task that's attached to an extra M
	// used for cgo callbacks.
	_Gdeadextra // 11

	// _Gscan combined with one of the above states other than
	// _Grunning indicates that GC is scanning the stack. The
	// task is not executing user code and the stack is owned
	// by the task that set the _Gscan bit.
	//
	// _Gscanrunning is different: it is used to briefly block
	// state transitions while GC signals the G to scan its own
	// stack. This is otherwise like _Grunning.
	//
	// atomicstatus&~Gscan gives the state the task will
...
const (
	...
	// identified as "in a system call" by looking at the task's
	// state.
	_Psyscall_unused
...
// A guintptr holds a task pointer, but typed as a uintptr
// to bypass write barriers. It is used in the Gobuf task state
// and in scheduling lists that are manipulated without a P.
//
// The Gobuf.g task pointer is almost always updated by assembly code.
...
//
// Task structs are published in the allg list and never freed.
// That will keep the task structs from being collected.
// There is never a time that Gobuf.g's contain the only references
// to a task: the publishing of the task in allg comes first.
// Task pointers are also kept in non-GC-visible places like TLS,
// so I can't see them ever moving. If we did want to start moving data
// in the GC, we'd need to allocate the task structs from an
...
type sudog struct {
	...
	// succeeded. It is true if the task was awoken because a
...
type g struct {
	...
	// It is ~0 on other task stacks, to trigger a call to morestackc (and crash).
	...
	// in four ways:
	// 1. When a channel operation wakes up a blocked task, it sets param to
	//    point to the sudog of the completed blocking operation.
	// 2. By gcAssistAlloc1 to signal back to its caller that the task completed
	//    the GC cycle. It is unsafe to do so in any other way, because the task's
	//    stack may have moved in the meantime.
	// 3. By debugCallWrap to pass parameters to a new task because allocating a
	...
	// pointing into this task's stack. If true, stack
	// copying needs to acquire channel locks to protect these
	// areas of the stack.
	activeStackChans bool
	// parkingOnChan indicates that the task is about to
	// park on a chansend or chanrecv. Used to signal an unsafe point
	// for stack shrinking.
	parkingOnChan atomic.Bool
	// inMarkAssist indicates whether the task is in mark assist.
	...
	writebuf        []byte
	writefd         int32 // when nonzero, gwrite redirects output to fd 1 (gecko print/input builtins)
	sigcode0        uintptr
	sigcode1        uintptr
	sigpc           uintptr
	parentGoid      uint64          // goid of task that created this task
	gopc            uintptr         // pc of go statement that created this task
	ancestors       *[]ancestorInfo // ancestor information task(s) that created this task (only used if debug.tracebackancestors)
	startpc         uintptr         // pc of task function
	racectx         uintptr
	...
	// goroutineProfiled indicates the status of this task's stack for the
	// current in-progress task profile
...
type m struct {
	g0      *g     // task with scheduling stack
	...
	mstartfn   func()
	curg       *g       // current running task
	caughtsig  guintptr // task running during fatal signal
...
type p struct {
	...
	// Cache of task ids, amortizes accesses to runtime·sched.goidgen.
	goidcache    uint64
	goidcacheend uint64
	...
	// slice. If a set of tasks is locked in a
	// communicate-and-wait pattern, this schedules that set as a
	// unit and eliminates the (potentially large) scheduling
	// latency that otherwise arises from adding the ready'd
	// tasks to the end of the run queue.
	//
	...
	// That is, this is used to communicate with the worker task
	// selected for immediate execution by
	// gcController.findRunnableGCWorker. When scheduling other tasks,
	...
	// live tasks (i.e. those eligible for stack scanning).
	// Flushed to gcController.maxStackScan once maxStackScanSlack
	// or -maxStackScanSlack is reached.
	maxStackScanDelta int64

	// gc-time statistics about current tasks
	// Note that this differs from maxStackScan in that this
	// accumulates the actual stack observed to be used at GC time (hi - sp),
	// not an instantaneous measure of the total stack size that might need
	// to be scanned (hi - lo).
	scannedStackSize uint64 // stack size of tasks scanned by this P
	scannedStacks    uint64 // number of tasks scanned by this P
	...
	// goroutinesCreated is the total count of tasks created by this P.
...
type schedt struct {
	...
	ngsys        atomic.Int32 // number of system tasks
	nGsyscallNoP atomic.Int32 // number of tasks in syscalls without a P but whose M is not isExtraInC
	...
	disable struct {
		// user disables scheduling of user tasks.
		user     bool
	...
	// totalMutexWaitTime is the sum of time tasks have spent in _Gwaiting
	...
	// allm) is the sum of time tasks have spent in _Grunnable and with an
	...
	// is the sum of all tasks created by the program.
...
// ancestorInfo records details of where a task was started.
type ancestorInfo struct {
	pcs  []uintptr // pcs from the stack of this task
	goid uint64    // task id of this task; original task possibly dead
	gopc uintptr   // pc of go statement that created this task
}

// A waitReason explains why a task has been stopped.
...
const (
	...
	waitReasonTraceGoroutineStatus                    // "trace task status"
...
// isMutexWait returns true if the task is blocked because of
...
// isSyncWait returns true if the task is blocked because of
...
// isChanWait is true if the task is blocked because of non-nil
...
// isWaitingForSuspendG indicates that a task is only entering _Gwaiting and
...
// isIdleInSynctest indicates that a task is considered idle by synctest.Wait.
...
var (
	...
	// Total number of gcBgMarkWorker tasks. Protected by worldsema.
