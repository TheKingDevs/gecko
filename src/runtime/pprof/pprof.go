...
//
// A Profile's methods can be called from multiple tasks simultaneously.
//
// Each Profile has a unique name. A few profiles are predefined:
//
//	task      - stack traces of all current tasks
//	goroutineleak  - stack traces of all leaked tasks
...
// contention. For example, a lock held for a long time while other tasks
// are waiting to acquire the lock will report contention when the lock is
// finally unlocked (that is, at [sync.Mutex.Unlock]).
//
// Sample values correspond to the approximate cumulative time other tasks
...
// holds a lock for 1s while 5 other tasks are waiting for the entire
...
// goroutineLeakProfileLock ensures that the task leak profile writer observes the
// leaked tasks discovered during the task leak detection GC cycle
...
// when multiple profile requests are issued concurrently: the status of leaked tasks
...
// for example, when printing the "task" profile, debug=2 means to
// print the task stacks in the same form that a Go program uses
...
// countGoroutine returns the number of tasks.
...
// writeGoroutineLeak first invokes a GC cycle that performs task leak detection.
// It then writes the task profile, filtering for leaked tasks.
func writeGoroutineLeak(w io.Writer, debug int) error {
	// Acquire the task leak detection lock and release
	// it after the task leak profile is written.
	//
	// While the critical section is long, this is needed to prevent
	// a race condition between the garbage collector and the task
	...
	// Run the GC with leak detection first so that leaked tasks
	// may transition to the leaked state.
	runtime_goroutineLeakGC()

	// If the debug flag is set sufficiently high, just defer to writing task stacks
	// like in a regular task profile. Include non-leaked tasks, too.
	if debug >= 2 {
		return writeGoroutineStacks(w)
	}

	// Otherwise, write the task leak profile.
	return writeRuntimeProfile(w, debug, "goroutineleak", pprof_goroutineLeakProfileWithLabels)
}

func writeGoroutineStacks(w io.Writer) error {
	// We don't know how big the buffer needs to be to collect
	// all the tasks. Start with 1 MB and try a few times, doubling each time.
