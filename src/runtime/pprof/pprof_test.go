...
func TestCPUProfileMultithreadMagnitude(t *testing.T) {
	...
	// Run a workload in a single task, then run copies of the same
	// workload in several tasks. For both the serial and parallel cases,
...
// Test that profiler does not observe runtime.gogo as "user" task execution.
...
func TestMorestack(t *testing.T) {
	...
	testCPUProfile(t, matches, func(duration time.Duration) {
		...
		for {
			go func() {
				growstack1()
				// NOTE(vsaioc): This task may leak without this select.
				select {
				case c <- true:
...
// shows a task in the given state with a stack frame in
// runtime/pprof.<fName>.
func awaitBlockedGoroutine(t *testing.T, state, fName string, count int) {
	...
	for {
		...
		if n == len(buf) {
			// Buffer wasn't large enough for a full task dump.
...
func TestGoroutineCounts(t *testing.T) {
	// Setting GOMAXPROCS to 1 ensures we can force all tasks to the
	...
	time.Sleep(10 * time.Millisecond) // let tasks exit
}
...
func TestGoroutineLeakProfileConcurrency(t *testing.T) {
	...
	// Regular task profile. Used to check that there is no interference between
	// the two profile types.
	goroutineProf := Lookup("goroutine")
	goroutineLeakProf := goroutineLeakProfile

	// We use this helper to count the total number of leaked tasks in a text profile.
	...
	// the tasks we'll leak in this test.
	...
	// Leak some tasks that will feature in the task leak profile
	const totalLeaked = leakCount * 2
	for i := 0; i < leakCount; i++ {
		go goroutineLeakExample()
		go func() {
			// Leak another task that will feature a slightly different stack.
	...
	// Wait for the tasks to leak. We might wait here until the timeout,
	...
	// Concurrent calls to the task leak profiler should not trigger data races
	// or corruption.
	...
	// TODO(thepudds,vsaioc): the next two subtests would ideally find totalLeaked tasks,
	...
	// Concurrent calls to the task leak profiler should not trigger data races
	// or corruption, or interfere with regular task profiles.
			...
			Do(ctx, Labels("i", fmt.Sprint(i)), func(context.Context) {
				...
				go func() {
					...
					for ctx.Err() == nil {
						var w strings.Builder
						goroutineProf.WriteTo(&w, 1)
						// The regular task profile should see the leaked
						// tasks. We simply check that the task leak
						// profile does not corrupt the task profile state.
...
func TestGoroutineProfileConcurrency(t *testing.T) {
	...
	// Concurrent calls to the task profiler should not trigger data races
	// or corruption.
	t.Run("overlapping profile requests", func(t *testing.T) {
		ctx := context.Background()
	...
	// The finalizer task should not show up in most profiles, since it's
	// marked as a system task when idle.
	...
	// The finalizer task should show up when it's running user code.
	...
	// Check that new tasks only show up in order.
	testLaunches := func(t *testing.T) {
		...
		// These tasks all survive until the end of the subtest, so we can
		// check that a (numbered) task appearing in the profile implies
		// that all older tasks also appear in the profile.
		ready.Add(1)
		done.Add(1)
		go func() {
			defer done.Done()
			for i := 0; ctx.Err() == nil; i++ {
				...
				// extra task (this one) with most recent label.
				...
				for j := 0; j < i; j++ {
					...
					// task will do O(N^2) work with the number of
					// tasks it launches. This should be slow relative to
					// the work involved in collecting a task profile,
					// which is O(N) with the high-water mark of the number of
					// tasks in this process (in the allgs slice).
					runtime.Gosched()
				}
				if i == 0 {
					ready.Done()
		...
		// Short-lived tasks exercise different code paths (tasks with
		...
		churn = func(i int) {
			...
			if i == 0 {
				...
				// Yield on occasion so this sequence of task launches
		...
		for i := range w {
			...
			// High-numbered loop-i tasks imply that every lower-numbered
			// loop-i task should be present in the profile too.
...
func TestGoroutineProfileCoro(t *testing.T) {
	...
	// Set up a task to just create and run coroutine tasks all day.
	iterFunc := func() {
		p, stop := iter.Pull2(
	...
	// Take a task profile. If the bug in #69998 is present, this will crash
	// with high probability. We don't care about the output for this bug.
	goroutineProf.WriteTo(io.Discard, 1)
}

// This test tries to provoke a situation wherein the finalizer task is
// erroneously inspected by the task profiler in such a way that could
...
func TestGoroutineProfileIssue74090(t *testing.T) {
	...
	for range 10 {
		...
		// system and user task on each call, since there's substantially
		...
		// Try to run a task profile concurrently with finalizer execution
		// to trigger the bug.
		var w strings.Builder
		goroutineProf.WriteTo(&w, 1)
	}
...
func BenchmarkGoroutine(b *testing.B) {
	...
	// Note that some costs of collecting a task profile depend on the
	// length of the runtime.allgs slice, which never shrinks. Stay within race
	// detector's 8k-task limit
...
// TestLabelSystemstack makes sure CPU profiler samples of tasks running
		...
		for _, loc := range s.Location {
			for _, l := range loc.Line {
				...
				case "runtime.bgsweep", "runtime.bgscavenge", "runtime.forcegchelper", "runtime.gcBgMarkWorker", "runtime.runFinalizers", "runtime.runCleanups", "runtime.sysmon":
					// Runtime system tasks or threads
...
func labelHog(stop chan struct{}, gogc int) {
	...
	// be initially triggered by a labelled task.
	runtime.GC()
...
// parallelLabelHog runs GOMAXPROCS tasks running labelHog.
