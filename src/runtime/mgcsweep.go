...
func bgsweep(c chan int) {
	...
	for {
		// bgsweep attempts to be a "low priority" task by intentionally
		// yielding time. It's OK if it doesn't run, because tasks allocating
		...
		if !isSweepDone() {
			...
			unlock(&sweep.lock)
			// This task must preempt when we have no work to do
...
func sweepone() uintptr {
	...
	// Increment locks to ensure that the task is not preempted
...
func (s *mspan) ensureSwept() {
	...
	// (if GC is triggered on another task).
	gp := getg()
