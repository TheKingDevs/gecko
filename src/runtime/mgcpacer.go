...
const (
	...
	// where tasks are unlikely to assist at all, so the actual
...
type gcControllerState struct {
	...
	// maxStackScan is the amount of allocated task stack space in
	// use by tasks.
	//
	// This number tracks allocated task stack space rather than used
	// task stack space (i.e. what is actually scanned) because used
	// task stack space is much harder to measure cheaply. By using
	...
	//
	// For example, consider a situation in which tasks block on the GC
...
// there's no background mark worker tasks in the pool. This case is
