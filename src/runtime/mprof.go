...
// SetBlockProfileRate controls the fraction of task blocking events
...
// The traceback will be recorded from the stack of the task associated with the current m.
...
// goroutineProfileState indicates the status of a task's stack for the
// current in-progress task profile. Tasks' stacks are initially
// "Absent" from the profile, and end up "Satisfied" by the time the profile is
// complete. While a task's stack is being captured, its
...
//
// Some tasks (the finalizer task, which at various times can be
// either a "system" or a "user" task, and the task that is
// coordinating the profile, any tasks created during the profile) move
...
func goroutineLeakProfileWithLabelsConcurrent(p []profilerecord.StackRecord, labels []unsafe.Pointer) (n int, ok bool) {
	...
	// Prepare a profile large enough to store all leaked tasks.
	...
	// Visit each leaked task and try to record its stack.
	var offset int
...
func goroutineProfileWithLabelsConcurrent(p []profilerecord.StackRecord, labels []unsafe.Pointer) (n int, ok bool) {
	...
	// of the number of live tasks, minus the number of tasks that are
	// alive and permanently marked as "system". But to make this count agree
	// with what we'd get from isSystemGoroutine, we need special handling for
	// tasks that can vary between user and system to ensure that the count
	// doesn't change during the collection. So, check the finalizer task
	// and cleanup tasks in particular.
	n = int(gcount(false))
	...
	// Prepare for all other tasks to enter the profile. Aside from ourg,
	// every task struct in the allgs list has its goroutineProfiled field
	// cleared. Any task created from this point on (while
	...
	// Visit each task that existed as of the startTheWorld call above.
	//
	// New tasks may not be in this list, but we didn't want to know about
	// them anyway. If they do appear in this list (via reusing a dead task
	...
	//
	// Any task that the scheduler tries to execute concurrently with this
	...
	// Restore the invariant that every task struct in allgs has its
	...
	if n != int(endOffset) {
		// It's a big surprise that the number of tasks changed while we
		// were collecting the profile. But probably better to return a
		// truncated profile than to crash the whole process.
		//
		// For instance, needm moves a task out of the _Gdeadextra state and so
		// might be able to change the task count without interacting with
...
// in the current task profile: either that it should not be profiled, or
...
func tryRecordGoroutineProfile(gp1 *g, pcbuf []uintptr, yield func()) {
	if status := readgstatus(gp1); status == _Gdead || status == _Gdeadextra {
		// Dead tasks should not appear in the profile. Tasks that
	...
	for {
		...
		if prev == goroutineProfileSatisfied {
			// This task is already in the profile (or is new since the
			// start of collection, so shouldn't appear in the profile).
			break
		}
		if prev == goroutineProfileInProgress {
			// Something else is adding gp1 to the task profile right now.
...
// task profile. Preemption is disabled.
//
// This may be called via tryRecordGoroutineProfile in two ways: by the
// task that is coordinating the task profile (running on its own
// stack), or from the scheduler in preparation to execute gp1 (running on the
// system stack).
func doRecordGoroutineProfile(gp1 *g, pcbuf []uintptr) {
	if isSystemGoroutine(gp1, false) {
		// System tasks should not appear in the profile.
		// Check this here and not in tryRecordGoroutineProfile because isSystemGoroutine
		// may change on a task while it is executing, so while the scheduler might
		// see a system task, goroutineProfileWithLabelsConcurrent might not, and
		// this inconsistency could cause invariants to be violated, such as trying to
		// record the stack of a running task below. In short, we still want system
		// tasks to participate in the same state machine on gp1.goroutineProfiled as
	...
	// exitsyscall where a task could be in _Grunning as it's exiting a syscall.
	// This is OK because task will not exit the syscall until it passes through
	...
	//
	// When called from the task coordinating the profile, we still have
...
func goroutineProfileWithLabelsSync(p []profilerecord.StackRecord, labels []unsafe.Pointer) (n int, ok bool) {
	...
	if n <= len(p) {
		...
		// If we have a place to put our task labelmap, insert it there.
		if labels != nil {
			lbl[0] = gp.labels
			lbl = lbl[1:]
		}
...
// GoroutineProfile returns n, the number of records in the active task stack profile.
...
func saveg(pc, sp uintptr, gp *g, r *profilerecord.StackRecord, pcbuf []uintptr) {
	...
	// to allocate one here. This will only happen for tasks that were in a
	// syscall when the task profile started or for tasks that manage
	// to execute before we finish iterating over all the tasks.
	if pcbuf == nil {
		pcbuf = makeProfStack()
	}
...
// Stack formats a stack trace of the calling task into buf
// and returns the number of bytes written to buf.
// If all is true, Stack formats stack traces of all other tasks
// into buf after the trace for the current task.
