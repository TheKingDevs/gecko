...
//
// The former happens on a task much like the background sweeper which is
...
type scavengerState struct {
	...
	// g is the task the scavenger is bound to.
	g *g
...
//
// Must be called from a regular task that can allocate.
...
func (s *scavengerState) wake() {
	...
	if s.parked {
		...
		// Ready the task by injecting it. We use injectglist instead
		// of ready or goready in order to allow us to run this function
		// without a P. injectglist also avoids placing the task in
		// the current P's runnext slot, which is desirable to prevent
		// the scavenger from interfering with user task scheduling
		// too much.
		var list gList
		list.push(s.g)
		injectglist(&list)
...
//
// Must be run on the scavenger task.
...
func (p *pageAlloc) scavengeOne(ci chunkIdx, searchIdx uint, max uintptr) uintptr {
	...
	if p.summary[len(p.summary)-1][ci].max() >= uint(minPages) {
		...
		if npages != 0 {
			...
			// we don't want any allocating tasks to grab it while
