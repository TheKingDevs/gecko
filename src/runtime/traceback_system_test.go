...
// parseable list of program counters for the running task that
...
// first running task out of a GOTRACEBACK=system traceback,
...
func parseStackPCs(crash string) ([]uintptr, error) {
	...
	var (
		...
		on             = false // are we in the first running task?
		lines          = strings.Split(crash, "\n")
		symLine        = true // within a task, every other line is a symbol or file/line/pc location, starting with symbol.
		currSymbol     string
	...
	for i := 0; i < len(lines); i++ {
		...
		// A blank line marks end of a task stack.
		if line == "" {
			break
		}

		// Skip the final "created by SYMBOL in task GID" part.
