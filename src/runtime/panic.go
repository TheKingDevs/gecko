...
const (
	...
	//
	// These throws do not include runtime frames, system tasks, or
	// frame metadata.
	throwTypeUser
	...
	// debugging the runtime, including runtime frames, system tasks,
	// and frame metadata.
	throwTypeRuntime
)
...
// The fact that it is a separate list from the main task
// defer list means that the main task's defers can still
...
// Goexit terminates the task that calls it. No other task is affected.
// Goexit runs all deferred calls before terminating the task. Because Goexit
// is not a panic, any recover calls in those deferred functions will return nil.
//
// Calling Goexit from the main task terminates that task
// without func main returning. Since func main has not returned,
// the program continues execution of other tasks.
// If all other tasks exit, the program crashes.
//
...
//
// fatal does not include runtime frames, system tasks, or frame metadata
...
func fatalpanic(msgs *_panic) {
	...
	systemstack(func() {
		...
		// print stacks for the tasks in the bubble.
...
// always g0.
// If bubble is non-nil, print the stacks for tasks in this group as well.
	...
	if level > 0 {
		...
		if !didothers {
			if all {
				...
				// Print stacks for tasks in the deadlocked bubble.
