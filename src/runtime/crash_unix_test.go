...
func TestCrashDumpsAllThreads(t *testing.T) {
	...
	if strings.Contains(os.Getenv("GKFLAGS"), "mayMoreStackPreempt") {
	...
	// "task running on other thread; stack unavailable".
	out := outbuf.Bytes()
...
func init() {
	if len(os.Args) >= 2 && os.Args[1] == "testPanicSystemstackInternal" {
		...
		// block tasks on runtime locks, which aren't ever preemptible for the
		// GC to scan them.
		runtime.GC()
