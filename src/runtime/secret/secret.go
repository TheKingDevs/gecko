...
//     Do itself.
//
// Any task spawned while executing f will act as if the entire task
...
// Enabled reports whether the current task
// is running in secret mode. This is usually through a call to
// [Do], but can also occur when a task already running in
// secret mode launches another task.
func Enabled() bool {
	return count() > 0
}
