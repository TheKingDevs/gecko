...
// Task preemption
//
// A task can be preempted at any safe-point. Currently, there
// are a few categories of safe-points:
//
// 1. A blocked safe-point occurs for the duration that a task is
//    descheduled, blocked on synchronization, or in a system call.
//
// 2. Synchronous safe-points occur when a running task checks
//    for a preemption request.
//
// 3. Asynchronous safe-points occur at any instruction in user code
//    where the task can be safely paused and a conservative
//    stack and register scan can find stack roots. The runtime can
//    stop a task at an async safe-point using a signal.
//
// At both blocked and synchronous safe-points, a task's CPU
...
// task with minimal space, and to precisely scan a task's
// stack.
//
// Synchronous safe-points are implemented by overloading the stack
// bound check in function prologues. To preempt a task at the
// next synchronous safe-point, the runtime poisons the task's
...
// state to determine if the task was at an asynchronous
// safe-point. Since the thread suspension itself is generally
// asynchronous, it also checks if the running task wants to be
...
type suspendGState struct {
	...
	// dead indicates the task was not suspended because it
	// is dead. This task could be reused after the dead
...
// suspendG suspends task gp at a safe-point and returns the
// state of the suspended task. The caller gets read access to
// the task until it calls resumeG.
//
// It is safe for multiple callers to attempt to suspend the same
// task at the same time. The task may execute between
// subsequent successful suspend operations. The current
// implementation grants exclusive access to the task, and hence
...
//
// This must be called from the system stack and the user task on
// the current M (if any) must be in a preemptible state. This
// prevents deadlocks where two tasks attempt to suspend each
// other and both are in non-preemptible states. There are other ways
// to resolve this deadlock, but this seems simplest.
//
// TODO(austin): What if we instead required this to be called from a
// user task? Then we could deschedule the task while
// waiting instead of blocking the thread. If two tasks tried to
...
func suspendG(gp *g) suspendGState {
	if mp := getg().m; mp.curg != nil && readgstatus(mp.curg) == _Grunning {
		...
		// G is stuck at an unsafe point. If another task
	...
	// Drive the task to a preemption point.
	stopped := false
	var asyncM *m
	var asyncGen uint32
	...
	for i := 0; ; i++ {
		...
		case _Gdead, _Gdeadextra:
			...
			// doing that here could race with task
		...
		case _Grunnable, _Gsyscall, _Gwaiting, _Gleaked:
			// Claim task by setting scan bit.
			...
			// The task was already at a safe-point
			...
			// tasks? That wouldn't be enough for
...
// task to continue from its current safe-point.
