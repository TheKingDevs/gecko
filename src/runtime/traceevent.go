...
//
// Before creating the event writer, this method will emit a status for the current task
// or proc if it exists, and if it hasn't had its status emitted yet. goStatus and procStatus indicate
// what the status of task or P should be immediately *before* the events that are about to
// be written using the eventWriter (if they exist). No status will be written if there's no active
// task or P.
//
// Callers can elect to pass a constant value here if the status is clear (e.g. a task must have
// been Runnable before a GoStart). Otherwise, callers can query the status of either the task
...
// startPC takes a start PC for a task and produces a unique
// stack ID for it.
//
// It then returns a traceArg representing that stack which may be
// passed to write.
