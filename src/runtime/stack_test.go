...
func TestStackMem(t *testing.T) {
	...
	for b := 0; b < BatchCount; b++ {
		...
		// The tasks have signaled via c that they are ready to exit.
...
func TestStackGrowth(t *testing.T) {
	...
	if d, ok := t.Deadline(); ok {
		...
		timer := time.AfterFunc(timeout, func() {
			...
			// that we get a useful task dump if the test times out, especially
...
func TestStackGrowthCallback(t *testing.T) {
	...
	// test stack growth at task creation
	wg.Add(1)
	go func() {
		defer wg.Done()
		growStackWithCallback(func() {
...
// the end of the stack. Each trial runs in a different task so that an earlier
...
func TestStackCache(t *testing.T) {
	// Allocate a bunch of tasks and grow their stacks.
...
func BenchmarkIssue18138(b *testing.B) {
	// Channel with N "can run a task" tokens
	const N = 10
	c := make(chan []byte, N)
	for i := 0; i < N; i++ {
		c <- make([]byte, 1)
...
// growAndShrinkStack grows the stack of the current task in order to
