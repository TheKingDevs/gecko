...
// is necessary while the calling task's stack is grey. In
...
// this will shade it.
//
// 3. Once a task's stack is black, the shade(ptr) becomes
...
// reachable by some task that currently cannot reach it.
//
//
...
func typedmemmove(typ *abi.Type, dst, src unsafe.Pointer) {
	...
	// There's a race here: if some other task can write to
	// src, it may change some pointer in src after we've
	// performed the write barrier but before we perform the
	// memory copy. This safe because the write performed by that
	// other task must also be accompanied by a write
