...
func LockOSThreadMain() {
	...
	// Check that this task is still running on a different
	// thread.
	if mainTID != 0 && gettid() == mainTID {
...
func LockOSThreadAlt() {
	...
	ready := make(chan bool, 1)
	go func() {
		// This task must be running on a new thread.
		runtime.LockOSThread()
		subTID = gettid()
		ready <- true
	...
	for i := 0; i < 100; i++ {
		time.Sleep(1 * time.Millisecond)
		// Check that this task is running on a different thread.
...
func LockOSThreadAvoidsStatePropagation() {
	...
	// Spawn yet another task and lock it. Since GOMAXPROCS=1, if
