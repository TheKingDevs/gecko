...
func printStack() {
	...
	for {
		...
		if n < len(buf) {
			...
			// Delete any ignored tasks, if present.
			for all != "" {
				var g string
...
func recurseThenCallGo(w chan struct{}, frames int, goroutines int, main bool) {
	if frames == 0 {
		// Signal to TracebackAncestors that we are done recursing and starting tasks.
		w <- struct{}{}
		<-w
		return
	}
	if goroutines == 0 {
		// Record which task this is so we can ignore it
		...
		// Start the next task now that there are no more recursions left
