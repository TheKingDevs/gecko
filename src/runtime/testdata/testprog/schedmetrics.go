...
func SchedMetrics() {
	...
	// generalSlack is the amount of tasks we allow ourselves to be
	// off by in any given category, either due to background system
	// tasks. This excludes GC tasks.
	generalSlack := uint64(4)

	// waitingSlack is the max number of blocked tasks controlled
	// by the runtime that we'll allow for. This includes GC tasks
	// as well as finalizer and cleanup tasks.
	...
	// Force Running count to be high. We'll use these tasks
	// for Runnable, too.
	const count = 10
	...
	// tasks are blocked, like js/wasm, since we have a bunch
	// of runnable tasks all spinning. We cannot write anything
	// out.
	if testenv.HasParallelism() {
		...
		run("running", func() {
			...
			// distribute the tasks to Ps, so retry until
		...
		// Done with the running/runnable tasks.
		exit.Store(1)
	} else {
		// Read metrics and then exit all the other tasks,
		...
		run("created", func() {
			// Look for count-1 tasks because we read metrics
			// *before* run task was created for this sub-test.
	...
	run("not-in-go", func() {
		// Block a bunch of tasks on an OS pipe.
		pr, pw, err := pipe()
		if err != nil {
			switch runtime.GOOS {
			case "js", "wasip1":
