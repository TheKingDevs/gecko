...
func traceSTWTarget(i int) {
	for !stop.Load() {
		...
		// task scheduling. As a workaround, ensure we have a
...
func TraceSTW() {
	...
	// The idea here is to have 2 target tasks that are constantly
	// running. When the world restarts after STW, we expect these
	// tasks to continue execution on the same M and P.
	//
	// Set GOMAXPROCS=4 to make room for the 2 target tasks, 1 parent,
	// and 1 slack for potential misscheduling.
	//
	// Disable the GC because GC STW generally moves tasks (see
...
func TraceGCSTW() {
	...
	// tasks to continue execution on the same M and P.
	//
	// Set GOMAXPROCS=8 to make room for the 2 target tasks, 1 parent,
	...
	// Make sure to run long enough for the children to schedule again
	// after STW. This is included for good measure, but the tasks
...
func triggerGC() {
	...
	// other tasks before their Ps get up and running again.
