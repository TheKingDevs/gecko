...
// Common task leak patterns. Extracted from:
// "Unveiling and Vanquishing Task Leaks in Enterprise Microservices: A Dynamic Analysis Approach"
// doi:10.1109/CGO57630.2024.10444835
//
// Tests in this file are not flaky iff. the test is run with GOMAXPROCS=1.
// The main task forcefully yields via `runtime.Gosched()` before
// running the profiler. This moves them to the back of the run queue,
// allowing the leaky tasks to be scheduled beforehand and get stuck.
...
//		Failing to stop a worker results in a task leak
type worker struct {
	ch   chan any
	done chan any
}

// Start spawns a background task that extracts items pushed to the queue.
func (w worker) Start() {
	go func() {
...
func (w worker) Stop() {
	// Allows task created by Start to terminate
	close(w.done)
}
...
func workerLifecycle(items []any) {
	...
	runtime.Gosched()
	// Exits without calling ’Stop’. Task created by `Start` eventually leaks.
}
...
func doubleSend(ch chan any, err error) {
	...
	// This send is still executed in the error case, which may lead to a task leak.
	ch <- struct{}{}
}
...
// earlyReturn demonstrates a common pattern of task leaks.
// A return statement interrupts the evaluation of the parent task before it can consume a message.
...
func earlyReturn(err error) {
	...
	go func() {
		...
		// Leaks if the parent task terminates early.
		ch <- struct{}{}
	}()
...
func NCastLeak() {
	...
	defer func() {
		for i := 0; i < yieldCount; i++ {
			// Yield enough times  to allow all the leaky tasks to
