 ...
 * returns without draining the channel ch, so that tasks created by anonymous
 * function will leak.
 */
...
// Example of task leak trace:
//
// G1              G2                G3                     helper task
//--------------------------------------------------------------------------------------
// .							 .                                        r.sendChans()
// r.beginCmds()   .                                        .
// .               .                                        ch1 <-
...
func Cockroach10790() {
	...
	for i := 0; i < 100; i++ {
		go func() {
			...
			go r.sendChans(ctx) // helper task
