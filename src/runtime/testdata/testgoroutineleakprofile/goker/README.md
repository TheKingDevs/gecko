...
The examples have been modified in order to run the task leak profiler.
...
main program sets up a waiting period (typically 1ms), followed by a task
...
The resulting task leak profile is analyzed to ensure that no unexpecte
...
test failures, when really all we care about are leaked tasks.
...
This task leak is caused by different order when acquiring
...
One task executing `(*Tx).awaitDone()` blocks and
...
so that one task running `(*Rows).awaitDone` is blocked forever.
The blocking task is waiting for cancel signal from context.
...
tasks created by `lt.RunWorker` in `lt.start` to be stuck sending
a message over `lt.Events`. The main thread is then stuck at `s.stop.Wait()`,
since the sender tasks cannot call `s.stop.Done()`.
...
This is another example of task leaks caused by recursively
acquiring `RWLock`.
There are two lock variables (`systemConfigCond` and `systemConfigMu`)
which refer to the same underlying lock. The leak invovlves two tasks.
The first acquires `systemConfigMu.Lock()`, then tries to acquire `systemConfigMu.RLock()`.
The second acquires `systemConfigMu.Lock()`.
If the second task interleaves in between the two lock operations of the
first task, both tasks will leak.
...
The `context.Done()` signal short-circuits the reader task, but not
...
This bug is caused by two tasks waiting for each other
...
`RowChannel`. This could lead to task leaks in rare cases due
...
cause task leaks during flow cancellation when trying to propagate
...
The task leak is caused by acquiring a RLock twice in a call chain.
...
This causes any call to `client.Close()` to leak tasks.
...
lead to task leaks when sending over `n.status`.
...
This task leak involves a task acquiring a lock and being
...
Two tasks are involved in this leak. The main task
is blocked at `case <- donec`, and is waiting for the second task
to close the channel.
The second task is created by the main task. It is blocked
when calling `stream.Read()`, which invokes `recvBufferRead.Read()`.
The second task is blocked at case `i := r.recv.get()`, and it is
waiting for someone to send a message to this channel.
It is the `client.CloseSream()` method called by the main task that
...
The task running `cc.lbWatcher` returns without
...
(the default), the client can leak tasks when transitioning
between having no active stream and having one active
stream.The keepalive() task is stuck at “<-t.awakenKeepalive”,
while the main task is stuck in NewStream() on t.mu.Lock().
...
The task executing resetAddrConn is leaked. The patch is to
...
A task can hold `Lock()` at line 20 then acquire `RLock()` at
...
A task first acquire `contentInitMu` at line 99 then
...
A task holds a `Mutex` at line 91 and is then blocked at line 93.
Another task attempts to acquire the same `Mutex` at line 101 to
...
Task 1 is blocked on a lock held by task 3,
while task 3 is blocked on sending message to `ch`,
which is read by task 1.
...
leading to a task leak. `n.cond.Signal()` at line 59 and line 81 are not
...
This is a lock-channel bug. The first task invokes
`distribute()`, which holds `m.lock.Lock()`, while blocking
at sending message to `w.result`. The second task
invokes `stopWatching()` function, which can unblock the first
task by closing `w.result`. However, in order to close `w.result`,
...
task, while no need to hold `m.lock.Lock()`.
...
A potential task leak occurs when an error has happened,
...
A task holds a `Mutex` at line 24 and blocked at line 35.
Another task blocked at line 58 by acquiring the same `Mutex`.
...
If the main task selects a case that doesn’t consumes
the channels, the anonymous task will be blocked on sending
to channel.
...
`cond.Wait()` will be invoked. There is another task (task D),
which will periodically invoke `rq.workerLock.Lock()`. Under the following
situation, deadlock will happen. Queue 1 is empty, so that some tasks
hold `rq.workerLock.RLock()`, and block at `cond.Wait()`. Task D is
blocked when acquiring `rq.workerLock.Lock()`. Some tasks try to process
...
from any queue. Therefore, when a task is blocked at `cond.Wait()`,
...
the function creates a task and the task only
...
other tasks trying to acquire `devices.Lock()`.
...
The call to `t.Fatalf` terminated the current task which was consuming
...
One task may acquire a lock and try to send a message over channel `stop`,
while the other will try to acquire the same lock. With the wrong ordering,
both tasks will leak.
...
The task created using an anonymous function is blocked 
sending a message over an unbuffered channel. However there
exists a path in the parent task where the parent function
