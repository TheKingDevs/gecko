...
// SpawnGC spawns a tree of task leaks and calls the task leak profiler
// for each node in the tree. It is supposed to stress the task leak profiler
// under a heavily concurrent workload.
func SpawnGC() {
	spawnGC(spawnGCMaxDepth)
}

// DaisyChain spawns a daisy-chain of runnable tasks.
//
// Each task in the chain creates a new channel and task.
//
// This illustrates a pathological worstcase for the task leak GC complexity,
...
func DaisyChain() {
	...
	// The channel buffer avoids task leaks.
