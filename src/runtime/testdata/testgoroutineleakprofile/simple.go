...
// This is a set of micro-tests with obvious task leaks that
// ensures task leak detection works.
//
...
func NilRecv() {
	...
	// Yield several times to allow the child task to run.
...
func SelectNoCasesMain() {
	...
	go func() {
		...
		// Write a task leak profile from
		// the child task.
		prof.WriteTo(os.Stdout, 2)
