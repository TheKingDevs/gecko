...
// This program ensures system tasks (GC workers, finalizer task)
// started from within a synctest bubble do not participate in that bubble.
//
// To ensure none of these tasks start before synctest.Run,
// it must have no dependencies on packages which may start system tasks.
...
func main() {
	...
	synctest.Run(func() {
		// Start the finalizer and cleanup tasks.
		{
			p := new(T)
		...
		for range iterations {
			...
			// If we've improperly put a GC task into the synctest group,
