...
func NumGoroutine() {
	// Test that there are just the expected number of tasks
	// running. Specifically, test that the spare M's task
	// doesn't show up.
	if _, ok := checkNumGoroutine("first", 1+baseGoroutines); !ok {
		return
	}

	// Test that the task for a callback from C appears.
	if C.CheckNumGoroutine(); !callbackok {
		return
	}

	// Make sure we're back to the initial tasks.
