...
// waitNotInGo waits for /sched/tasks/not-in-go to read want, and reports
...
// through taking a P away from a task in a cgo call, that task is
...
// NotInGoMetric just double-checks that N tasks in cgo count as the metric reading N.
func NotInGoMetricCgoCall() {
	const N = 10

	// Spin up the same number of tasks that will all wait in a cgo call.
	for range N {
		go func() {
			C.SpinForever()
		}()
	...
	//
	// This will force a re-accounting of some of the tasks and
