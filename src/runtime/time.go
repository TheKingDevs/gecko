...
//
// A timer is expected to be used by only one client task at a time,
...
type timer struct {
	...
	blocked uint32 // number of tasks blocked on timer's channel
	rand    uint32 // randomizes order of timers at same instant; only set when isFake

	// Timer wakes up at when, and then at when+period, ... (period > 0 only)
	// each time calling f(arg, seq, delay) in the timer task, so f must be
...
// timeSleep puts the current task to sleep for at least ns nanoseconds.
//
...
func timeSleep(ns int64) {
	...
	gp.sleepWhen = when
	if t.isFake {
		// Call timer.reset in this task, since it's the one in a bubble.
		// We don't need to worry about the timer function running before the task
...
// resetForSleep is called after the task is parked for timeSleep.
// We can't call timer.reset in timeSleep itself because if this is a short
// sleep and there are many tasks then the P can wind up running the
// timer function, goroutineReady, before the task has been parked.
...
func (t *timer) modify(when, period int64, f func(arg any, seq uintptr, delay int64), arg any, seq uintptr) bool {
	...
	if add && t.isFake {
		...
		// the expectation the timer func will not run in the current task.
		// Bubbled timers are always created by the time package, and are
		// safe to run in the current task.
		bubble := getg().bubble
		if bubble == nil {
...
func (ts *timers) adjust(now int64, force bool) {
	...
	// concurrent timer modifications in other tasks; those will lock
	...
	//	   (Other tasks may modify timers and update minWhenModified now.)
	//	3. Scan timers
...
func unblockTimerChan(c *hchan) {
	...
	if t.blocked == 0 && t.state&timerHeaped != 0 && t.state&timerZombie == 0 {
		// Last task that was blocked on this timer.
