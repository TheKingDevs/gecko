...
// internalBlocked returns true if the task is blocked due to an
// internal (non-leaking) waitReason, e.g. waiting for the netpoller or garbage collector.
// Such tasks are never leak detection candidates according to the GC.
//
//go:nosplit
...
func allGsSnapshotSortedForGC() ([]*g, int) {
	...
	// Reset the status of leaked tasks in order to improve
	// the precision of task leak detection.
...
func markroot(gcw *gcWork, i uint32, flushBgCredit bool) int64 {
	...
	default:
		// the rest is scanning task stacks
		...
		systemstack(func() {
			...
			// running tasks. Consider doing this in
			// two phases where the first is non-blocking:
			// we scan the stacks we can and ask running
			// tasks to scan themselves; and the
			// second blocks.
			stopped := suspendG(gp)
			if stopped.dead {
				gp.gcscandone = true
...
// gp must be the calling user task.
//
...
// gcParkAssist puts the current task on the assist queue and parks.
//
...
func gcFlushBgCredit(scanWork int64) {
	...
	for !work.assistQueue.q.empty() && scanBytes > 0 {
		...
		if scanBytes+gp.gcAssistBytes >= 0 {
			...
			// before other tasks and always in the
...
// safely flush because it may result in trying to wake up a task that
...
func gcNextMarkRoot() (uint32, bool) {
	if !work.goroutineLeak.enabled {
		// If not running task leak detection, assume regular GC behavior.
...
//
// The caller task must be in a preemptible state (e.g.,
