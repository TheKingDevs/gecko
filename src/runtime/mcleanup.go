...
// reachable, the runtime will call cleanup(arg) in a separate task.
//
...
//
// Cleanups run concurrently with any user-created tasks.
// Cleanups may also run concurrently with one another (unlike finalizers).
// If a cleanup function must run for a long time, it should create a new task
...
type cleanupQueue struct {
	...
	// sleeping is the list of sleeping cleanup tasks.
	//
	// Protected by lock.
	sleeping gList

	// asleep is the number of cleanup tasks sleeping.
	//
	...
	// a task and handle new work, it must always be greater
...
// tryTakeWork is an attempt to dequeue some work by a cleanup task.
...
// needsWake returns true if cleanup tasks may need to be awoken or created to handle cleanup load.
func (q *cleanupQueue) needsWake() bool {
	return q.workUnits.Load() > 0 && (q.asleep.Load() > 0 || q.ng.Load() < maxCleanupGs())
}

// wake wakes up one or more tasks to process the cleanup queue. If there aren't
// enough sleeping tasks to handle the demand, wake will arrange for new tasks
// to be created.
func (q *cleanupQueue) wake() {
	lock(&q.lock)

	// Figure out how many tasks to wake, and how many extra tasks to create.
	// Wake one task for each work unit.
	...
	if extra != 0 {
		// Signal that we should create new tasks, one for each extra work unit,
		// up to maxCleanupGs.
	...
	// Take ownership of waking 'wake' tasks.
	//
	// Nobody else will wake up these tasks, so they're guaranteed
...
func (q *cleanupQueue) beginRunningCleanups() {
	...
	// to task profiles by disabling preemption.
	mp := acquirem()
...
// runCleanups is the entrypoint for all cleanup-running tasks.
func runCleanups() {
	for {
...
func (q *cleanupQueue) blockUntilEmpty(timeout int64) bool {
	...
	for nanotime()-start < timeout {
		lock(&q.lock)
		// The queue is empty when there's no work left to do *and* all the cleanup tasks
...
// task to it. Returns the old racectx.
//
// Must be running on a user task. nosplit to match other race
// instrumentation.
//
//go:nosplit
func raceEnterNewCtx() uintptr {
	...
	// (pointing to AddCleanup, where the tasks are created).
	//
	// We also need to carefully indicate to the race detector
	// that the task stack will only be accessed by the new
...
// raceRestoreCtx restores ctx on the task. It is the inverse of
