...
// debugCallWrap starts a new task to run a debug call and blocks
// the calling task. On the task, it prepares to recover
...
func debugCallWrap(dispatch uintptr) {
	...
	// Create a new task to execute the call on. Run this on
	// the system stack to avoid growing our stack.
	systemstack(func() {
		// TODO(mknyszek): It would be nice to wrap these arguments in an allocated
		// closure and start the task with that closure, but the compiler disallows
		...
		// Transfer locked-ness to the new task.
		...
		// Mark the calling task as being at an async
	...
	mcall(func(gp *g) {
		...
		// Directly execute the new task. The debug
		// protocol will continue on the new task, so
		// it's important we not just let the scheduler do
		// this or it may resume a different task.
		execute(newg, true)
	})
...
func debugCallWrap1() {
	...
	// Resume the caller task.
	getg().schedlink.set(callingG)
	mcall(func(gp *g) {
		callingG := gp.schedlink.ptr()
		gp.schedlink = 0

		// Unlock this task from the M if necessary. The
		...
		// Switch back to the calling task. At some point
