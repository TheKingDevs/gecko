...
// checkTimeouts resumes tasks that are waiting on a note which has reached its deadline.
func checkTimeouts() {
	now := nanotime()
...
type event struct {
	// g was the active task when the call from JavaScript occurred.
	// It needs to be active when returning to JavaScript.
	gp *g
	// returned reports whether the event handler has returned.
	// When all tasks are idle and the event handler has returned,
...
// beforeIdle gets called by the scheduler if no task is awake.
// If we are not already handling an event, then we pause for an async event.
// If an event handler returned, we resume it and it will pause the execution.
// beforeIdle either returns the specific task to schedule next or
// indicates with otherReady that some task became ready.
...
// and then parks the handler task to allow other tasks to run before giving execution back to JavaScript.
// When no other task is awake any more, beforeIdle resumes the handler task. Now that the same task
