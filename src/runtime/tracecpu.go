...
// traceStartReadCPU creates a task to start reading CPU profile
...
func traceStartReadCPU() {
	...
	go func() {
		for traceEnabled() {
			...
			// we would still want to do a task-level sleep in between
...
// traceStopReadCPU blocks until the trace CPU reading task exits.
//
...
func traceStopReadCPU() {
	...
	// - The logger task has already exited because it observed
	//   that the trace is disabled.
	// - The logger task is asleep.
	//
	// Wake the task so it can observe that their the buffer is
	// closed an exit.
	...
	// Wait until the logger task exits.
	<-trace.cpuLogDone
...
//
// No more than one task may be in traceReadCPU for the same
// profBuf at a time.
//
...
func traceReadCPU(gen uintptr) bool {
	...
	for len(data) > 0 {
		...
		data = data[recordLen:]
		// No support here for reporting task tags at the moment; if
...
func traceCPUSample(gp *g, mp *m, pp *p, stk []uintptr) {
	...
	// followed by the IDs of the P and task. (For normal CPU profiling, it's
