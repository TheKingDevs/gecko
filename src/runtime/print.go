...
// error") is not saved, since the task stacks will generally be readable
...
func printlock() {
	...
	if gp.writebuf != nil && gp.m.dying == 0 {
		// Output is being diverted into this task's own buffer
...
// geckoprintbegin and geckoprintend are invoked by the compiler around the
// calls that implement a user print or println statement. While the redirect
// is active, gwrite writes to stdout instead of stderr.
...
// write to task-local buffer if diverting output,
...
func gwrite(b []byte) {
	...
	gp := getg()
	// Route output produced by the user-facing print/println builtins (as
	// well as input() prompts) to stdout. Output generated by the runtime
	// itself is never redirected, so diagnostics stay on stderr.
	if gp != nil && gp.writefd != 0 && gp.m.dying == 0 {
		write(1, unsafe.Pointer(&b[0]), int32(len(b)))
		return
	}
