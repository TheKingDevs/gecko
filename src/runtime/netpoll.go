...
//     Return a list of tasks built by calling netpollready,
//     and a delta to add to netpollWaiters when all tasks are ready.
...
// tasks respectively. The semaphore can be in the following states:
//
//	pdReady - io readiness notification is pending;
//	          a task consumes the notification by changing the state to pdNil.
//	pdWait - a task prepares to park on the semaphore, but not yet parked;
//	         the task commits to park by changing the state to G pointer,
//	         or, alternatively, concurrent io notification changes the state to pdReady,
//	         or, alternatively, concurrent timeout/close changes the state to pdNil.
//	G pointer - the task is blocked on the semaphore;
//	            io notification or timeout/close changes the state to pdReady or pdNil respectively
//	            and unparks the task.
...
// The toRun argument is used to build a list of tasks to return
...
func netpollblockcommit(gp *g, gpp unsafe.Pointer) bool {
	...
	if r {
		// Bump the count of tasks waiting for the poller.
...
// can hold only a single waiting task for each mode.
...
// This returns any task blocked on pd.{rg,wg}.
// It adds any adjustment to netpollWaiters to *delta;
// this adjustment should be applied after the task has
// been marked ready.
...
// netpollAnyWaiters reports whether any tasks are waiting for I/O.
