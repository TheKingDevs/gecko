...
// traceStack captures a stack trace from a task and registers it in the trace
...
func traceStack(skip int, gp *g, tab *traceStackTable) uint64 {
	...
	if debug.traceCheckStackOwnership != 0 && gp != nil {
		...
		if status&_Gscan == 0 {
			...
			// where a running task might be in _Gwaiting, and these cases
...
// startPCForTrace returns the start PC of a task for tracing purposes.
