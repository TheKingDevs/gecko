...
// a task blocked on it. If another task calls coroswitch(c),
// the caller becomes the task blocked in c, and the task
...
// task in c and exiting the current task.
//
...
func newcoro(f func(*coro)) *coro {
	...
	systemstack(func() {
		...
		// thread-lock state to the new task.
...
//go:linknamestd coroswitch

// coroswitch switches to the task blocked on c
// and then blocks the current task on c.
...
// than regular task switches, so this path is heavily
...
func coroswitch_m(gp *g) {
	...
	//
	// The rules with thread-lock interactions are simple. When a coro task is switched to,
	// the same thread must be used, and the locked state must match with the thread-lock state of
	// the task which called newcoro. Thread-lock state consists of the thread and the number
	...
	if locked {
		// Detach the task from the thread; we'll attach to the task we're
	...
	if exit {
		...
		// the thread, because we'll be switching back to another task anyway, which
	...
	// The task stored in c is the one to run next.
	...
	// Synchronize with any out-standing task profile. We're about to start
	// executing, and an invariant of the profiler is that we tryRecordGoroutineProfile
	// whenever a task is about to start running.
	//
