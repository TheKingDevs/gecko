...
// Trace task and P status management.
...
// writeGoStatus emits a GoStatus event as well as any active ranges on the task.
//
...
func (w traceWriter) writeProcStatusForP(pp *p, inSTW bool) traceWriter {
	...
	case _Prunning:
		...
		// own P, then the task isn't going to transition and we can trivially
		// check if the task is in a syscall. This used to be just a small problematic
...
func goStatusToTraceGoStatus(status uint32, wr waitReason) tracev2.GoStatus {
	...
	case _Gwaiting, _Gpreempted, _Gleaked:
		...
		// want these tasks to appear in the final trace as
