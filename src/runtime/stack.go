...
The per-task g->stackguard is set to point StackGuard bytes
...
func copystack(gp *g, newsize uintptr) {
	...
	if !gp.activeStackChans {
		...
		// released channel locks, so other tasks could
...
func newstack() {
	...
	if goexperiment.RuntimeSecret && gp.secret > 0 {
		...
		// might context switch to a different task which
	...
	// this task has to wait for the GC to finish before continuing.
	// If the GC is in some way dependent on this task (for example,
	// it needs a lock held by the task), that small preemption turns
	// into a real deadlock.
	preempt := stackguard0 == stackPreempt
	if preempt {
		if !canPreemptM(thisg.m) {
			// Let the task keep running for now.
	...
	if preempt {
		...
		// Act like task called runtime.Gosched.
	...
	// The task must be executing in order to call newstack,
...
func isShrinkStackSafe(gp *g) bool {
	...
	// task calling gopark to park on a channel and
	...
	// G with the _Gscan bit. This prevents the task from transitioning,
...
// startingStackSize is the amount of stack that new tasks start with.
...
func gcComputeStartingStackSize() {
	...
	// and start tasks with stack equal to that average size.
	...
	// early in a task's lifetime. See issue 18138. Stacks that
	...
	// Note: we add stackGuard to ensure that a task that
