...
func testSemaHandoff() bool {
	...
	// We're testing that the current task is able to yield its time slice
	// to another task. Stop the current task from migrating to
	...
	for SemNwait(&sema) == 0 {
		Gosched() // wait for task to block in Semacquire
	}
	...
	wg.Wait() // wait for tasks to finish to avoid data races
