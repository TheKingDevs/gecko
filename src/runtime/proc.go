...
// Task scheduler
// The scheduler's job is to distribute ready-to-run tasks over worker threads.
//
...
// a worker thread when a new task will be readied in near future).
//
// Three rejected approaches that would work badly:
// 1. Centralize all scheduler state (would inhibit scalability).
// 2. Direct task handoff. That is, when we ready a new task and there
//    is a spare P, unpark a thread and handoff it the thread and the task.
//    This would lead to thread state thrashing, as the thread that readied the
//    task can be out of work the very next moment, we will need to park it.
//    Also, it would destroy locality of computation as we want to preserve
//    dependent tasks on the same thread; and introduce additional latency.
// 3. Unpark an additional thread whenever we ready a task and there is an
...
// task, new/modified-earlier timers, and idle-priority GC. See below for
// additional details.
//
...
// also considered spinning; we don't do task handoff so such threads are
...
// * Ready task: this is an obvious source of work; the task is
...
func main() {
	...
	// Racectx of m0->g0 is used only as the parent of the main task.
	...
	// Lock the main task onto this, the main OS thread,
	...
	// another task at the same time as main returns,
	// let the other task finish printing the panic trace.
...
func forcegchelper() {
	...
	for {
		...
		// this task is explicitly resumed by sysmon
		if debug.gctrace > 0 {
			println("GC forced")
		}
...
// Gosched yields the processor, allowing other tasks to run. It does not
// suspend the current task, so execution resumes automatically.
//
//go:nosplit
func Gosched() {
	checkTimeouts()
...
// Puts the current task into a waiting state and calls unlockf on the
// system stack.
//
...
// externally readied.
//
// Reason explains why the task has been parked. It is displayed in stack
...
func gopark(unlockf func(*g, unsafe.Pointer) bool, lock unsafe.Pointer, reason waitReason, traceReason traceBlockReason, traceskip int) {
	if reason != waitReasonSleep {
		checkTimeouts() // timeouts may expire while two tasks keep the scheduler busy
	}
	mp := acquirem()
	gp := mp.curg
...
// Puts the current task into a waiting state and unlocks the lock.
// The task can be made runnable again by calling goready(gp).
...
const (
	// Number of task ids to grab from sched.goidgen to local per-P cache at once.
...
func checkmcount() {
	...
	// bomb from something like millions of tasks blocking on system
...
func freezetheworld() {
	...
	if debug.dontfreezetheworld > 0 {
		// Don't prempt Ps to stop tasks. That will perturb
		// scheduler state, making debugging more difficult. Instead,
		// allow tasks to continue execution.
		//
		// fatalpanic will tracebackothers to trace all tasks. It
		// is unsafe to trace a running task, so tracebackothers
		// will skip running tasks. That is OK and expected, we
		// expect users of dontfreezetheworld to use core files anyway.
		//
		// However, allowing the scheduler to continue running free
		// introduces a race: a task may be stopped when
	...
	for i := 0; i < 5; i++ {
		// this should tell the scheduler to not start any new tasks
		sched.stopwait = freezeStopWait
		sched.gcwaiting.Store(true)
		// this should stop running tasks
		if !preemptall() {
			break // no running tasks
		}
		usleep(1000)
	}
	// to be sure
...
func casgstatus(gp *g, oldval, newval uint32) {
	...
	if (oldval == _Grunning || oldval == _Gsyscall) && (newval != _Grunning && newval != _Gsyscall) {
		// Track every gTrackingPeriod time a task transitions out of _Grunning or _Gsyscall.
...
//
// While a task is in this state, it's stack is effectively pinned.
// The garbage collector must not shrink or otherwise mutate the task's stack.
//
// Use this over casgstatus when possible to ensure that a waitreason is set.
...
func casGToPreemptScan(gp *g, old, new uint32) {
	...
	// We never notify gp.bubble that the task state has moved
...
const (
	...
	stwGoroutineProfile                             // "task profile"
	stwGoroutineProfileCleanup                      // "task profile cleanup"
	stwAllGoroutinesStack                           // "all tasks stack trace"
...
// stopTheWorld stops all P's from executing tasks, interrupting
// all tasks at GC safe points and records reason as the reason
// for the stop. On return, only the current task's P is running.
// stopTheWorld must not be called from a system stack and the caller
// must not hold worldsema. The caller must call startTheWorld when
// other P's should resume execution.
//
// stopTheWorld is safe for multiple tasks to call at the
...
// Holding worldsema causes any other tasks invoking
...
func stopTheWorldWithSema(reason stwReason) worldStop {
	// Mark the task which called stopTheWorld preemptible so its
	...
	//
	// casGToWaitingForSuspendG marks the task as ineligible for a
...
func startTheWorldWithSema(now int64, w worldStop) int64 {
	...
	// Wakeup an additional proc in case we have excessive runnable tasks
...
// part of the current task's stack, since the GC may move it.
func forEachP(reason waitReason, fn func(*p)) {
	systemstack(func() {
		...
		// deadlock as we attempt to preempt a task that's trying
...
// or it must leave its task in a preemptible state before it switches
...
// task for the duration of the cgo callback.
//
...
func needm(signal bool) {
	...
	// We're going into Go anyway, so it's okay to pretend we're a real task now.
	addGSyscallNoP(mp)
...
func oneNewExtraM() {
	// Create extra task locked to extra m.
	// The task is the context in which the cgo callback will run.
...
func dropm() {
	...
	//
	// In the new tracer, we model needm and dropm and a task being created and
	...
	// Reset trace state unconditionally. This task is being 'destroyed'
...
func execute(gp *g, inheritTime bool) {
	...
	if goroutineProfile.active {
		// Make sure that gp has had its stack written out to the task
		// profile, exactly as it was when the task profiler first stopped
		// the world.
	...
	if sys.DITSupported && debug.dataindependenttiming != 1 {
		if gp.ditWanted && !mp.ditEnabled {
			// The current M doesn't have DIT enabled, but the task we're
			// executing does need it, so turn it on.
			sys.EnableDIT()
			mp.ditEnabled = true
		} else if !gp.ditWanted && mp.ditEnabled {
			// The current M has DIT enabled, but the task we're executing does
			// not need it, so turn it off.
			// NOTE: turning off DIT here means that the scheduler will have DIT enabled
			// when it runs after this task yields or is preempted. This may have
...
// Finds a runnable task to execute.
// Tries to steal from other P's, get g from local or global queue, poll network.
// tryWakeP indicates that the returned task is not normal (GC worker, trace
...
top:
	...
	// Otherwise two tasks can completely occupy the local runqueue
	...
	// wasm only:
	// If a callback returned and no other task is awake,
	// then wake event handler task which pauses execution
	...
	//
	// * Tasks added to the global or a per-P run queue.
	// * New/modified-earlier timers on a per-P timer heap.
	// * Idle-priority GC work (barring golang.org/issue/19112).
	//
	// If we discover new work below, we need to restore m.spinning as a
	// signal for resetspinning to unpark a new worker thread (because
	// there can be more than one starving task).
	//
...
// stealWork attempts to steal a runnable task or timer from any P.
//
...
func stealWork(now int64) (gp *g, inheritTime bool, rnow, pollUntil int64, newWork bool) {
	...
	// No tasks found to steal. Regardless, running a timer may have
	// made some task ready that we missed. Indicate the next timer to
	// wait for.
...
func injectglist(glist *gList) {
	...
	// Mark all the tasks as runnable before we put them
	// on the run queues.
	var tail *g
	trace := traceAcquire()
	...
	*glist = gList{}
	...
	// but before any tasks were added to the queue, which could
	// lead to idle P's when there is work available in the global queue.
	// That could potentially last until other tasks become ready
...
top:
	...
	// goready to put a ready task on the local run queue.
	...
	//
	// N.B. If this occurs because a higher-priority task was selected
	...
	if debug.dontfreezetheworld > 0 && freezing.Load() {
		...
		// also don't want to allow new tasks to run.
		//
	...
	if sched.disable.user && !schedEnabled(gp) {
		// Scheduling of this task is disabled. Put it on
		// the list of pending runnable tasks for when we
	...
	// If about to schedule a not-normal task (a GCworker or tracereader),
...
// dropg removes the association between m and the current task m->curg (gp for short).
// Typically a caller sets gp's status away from Grunning and then
// immediately calls dropg to finish the job. The caller is also responsible
// for arranging that gp will be restarted using ready at an
// appropriate time. After calling dropg and arranging for gp to be
// readied later, the caller can do other work but eventually should
// call schedule to restart the scheduling of tasks on this m.
func dropg() {
	gp := getg()
...
func preemptPark(gp *g) {
	...
	// for us to emit an event for a task we do not own.
	// The moment we CAS into _Gpreempted, suspendG could CAS the
	// task to _Gwaiting, effectively taking ownership. All of
	...
	// the _Gscan bit on the task, since the _Gscan bit means
	...
	// Drop the task from the M. Only do this after the tracer has
...
func gdestroy(gp *g) {
	...
	if gcBlackenEnabled != 0 && gp.gcAssistBytes > 0 {
		...
		// rapidly creating an exiting tasks.
	...
	gfput(pp, gp)
	if locked {
		// The task may have locked this thread because
...
// The task g is about to enter a system call.
...
// The task g exited its system call.
...
func exitsyscall() {
	...
	if pp != nil {
		if goroutineProfile.active {
			// Make sure that gp has had its stack written out to the task
			// profile, exactly as it was when the task profiler first
			// stopped the world.
			systemstack(func() {
		...
		if sched.disable.user && !schedEnabled(gp) {
			// Scheduling of this task is disabled.
			Gosched()
		}
		return
	}
...
// addGSyscallNoP must be called when a task in a syscall loses its P.
...
func addGSyscallNoP(mp *m) {
	...
	// to a task in _Gsyscall and blocked from exiting.
	if !mp.isExtraInC {
		// Increment nGsyscallNoP since we're taking away a P
		// from a _Gsyscall task, but only if isExtraInC
		// is not set on the M. If it is, then this thread is
		// back to being a full C thread, and will just inflate
		// the count of not-in-go tasks. See go.dev/issue/76435.
		sched.nGsyscallNoP.Add(1)
	}
}

// decGSsyscallNoP must be called whenever a task in a syscall without
...
func newproc1(fn *funcval, callergp *g, callerpc uintptr, parked bool, waitreason waitReason) *g {
	...
	if isSystemGoroutine(newg, false) {
		...
		// Only user tasks inherit synctest groups and pprof labels.
		newg.bubble = callergp.bubble
		if mp.curg != nil {
			newg.labels = mp.curg.labels
		}
		if goroutineProfile.active {
			// A concurrent task profile is running. It should include
			// exactly the set of tasks that were alive when the task
	...
	// since it's a task transition. See tracer invariants in trace.go.
	trace := traceAcquire()
	...
	if pp.goidcache == pp.goidcacheend {
		...
		// At startup sched.goidgen=0, so main task receives goid=1.
	...
	if goexperiment.RuntimeSecret && callergp.secret > 0 {
		...
		// Whether a task is running in secret mode should be more visible,
...
func saveAncestors(callergp *g) *[]ancestorInfo {
	// Copy all prior info, except for the root task (goid 0).
...
retry:
	...
	if gp.stack.lo != 0 && gp.stack.hi-gp.stack.lo != uintptr(startingStackSize) {
		...
		// right size when the task was put on the free list, but
...
// LockOSThread wires the calling task to its current operating system thread.
// The calling task will always execute in that thread,
// and no other task will execute in it,
// until the calling task has made as many calls to
// [UnlockOSThread] as to LockOSThread.
// If the calling task exits without unlocking the thread,
...
// that thread.
//
// A task should call LockOSThread before calling OS services or
...
// calling task to zero, it unwires the calling task from
// its fixed operating system thread.
// If there are no active LockOSThread calls, this is a no-op.
//
// Before calling UnlockOSThread, the caller must ensure that the OS
// thread is suitable for running other tasks. If the caller made
// any permanent changes to the state of the thread that would affect
// other tasks, it should not call this function and thus leave
// the task locked to the OS thread until the task (and
...
func gcount(includeSys bool) int32 {
	...
	// But at least the current task is running.
	if n < 1 {
		n = 1
	}
	return n
...
// goroutineleakcount returns the number of leaked tasks last reported by
// the runtime.
//
...
func procresize(nprocs int32) *p {
	...
	// Assign Ms to Ps with runnable tasks.
	var runnablePs *p
	...
	// Now that we've assigned Ms to Ps with runnable tasks, assign GC
	...
	// tasks and GC mark workers all at once, rather than using a
	// sequence of wakep calls as each P's findRunnable realizes it needs
	// to run a mark worker instead of a user task.
	//
	// By assigning GC workers to Ps only _after_ previously-running Ps are
	// assigned Ms, we ensure that tasks previously running on a P
	// continue to run on the same P, with GC mark workers preferring
	// previously-idle Ps. This helps prevent tasks from shuffling
...
func checkdead() {
	...
	// there are no running tasks. The calling program is
	// assumed to be running.
	// One exception is Wasm, which is single-threaded. If we are
	// in Go and all tasks are blocked, it deadlocks.
	...
	if grunning == 0 { // possible if main task calls runtime·Goexit()
	...
	// There are no tasks running, so we can look at the P's.
	for _, pp := range allp {
...
func sysmon() {
	...
	for {
		...
		if netpollinited() && lastpoll != 0 && lastpoll+10*1000*1000 < now {
			...
			list, delta := netpoll(0) // non-blocking - returns list of tasks
			if !list.empty() {
...
func retake(now int64) uint32 {
	...
	for i := 0; i < len(allp); i++ {
		...
		// task or a sequence of tasks run via
		...
		if int64(pd.schedtick) != schedt {
			...
			// The task nor the thread can respond to a
		...
		// Take the P. Note: because we have the scan bit, the task
...
// the task is in _Gsyscall, and prevented from transitioning out
...
func setBlockOnExitSyscall(pp *p) (syscallingThread, bool) {
	...
	// A task is considered in a syscall, and may have a corresponding
...
// Tell all tasks that they have been preempted and they should stop.
// This function is purely best-effort. It can fail to inform a task if a
// processor just started running it.
// No locks need to be held.
// Returns true if preemption request was issued to at least one task.
...
// Tell the task running on processor P to stop.
// This function is purely best-effort. It can incorrectly fail to inform the
// task. It can inform the wrong task. Even if it informs the
// correct task, that task might ignore the request if it is
...
func preemptone(pp *p) bool {
	...
	if readgstatus(gp)&^_Gscan == _Gsyscall {
		// Don't bother trying to preempt a task in a syscall.
		return false
	}

	gp.preempt = true

	// Every call in a task checks for stack overflow by
...
var (
	...
	// However, we would like to avoid scheduling this task every
...
// Start GOMAXPROCS update helper task.
//
...
// tasks.
//
// This does not stop already running user tasks, so the caller
// should first stop the world when disabling user tasks.
...
// Put a batch of runnable tasks on the global runnable queue.
// This clears *batch.
// sched.lock must be held.
// May run during STW, so write barriers are not allowed.
//
...
func runqput(pp *p, gp *g, next bool) {
	if !haveSysmon && next {
		// A runnext task shares the same time slice as the
		// current task (inheritTime from runqget). To prevent a
		// ping-pong pair of tasks from starving all others, we
		// depend on sysmon to preempt "long-running tasks". That
		// is, any set of tasks sharing the same time slice.
		//
...
// runqdrain drains the local runnable queue of pp and returns all tasks in it.
// Executed only by the owner P.
...
// Grabs a batch of tasks from pp's runnable queue into batch.
					...
					if pp.status == _Prunning {
						if mp := pp.m.ptr(); mp != nil {
							if gp := mp.curg; gp == nil || readgstatus(gp)&^_Gscan != _Gsyscall {
								...
								// task hanging off of a running P, suggesting it
...
func doInit1(t *initTask) {
	...
	default: // not initialized yet
		...
		if inittrace.active {
			start = nanotime()
			// Load stats non-atomically since tracinit is updated only by this init task.
			before = inittrace
		}
