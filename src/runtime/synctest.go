...
// A synctestBubble is a set of tasks started by synctest.Run.
type synctestBubble struct {
	...
	main    *g     // task started by synctest.Run
	waiting bool   // true if a task is calling synctest.Wait
	done    bool   // true if main has exited

	// The bubble is active (not blocked) so long as running > 0 || active > 0.
	//
	// running is the number of tasks which are not "durably blocked":
	// Tasks which are either running, runnable, or non-durably blocked
	// (for example, blocked in a syscall).
	//
	// active is used to keep the bubble from becoming blocked,
	// even if all tasks in the bubble are blocked.
	// For example, park_m can choose to immediately unpark a task after parking it.
...
func (bubble *synctestBubble) changegstatus(gp *g, oldval, newval uint32) {
	...
	// If this isn't a task starting, stopping, durably blocking,
	...
	// for example, if a new task is created in a non-running state.
	...
	if wasRunning != isRunning {
		if isRunning {
			...
			if raceenabled && newval != _Gdead && newval != _Gdeadextra {
				// Record that this task parking happens before
				// any subsequent Wait.
...
func (bubble *synctestBubble) maybeWakeLocked() *g {
	...
	// The woken task will decrement the count.
	...
	// and a task that we considered durably blocked wakes up unexpectedly.
	...
	if next > 0 && next <= bubble.now {
		// A timer is scheduled to fire. Wake the root task to handle it.
		return bubble.root
	}
	if gp := bubble.waiter; gp != nil {
		// A task is blocked in Wait. Wake it.
		return gp
	}
	// All tasks in the bubble are durably blocked, and nothing has called Wait.
	// Wake the root task.
	return bubble.root
}
...
func (bubble *synctestBubble) raceaddr() unsafe.Pointer {
	...
	// the blocking operations which caused other tasks in the bubble to park.
...
func synctestRun(f func()) {
	...
	for {
		...
		systemstack(func() {
			...
			// so timer tasks inherit their child race context from g0.
			curg := gp.m.curg
			gp.m.curg = nil
		...
		if bubble.done {
			// Time stops once the bubble's main task has exited.
			break
		}
		bubble.now = next
	}
	...
	unlock(&bubble.mu)
	if raceenabled {
		// Establish a happens-before relationship between bubbled tasks exiting
		// and Run returning.
	...
	if gp.timer != nil && gp.timer.isFake {
		// Verify that we haven't marked this task's sleep timer as fake.
...
func synctestidle_c(gp *g, _ unsafe.Pointer) bool {
	...
	if gp.bubble.running == 0 && gp.bubble.active == 1 {
		// All tasks in the bubble have blocked or exited.
		canIdle = false
	} else {
		gp.bubble.active--
	}
