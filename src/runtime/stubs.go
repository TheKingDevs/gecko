...
// where g is the task that made the call.
// mcall saves g's current PC/SP in g->sched so that it can be restored later.
// It is up to fn to arrange for that later execution, typically by recording
// g in a data structure, causing something to call ready(g) later.
// mcall returns to the original task g later, when g has been rescheduled.
...
// of an ordinary task. In this case, systemstack switches
...
//
// reflectcall copies stack arguments from stackArgs to the task stack, and
...
// goexit is the return stub at the top of every task call stack.
// Each task stack is constructed as if goexit called the
// task's entry point function, so that when the entry point
