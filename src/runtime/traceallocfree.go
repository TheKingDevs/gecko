...
// (heap spans, heap objects, task stacks, etc.) and writes out events for them.
//
...
// GoroutineStackExists records that a task stack already exists at address base with the provided size.
...
// GoroutineStackAlloc records that a task stack was newly allocated at address base with the provided size..
...
// GoroutineStackFree records that a task stack at address base is about to be freed.
...
// traceGoroutineStackID creates a trace ID for the task stack from its base address.
