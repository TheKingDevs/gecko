...
func TestGoroutineLeakProfile(t *testing.T) {
	...
	type testCase struct {
		...
		// flakyLeaks are task leaks that are too flaky to be reliably detected.
		...
		//
		// Note that all flaky leaks are true positives, i.e. real task leaks,
	...
	extractLeaks := func(output string) []string {
		...
		for _, stack := range stacks {
			...
			if !strings.Contains(lines[0], "(leaked)") {
				// Ignore non-leaked tasks.
				continue
			}

			// Get the wait reason from the task header.
			header := lines[0]
	...
	microTests := []testCase{
		...
		// If the main task is waiting on select{}, it should not be reported as a leak.
		makeTest("SelectNoCasesMain",
			// However, any task leaking on a channel that is reachable from the main
	...
	// They are only intended to stress the task leak detector and profiling
	...
	// Common task leak patterns.
	// Extracted from "Unveiling and Vanquishing Task Leaks in Enterprise Microservices: A Dynamic Analysis Approach"
	...
	gokerTestCases := []testCase{
		...
		makeFlakyTest("Grpc3017",
			// grpc/3017 involves a task leak that also simultaneously engages many GC assists.
