...
func initsig(preinit bool) {
	...
	for i := uint32(0); i < _NSIG; i++ {
		...
		// there shouldn't be any other tasks running yet.
		fwdSig[i] = getsig(i)
...
// marked for preemption and the task is at an asynchronous
// safe-point, it will preempt the task. It always atomically
...
func sigtrampgo(sig uint32, info *siginfo, ctx unsafe.Pointer) {
	...
	if gp == nil || (gp.m != nil && gp.m.isExtraInC) {
		...
		// since the current g maybe a normal task and actually running on the signal stack,
...
// set to a gsignal task and we will be running on the alternate
...
func ensureSigM() {
	...
	go func() {
		// Signal masks are per-thread, so make sure this task stays on one
		// thread.
		LockOSThread()
...
func sigfwdgo(sig uint32, info *siginfo, ctx unsafe.Pointer) bool {
	...
	//   (2) we were in a task (i.e., m.curg != nil), and
	//   (3) we weren't in CGO.
