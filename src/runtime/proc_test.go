...
func TestGoroutineParallelism(t *testing.T) {
	...
	// since the tasks can't be stopped/preempted.
	...
	for try := 0; try < N; try++ {
		...
		x := uint32(0)
		for p := 0; p < P; p++ {
			// Test that all P tasks are scheduled at the same time
			go func(p int) {
				for i := 0; i < 3; i++ {
...
// Test that all runnable tasks are scheduled at the same time.
...
func testGoroutineParallelism2(t *testing.T, load, netpoll bool) {
	...
	for try := 0; try < N; try++ {
		if load {
			// Create P tasks and wait until they all run.
			// When we run the actual test below, worker threads
			// running the tasks will start parking.
			done := make(chan bool)
			x := uint32(0)
			for p := 0; p < P; p++ {
				go func() {
		...
		x := uint32(0)
		// Spawn P tasks in a nested fashion just to differ from TestGoroutineParallelism.
		for p := 0; p < P/2; p++ {
			go func(p int) {
				for p2 := 0; p2 < 2; p2++ {
					go func(p2 int) {
...
func TestPreemption(t *testing.T) {
	...
	// Test that tasks are preempted at function calls.
	N := 5
	if testing.Short() {
		N = 2
	}
...
func TestPreemptionGC(t *testing.T) {
	...
	// Test that pending GC preempts running tasks.
	P := 5
	N := 10
	if testing.Short() {
		P = 3
...
func TestNumGoroutine(t *testing.T) {
	...
	for i := 0; ; i++ {
		// Give tasks about to exit a chance to exit.
		...
		// To avoid double-counting "task" in "task $m [running]:"
		// and "created by $func in task $n", remove the latter
...
func TestPingPongHog(t *testing.T) {
	...
	// Start two co-scheduled hog tasks.
	for i := 0; i < 2; i++ {
		go run(1e6, &hogCount, hogChan)
	}

	// Start two co-scheduled light tasks.
	for i := 0; i < 2; i++ {
		go run(1e3, &lightCount, lightChan)
	}

	// Start task pairs and wait for a few preemption rounds.
	hogChan <- true
	lightChan <- true
	...
	// 20, which indicates that both pairs of tasks handed off
...
func TestSchedLocalQueueEmpty(t *testing.T) {
	...
	// If runtime triggers a forced GC during this test then it will deadlock,
	// since the tasks can't be stopped/preempted during spin wait.
	defer debug.SetGCPercent(debug.SetGCPercent(-1))
	// SetGCPercent waits until the mark phase is over, but the runtime
	// also preempts at the start of the sweep phase, so make sure that's
	// done too. See #45867.
...
// BenchmarkCreateGoroutinesSingle creates many tasks, all from a single
// producer (the main benchmark task).
//
...
func benchmarkWakeupParallel(b *testing.B, spin func(time.Duration)) {
	...
	for _, delay := range []time.Duration{
		...
		b.Run(delay.String(), func(b *testing.B) {
			...
			// Start two tasks, which alternate between being
			...
			//
			// In each iteration of the benchmark, each task
			// acts once as sender and once as receiver, so each
			// task spins for delay twice.
			//
			// BenchmarkWakeupParallel is used to estimate how
			// efficiently the scheduler parallelizes tasks in
			// the presence of blocking:
			//
			// - If both tasks are executed on the same core,
			...
			// for the unblocked task to be scheduled, to be
			// estimated.
...
// call time.Sleep, which would deschedule the task.
...
// Check that a task will be preempted if it is calling short system calls.
...
type goroutineState struct {
	G trace.GoID     // This task.
	P trace.ProcID   // Most recent P this task ran on.
	M trace.ThreadID // Most recent M this task ran on.
}
...
// TestTraceSTW verifies that tasks continue running on the same M and P
// after a STW.
func TestTraceSTW(t *testing.T) {
	// Across STW, the runtime attempts to keep tasks running on the
	// same P and the P running on the same M. It does this by keeping
	// tasks in the P's local runq, and remembering which M the P ran
	...
	// spinning M may steal a task from another P if that P is too
	// slow to start.
...
// TestTraceGCSTW verifies that tasks continue running on the same M and P
// after a GC STW.
...
func runTestTracesSTW(t *testing.T, run int, name, stwType string) (err error) {
	...
	// tasks to race. This slows down execution for this test far too
	...
	// unwanted scheduling. i.e., the target tasks did not remain on
	// the same P/M.
	//
	...
	// 1. Find the task IDs for the target tasks. This will be in
	// the StateTransition from NotExist.
	//
	// 2. Once found, track which M and P the target tasks run on until...
	//
	// 3. Look for the first STW after the "TraceSTW" "start" log message,
	// where we commit the target tasks' "before" M and P.
	//
	// N.B. We must do (1) and (2) together because the first target
	// task may start running before the second is created.
...
findStart:
	for {
		...
		case trace.EventStateTransition:
			...
			// Potentially case (1): Task creation.
			if from == trace.GoNotExist {
				for sf := range st.Stack.Frames() {
					...
					// Always break, the task entrypoint is always the
					// first frame.
					break
				}
			}

			// Potentially case (2): Task running.
	...
	//
	// What we want to observe is that the target tasks run only on
	...
	// kernel scheduling), the spinning M may legally steal the task
	// and run it instead.
	//
	...
	// the "start" log and actual STW during which the target tasks
	...
	for {
		...
		case trace.EventStateTransition:
			...
			case trace.ResourceGoroutine:
				...
				// Task running. Ensure it didn't migrate.
				if to == trace.GoRunning {
					p := ev.Proc()
					m := ev.Thread()
