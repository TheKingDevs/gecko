...
//    task, shades any pointers found on its stack, and then
//    resumes the task.
//
...
// The heap is swept span-by-span both lazily (when a task needs another span)
// and concurrently in a background task (this helps programs that are not CPU bound).
// At the end of STW mark termination all spans are marked as "needs sweeping".
//
// The background sweeper task simply sweeps spans one-by-one.
//
// To avoid requesting more OS memory while there are unswept spans, when a
// task needs another span, it first attempts to reclaim that much memory
// by sweeping. When a task needs to allocate a new small-object span, it
// sweeps small-object spans for the same object size until it frees at least
// one object. When a task needs to allocate large-object span from heap,
// it sweeps spans until it frees at least that many pages into heap. There is
// one case where this may not suffice: if a task sweeps and frees two
...
// so they are empty. When a task grabs a new span into mcache, it sweeps it.
// When a task explicitly frees an object or sets a finalizer, it ensures that
// the span is swept (either by sweeping it, or by waiting for the concurrent sweep to finish).
// The finalizer task is kicked off only when all spans are swept.
...
// It kicks off the background sweeper task, the background
// scavenger task, and enables GC.
func gcenable() {
...
type workType struct {
	...
	// during task leak detection, nMaybeRunnableStackRoots is the number of stackRoots
	...
	goroutineLeak struct {
		// Once set, it indicates that the GC will perform task leak detection during
		// the next GC cycle; it is set by goroutineLeakGC and unset during gcStart.
		pending atomic.Bool
		// Once set, it indicates that the GC has started a task leak detection run;
		// it is set during gcStart and unset during gcMarkTermination;
		//
		// Protected by STW.
		enabled bool
		// Once set, it indicates that the GC has performed task leak detection during
		// the current GC cycle; it is set during gcMarkDone, right after task leak detection,
		// and unset during gcMarkTermination;
		//
		// Protected by STW.
		done bool
		// The number of leaked tasks during the last leak detection GC cycle.
		//
	...
	// beginning of concurrent marking.  During task leak detection, stackRoots
	// is partitioned into two sets; to the left of nMaybeRunnableStackRoots are stackRoots
	// of running / runnable tasks and to the right of nMaybeRunnableStackRoots are
	// stackRoots of unmarked / not runnable tasks
	...
	// sweepWaiters is a list of blocked tasks to wake when
	...
	strongFromWeak struct {
		...
		// task and enqueuing it onto q.
		//
		// Mutated only by one task at a time in gcMarkDone,
		// with globally-synchronizing events like forEachP and
		// stopTheWorld.
		block bool

		// q is a queue of tasks that attempted to perform a
...
// goroutineLeakGC runs a GC cycle that performs task leak detection.
//
...
func goroutineLeakGC() {
	...
	// actually performs task leak detection.
	//
...
func gcStart(trigger gcTrigger) {
	...
	// that multiple tasks that detect the heap trigger don't
	...
	// disable scheduling of this task, so it may block as
	// soon as we start the world again.
	if mode != gcBackgroundMode {
		schedEnableUser(false)
	}

	// If task leak detection is pending, enable it for this GC cycle.
	if work.goroutineLeak.pending.Load() {
		work.goroutineLeak.enabled = true
		work.goroutineLeak.pending.Store(false)
		// Set all sync objects of blocked tasks as untraceable
	...
	// this task becomes runnable again, and we could
...
// other workers.
//
// All tasks performing GC work must call gcBeginWork to signal
...
top:
	...
	// or because of task leak detection.
	if restart || (work.goroutineLeak.enabled && !work.goroutineLeak.done) {
		if restart {
			...
			// Marking has reached a fixed-point. Attempt to detect task leaks.
			//
			// If the returned value is true, then detection already concluded for this cycle.
			// Otherwise, more runnable tasks were discovered, requiring additional mark work.
	...
	// In STW mode, re-enable user tasks. These will be
...
// isMaybeRunnable checks whether a task may still be semantically runnable.
// For tasks which are semantically runnable, this will eventually return true
// as the GC marking phase progresses. It returns false for leaked tasks, or for
// tasks which are not yet computed as possibly runnable by the GC.
func (gp *g) isMaybeRunnable() bool {
	// Check whether the task is actually in a waiting state first.
	if readgstatus(gp) != _Gwaiting {
		// If the task is not waiting, then clearly it is maybe runnable.
		return true
	}
	...
	case waitReasonSelectNoCases,
		...
		// make tasks unrunnable by definition.
		return false
	case waitReasonChanReceive,
		waitReasonSelect,
		waitReasonChanSend:
		// Cycle all through all *sudog to check whether
		// the task is waiting on a marked channel.
...
// findMaybeRunnableGoroutines checks to see if more blocked but maybe-runnable tasks exist.
...
func setSyncObjectsUntraceable() {
	...
	forEachGRace(func(gp *g) {
		// Set as untraceable all synchronization objects of tasks
...
// Should be invoked after the task leak detection phase.
...
// Returns true if the task leak check was performed (or unnecessary).
// Returns false if the GC cycle has not yet computed all maybe-runnable tasks.
func findGoroutineLeaks() bool {
	assertWorldStopped()

	// Report task leaks and mark them unreachable, and resume marking
	...
	if work.nMaybeRunnableStackRoots == work.nStackRoots {
		// nMaybeRunnableStackRoots == nStackRoots means that all tasks are marked.
		return true
	}

	// Check whether any more maybe-runnable tasks can be found by the GC.
	if findMaybeRunnableGoroutines() {
		// We found more work, so we need to resume the marking phase.
		return false
	}

	// For the remaining tasks, mark them as unreachable and leaked.
	...
	// Do not report the main task if it is waiting on select{}.
	//
	// NOTE: We still treat the main task as leaked during the analysis,
	// but revert its status to _Gwaiting after the analysis to not include
	// it in the task leak profile.
	// This preserves the effectiveness of task leak detection
	// if the main task holds references to concurrency primitives causing
	// other leaks.
	//
	// Example:
	//
	...
	// ```
	//
	// The main task is blocked by select{}, but holds a reference to "ch".
	// Not treating the main task as leaked would cause the analysis to
	// miss the legitimate leak at the child task.
	//
	// The main task should always be allgs[0], but double check
...
func gcMarkTermination(stw worldStop) {
	...
	// Bump GC cycle count and wake tasks waiting on sweep.
	lock(&work.sweepWaiters.lock)
	memstats.numgc++
	...
	systemstack(func() {
		// Pull the GC out of task leak detection mode.
...
// gcBgMarkStartWorkers prepares background mark worker tasks. These
// tasks will not run until the mark phase, but they must be started while
...
func gcBgMarkStartWorkers() {
	...
	for gcBgMarkWorkerCount < gomaxprocs {
		...
		// N.B. we intentionally wait on each task individually
		// rather than starting all in a batch and then waiting once
		// afterwards. By running one task at a time, we can take
		// advantage of runnext to bounce back and forth between
		// workers and this task. In an overloaded application,
		// this can reduce GC start latency by prioritizing these
		// tasks rather than waiting on the end of the run queue.
		<-ready
...
func gcBgMarkWorker(ready chan struct{}) {
	...
	for {
		...
		systemstack(func() {
			// Mark our task preemptible so its stack can be scanned or observed
		...
		// point, signal the main GC task.
		if gcEndWork() {
			// We don't need the P-local buffers here, allow
			// preemption because we may schedule like a regular
			// task in gcMarkDone (block on locks, etc).
			releasem(node.m.ptr())
			node.m.set(nil)
