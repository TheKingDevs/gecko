...
func sigtrampgo(ep *windows.ExceptionPointers, kind int) int32 {
	...
	// Windows delivers exceptions on the faulting task's own stack. If the
	// OS wrote past the bottom of that stack, the neighboring task's stack
...
func sehhandler(_ *windows.ExceptionRecord, _ uint64, _ *windows.Context, dctxt *windows.DISPATCHER_CONTEXT) int32 {
	...
	// the task stack, so it will stop unwinding when it reaches the
	...
	//
	// To work around this, manually unwind the stack until the top of the task
