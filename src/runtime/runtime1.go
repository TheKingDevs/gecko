...
// If all is set, print all task stacks. Otherwise, print just the current task.
...
var debug struct {
	...
	// tracebacklabels controls the inclusion of task labels in the
	// task status header line.
...
func parseRuntimeDebugVars(godebug string) {
	...
	debug.cgocheck = 1
	debug.invalidptr = 1
	debug.adaptivestackstart = 1 // set this to 0 to turn larger initial task stacks off
	if GOOS == "linux" {
...
func parsegodebug(godebug string, seen map[string]bool) {
	...
	if debug.cgocheck > 1 {
		throw("cgocheck > 1 mode is no longer supported at runtime. Use GKEXPERIMENT=cgocheck2 at build time instead.")
	}
}
