...
// reset resets the gTraceState for a new task.
...
// traceBlockReason is an enumeration of reasons a task might block.
// This is the interface the rest of the runtime uses to tell the
// tracer why a task blocked. The tracer then propagates this information
...
// traceGoStopReason is an enumeration of reasons a task might yield.
//
...
// traceExitingSyscall marks a task as exiting the syscall slow path.
//
...
// traceExitedSyscall marks a task as having exited the syscall slow path.
...
func (tl traceLocker) ProcStart() {
	pp := tl.mp.p.ptr()
	// Procs are typically started within the scheduler when there is no user task. If there is a user task,
	// it must be in _Gsyscall because the only time a task is allowed to have its Proc moved around from under it
	// is during a syscall.
...
func (tl traceLocker) ProcStop(pp *p) {
	// The only time a task is allowed to have its Proc moved around
...
//
// Must be emitted by an actively running task on an active P. This restriction can be changed
...
func (tl traceLocker) GoUnpark(gp *g, skip int) {
	// Emit a GoWaiting status if necessary for the unblocked task.
...
// GoSwitch emits a GoSwitch event. If destroy is true, the calling task
...
// emitUnblockStatus emits a GoStatus GoWaiting event for a task about to be
...
func (tl traceLocker) emitUnblockStatus(gp *g, gen uintptr) {
	if !gp.trace.statusWasTraced(gen) && gp.trace.acquireStatus(gen) {
		...
		// We can fix this by acquiring the task's scan bit.
...
// if lostP is true.
//
// lostP must be true in all cases that a task loses its P during a syscall.
// This means it's not sufficient to check if it has no P. In particular, it needs to be
// true in the following cases:
// - The task lost its P, it ran some other code, and then got it back. It's now running with that P.
// - The task lost its P and was unable to reacquire it, and is now running without a P.
// - The task lost its P and acquired a different one, and is now running with that P.
...
func (tl traceLocker) ProcSteal(pp *p) {
	...
	// The status of the proc and task, if we need to emit one here, is not evident from the
	...
	// fairly succinctly. In the former, we're a regular running task and proc, if we have either.
	// In the latter, we're a task in a syscall.
...
// GoCreateSyscall indicates that a task has transitioned from dead to GoSyscall.
//
...
func (tl traceLocker) GoCreateSyscall(gp *g) {
	// N.B. We should never trace a status for this task (which we're currently running on),
	// since we want this to appear like task creation.
...
// GoDestroySyscall indicates that a task has transitioned from GoSyscall to dead.
//
...
func (tl traceLocker) GoDestroySyscall() {
	// Write the status for the task if necessary.
