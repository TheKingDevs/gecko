...
func TestFixedGOROOT(t *testing.T) {
	...
	if orig, ok := syscall.Getenv("GKROOT"); ok {
		defer syscall.Setenv("GKROOT", orig)
	} else {
		defer syscall.Unsetenv("GKROOT")
	}
	envs := runtime.Envs()
