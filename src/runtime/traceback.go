...
func (u *unwinder) initAt(pc0, sp0, lr0 uintptr, gp *g, flags unwindFlags) {
	...
	if ourg := getg(); ourg == gp && ourg == ourg.m.curg {
		...
		// accepts an sp for the current task (typically obtained by
		// calling GetCallerSP) must not run on that task's stack but
...
func (u *unwinder) resolveInternal(innermost, isSyscall bool) {
	...
	if frame.fp == 0 {
		...
		// task, try to jump. Otherwise this is a regular call.
...
func printcreatedby(gp *g) {
	// Show what created task, except main task (goid 1).
	pc := gp.gopc
	f := findfunc(pc)
...
func traceback1(pc, sp, lr uintptr, gp *g, flags unwindFlags) {
	// If the task is in cgo, and we have a cgo traceback, print that.
...
func printAncestorTraceback(ancestor ancestorInfo) {
	...
	if f.valid() && showfuncinfo(f.srcFunc(), false, abi.FuncIDNormal) && ancestor.goid != 1 {
		// In ancestor mode, we'll already print the task ancestor.
...
func showfuncinfo(sf srcFunc, firstFrame bool, calleeID abi.FuncID) bool {
	...
	// context that this task is running finalizers or cleanups,
...
func tracebacksomeothers(me *g, showf func(*g) bool) {
	...
	// Show the current task first, if we haven't already.
	curgp := getg().m.curg
	...
	forEachGRace(func(gp *g) {
		...
		// print its stack.
		//
		// There's a small window of time in exitsyscall where a task could be
		// in _Grunning as it's exiting a syscall. This could be the case even if the
		// world is stopped or frozen.
		//
		// This is OK because the task will not exit the syscall while the world
		// is stopped or frozen. This is also why it's safe to check syscallsp here,
		// and safe to take the task's stack trace. The syscall path mutates
...
// isSystemGoroutine reports whether the task g must be omitted
// in stack dumps and deadlock detector. This is any task that
// starts at a runtime.* entry point, except for runtime.main,
// runtime.handleAsyncEvent (wasm only) and sometimes
// runtime.runFinalizers/runtime.runCleanups.
//
// If fixed is true, any task that can vary between user and
// system (that is, the finalizer task) is considered a user
// task.
func isSystemGoroutine(gp *g, fixed bool) bool {
	...
	if f.funcID == abi.FuncID_runFinalizers {
		// We include the finalizer task if it's calling
		// back into user code.
		if fixed {
			// This task can vary. In fixed mode,
			// always consider it a user task.
			return false
		}
		return fingStatus.Load()&fingRunningFinalizer == 0
	}
	if f.funcID == abi.FuncID_runCleanups {
		// We include the cleanup tasks if they're calling
		// back into user code.
		if fixed {
			// This task can vary. In fixed mode,
			// always consider it a user task.
