...
// where n is the number of distinct addresses with tasks blocked
...
func semrelease1(addr *uint32, handoff bool, skipframes int) {
	...
	lockWithRank(&root.lock, lockRankRoot)
	if root.nwait.Load() == 0 {
		// The count is already consumed by another task,
		// so no need to wake up another task.
		unlock(&root.lock)
		return
	}
	...
	if s != nil { // May be slow or even yield, so unlock first
		...
		if acquiretime != 0 {
			...
			// If there are N more tasks waiting beyond the
			// one that's waking up, charge their delay as well, so that
			// contention holding up many tasks shows up as
			// more costly than contention holding up a single task.
			// It would take O(N) time to calculate how long each task
...
// dequeue searches for and finds the first task
...
Found:
	...
	// Task is no longer blocked. Clear the waiting pointer.
	s.g.waiting = nil
	s.parent = nil
	s.elem.set(nil)
	s.next = nil
...
type notifyList struct {
	...
	// For this not to be the case, we'd need to have 2^31+ tasks
...
func notifyListWait(l *notifyList, t uint32) {
	...
	// Task is no longer blocked. Clear up its waiting pointer,
