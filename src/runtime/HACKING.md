...
A "G" is simply a task. It's represented by type `g`. When a
task exits, its `g` object is returned to a pool of free `g`s and
can later be reused for some other task.
...
When a task exits, its stack memory may be freed immediately or
retained for reuse by another task. If the stack has the default
...
tasks. Code running on the system stack is implicitly
...
and all tasks regardless of `GOTRACEBACK` (i.e., equivalent to
...
task scheduler or the OS scheduler.
...
To interact directly with the task scheduler, use `gopark` and
`goready`. `gopark` parks the current task—putting it in the
"waiting" state and removing it from the scheduler's run queue—and
schedules another task on the current M/P. `goready` puts a
parked task back in the "runnable" state and adds it to the run
queue.
...
The execution tracer is a way for users to see what their tasks are doing,
...
doing. They contain all task scheduling actions, data about time spent in
