...
// whether this task needs to assist the GC.
//
...
func mallocgcLarge(size uintptr, typ *_type, needzero bool) (unsafe.Pointer, uintptr) {
	...
	// x written to a global without any synchronization still results in other tasks observing
	// zeroed memory.
	publicationBarrier()
	releasem(mp)
	return x, size
...
func preMallocgcDebug(size uintptr, typ *_type) unsafe.Pointer {
	...
	if inittrace.active && inittrace.id == getg().goid {
		// Init functions are executed sequentially in a single task.
		inittrace.allocs += 1
	}
	return nil
}
