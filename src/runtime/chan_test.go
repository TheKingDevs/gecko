...
//
// The second task does a non-blocking select receiving from c1 and c2.
// From the time the second task is created, at least one of c1 and c2
// is always ready for receiving, so the select in the second task must
...
func TestSelectStress(t *testing.T) {
	...
	// There are 4 tasks that send N values on each of the chans,
	// + 4 tasks that receive N values on each of the chans,
	// + 1 task that sends N values on each of the chans in a single select,
	// + 1 task that receives N values on each of the chans in a single select.
...
func TestSelectFairness(t *testing.T) {
	...
	// If the select in the task is fair,
...
func TestSelectDuplicateChannel(t *testing.T) {
	...
	time.Sleep(time.Millisecond) // make sure task A gets queued first on c

	// task B
	go func() {
		<-c
	}()
	time.Sleep(time.Millisecond) // make sure task B gets queued on c before continuing
...
func TestSelectStackAdjust(t *testing.T) {
	...
	// Let the tasks get into the select.
	<-ready1
	<-ready2
