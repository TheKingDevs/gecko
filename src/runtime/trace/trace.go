...
// task creation/blocking/unblocking, syscall enter/exit/block,
...
// which task called [Log]. The execution tracer provides UIs to filter
// and group tasks using the log category and the message supplied
// in [Log].
//
// A region is for logging a time interval during a task's execution.
// By definition, a region starts and ends in the same task.
...
// interesting local operation which may require multiple tasks
// working together. Since tasks can involve multiple tasks,
...
// and mix milk and coffee in separate tasks. With a task,
// the trace tool can identify the tasks involved in a specific
// cappuccino order.
//
