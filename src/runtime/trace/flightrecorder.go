...
// It is safe to call from multiple tasks simultaneously.
...
// Only one task may execute WriteTo at a time.
...
func (fr *FlightRecorder) WriteTo(w io.Writer) (n int64, err error) {
	...
	//
	// N.B. traceAdvance blocks until the tracer task has actually written everything
	// out, which means the generation we just flushed must have been already been observed
	// by the recorder task. Because we flushed twice, the first flush is guaranteed to
	// have been both completed *and* processed by the recorder task.
	fr.ringMu.Lock()
	gens := fr.ring
	fr.ringMu.Unlock()
