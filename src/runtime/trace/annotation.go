...
//	go func() {  // continue processing the task in a separate task.
//	    defer task.End()
...
// WithRegion starts a region associated with its calling task, runs fn,
...
// from the same task where the region was started.
// Within each task, regions must nest. That is, regions started
