...
func (t *traceMultiplexer) addedSubscriber() error {
	if t.enabled.Load() {
		// This is necessary for the trace reader task to pick up on the new subscriber.
...
func (t *traceMultiplexer) startLocked() error {
	...
	// Grab the trace reader task's subscribers.
	//
	...
	// task sees an end-of-generation signal.
	t.subscribersMu.Lock()
