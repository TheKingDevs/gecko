...
func TestGoroutineProfileTrivial(t *testing.T) {
	// Calling GoroutineProfile twice in a row should find the same number of tasks,
	// but it's possible there are tasks just about to exit, so we might end up
	// with fewer in the second call. Try a few times; it should converge once those
	// zombies are gone.
	for i := 0; ; i++ {
		n1, ok := GoroutineProfile(nil) // should fail, there's at least 1 task
		if n1 < 1 || ok {
...
func BenchmarkGoroutineProfile(b *testing.B) {
	...
	// Measure the cost of counting tasks
	b.Run("small-nil", run(func() bool {
		GoroutineProfile(nil)
		return true
	}))

	// Measure the cost with a small set of tasks
	n := NumGoroutine()
	...
	// Measure the cost with a large set of tasks
	ch := make(chan int)
	...
	// Count tasks with a large allgs list
	...
	// Count tasks with a large (but unused) allgs list
...
func BenchmarkMutexHandoff(b *testing.B) {
	testcase := func(delay func(l *Mutex)) func(b *testing.B) {
		return func(b *testing.B) {
			...
			// "coordinator" task and the other running a "worker"
			// task. We don't override the runtime's typical
			// task/thread mapping behavior.
			//
			...
			//
			// The two threads' tasks communicate their current position to
