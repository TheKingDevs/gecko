...
type Conn struct {
	...
	// the rest of the bits are the number of tasks in Conn.Write.
...
func (c *Conn) Read(b []byte) (int, error) {
	...
	// task that the connection is now closed. This eliminates a race
	// where the HTTP response reading task would otherwise not observe
	// the EOF until its next read, by which time a client task might
