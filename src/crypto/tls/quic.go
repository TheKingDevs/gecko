...
type quicState struct {
	...
	// readbuf is shared between HandleData and the handshake task.
	// HandshakeCryptoData passes ownership to the handshake task by
...
func (q *QUICConn) Close() error {
	...
	for range q.conn.quic.blockedc {
		// Wait for the handshake task to return.
	}
	return q.conn.handshakeErr
}
...
func (q *QUICConn) HandleData(level QUICEncryptionLevel, data []byte) error {
	...
	if ok {
		// The handshake task is waiting for more data.
		return nil
	}
	// The handshake task has exited.
