...
func testClosedPipeRace(t *testing.T, read bool) {
	...
	// Close the read end of the pipe in a task while we are
	// writing to the write end, or vice-versa.
	go func() {
		// Give the main task a chance to enter the Read or
...
func testCloseWithBlockingRead(t *testing.T, r, w *os.File) {
	...
	// Give the task a chance to enter the Read
...
func testPipeEOF(t *testing.T, r io.ReadCloser, w io.WriteCloser) {
	// parkDelay is an arbitrary delay we wait for a pipe-reader task to park
...
func TestFdRace(t *testing.T) {
	// This test starts 100 simultaneous tasks, which could bury a more
...
func TestFdReadRace(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		<-c
		// Give the other task a chance to enter the Read.
