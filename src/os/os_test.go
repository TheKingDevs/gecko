...
func TestProgWideChdir(t *testing.T) {
	...
	// to ensure the N tasks have been released even if the main task
	...
	// so as not to interfere while the N tasks are still running.
	defer wg.Wait()
	defer close(done)
	...
	for i := 0; i < N; i++ {
		...
		go func(i int) {
			defer wg.Done()
			// Lock half the tasks in their own operating system
			// thread to exercise more scheduler possibilities.
			if i%2 == 1 {
				// On Plan 9, after calling LockOSThread, the tasks
...
func TestPipeThreads(t *testing.T) {
	...
	// If we are still alive, it means that the 100 tasks did
...
func TestPipeIOCloseRace(t *testing.T) {
	...
	go func() {
		...
		// Let the other tasks start. This is just to get
