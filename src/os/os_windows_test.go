...
func TestFileAssociatedWithExternalIOCP(t *testing.T) {
	...
	// Wait fot he task to call GetQueuedCompletionStatus.
	time.Sleep(100 * time.Millisecond)
...
func TestFileFdWithConcurrentIO(t *testing.T) {
	...
	// Write will block until the task reads all 2 bytes.
...
func TestNewFileStdinBlocked(t *testing.T) {
	...
	// Unblock the read to let the task exit.
	if _, err := file.Write(make([]byte, 1)); err != nil {
		t.Fatal(err)
	}
	wg.Wait() // Don't leave tasks behind.
}
