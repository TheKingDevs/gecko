...
func TestTickerStress(t *testing.T) {
	...
	go func() {
		for !stop.Load() {
			...
			// so that it can generate channel sends for the main task,
...
func TestAfterFuncStarvation(t *testing.T) {
	// Start two tasks ping-ponging on a channel send.
	// At any given time, at least one of these tasks is runnable:
	...
	// the AfterFunc task instead of the runnable channel task.
...
func testAfterQueuing1(delta Duration, after func(Duration) <-chan Time) error {
	...
	// fired in. The order in which the tasks started above manage to send
...
func TestSleepZeroDeadlock(t *testing.T) {
	...
	// Then the task calls e.g. new and falls down into the scheduler due to pending GC.
	// After the GC nobody wakes up the task from Gwaiting status.
...
// Test having a large number of tasks wake up a ticker simultaneously.
...
// Test having a large number of tasks wake up a timer simultaneously.
// This used to trigger a crash when run under x/tools/cmd/stress.
...
func BenchmarkParallelTimerLatency(b *testing.B) {
	...
	// avoids using any other timers. In particular, the main task uses
