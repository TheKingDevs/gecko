...
func TestLongAdjustTimers(t *testing.T) {
	...
	// Set up a queuing task to ping pong through the scheduler.
	inQ := make(chan func())
	outQ := make(chan func())
...
func testTimerChan(t *testing.T, tim timer, C <-chan Time) {
	...
	// Test stop while timer in heap (because task is blocked on <-C).
	done := make(chan bool)
	notDone(done)
	go func() {
		<-C
	...
	// Again using select and with two tasks waiting.
	tim.Reset(10000 * Second)
