...
// Sleep pauses the current task for at least the duration d.
...
// and the function f has been started in its own task;
...
// task running f does not run concurrently with the prior
...
func sendTime(c any, seq uintptr, delta int64) {
	...
	// can delay a sendTime call until a task tries to receive from
...
// in its own task. It returns a [Timer] that can
