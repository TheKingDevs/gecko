...
//
// If any task calls [RWMutex.Lock] while the lock is already held by
...
// task. One task may [RWMutex.RLock] ([RWMutex.Lock]) a RWMutex and then
// arrange for another task to [RWMutex.RUnlock] ([RWMutex.Unlock]) it.
...
// syscall_hasWaitingReaders reports whether any task is waiting
