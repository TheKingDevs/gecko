...
// for tasks waiting for or announcing the occurrence
// of an event.
//
...
// of the calling task. After later resuming execution,
...
// Signal wakes one task waiting on c, if there is any.
//
// It is allowed but not required for the caller to hold c.L
// during the call.
//
// Signal() does not affect task scheduling priority; if other tasks
// are attempting to lock c.L, they may be awoken before a "waiting" task.
func (c *Cond) Signal() {
	c.checker.check()
	runtime_notifyListNotifyOne(&c.notify)
}

// Broadcast wakes all tasks waiting on c.
//
// It is allowed but not required for the caller to hold c.L during the call.
// The time it takes to run Broadcast is proportional to the number of waiting tasks;
