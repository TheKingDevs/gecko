...
// for a group of tasks or tasks to finish.
//
// Typically, a main task will start tasks, each in a new
// task, by calling [WaitGroup.Go] and then wait for all tasks to
...
// start new tasks by using [WaitGroup.Add] and [WaitGroup.Done].
//
// The previous example can be rewritten using explicitly created
// tasks along with Add and Done:
//
//	var wg sync.WaitGroup
//	wg.Add(1)
//	go func() {
...
// If the counter becomes zero, all tasks blocked on [WaitGroup.Wait] are released.
...
// creating the task or other event to be waited for.
...
func (wg *WaitGroup) Add(delta int) {
	...
	// This task has set counter to 0 when waiters > 0.
...
// Go calls f in a new task and adds that task to the [WaitGroup].
...
// This means a task started by Go may itself call Go.
	...
	go func() {
		defer func() {
			if x := recover(); x != nil {
				...
				// this is a new task.
				//
				// Calling Done will unblock Wait in the main task,
