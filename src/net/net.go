...
request consumes only a task, while a blocked C call consumes an operating system thread.
...
//
// Multiple tasks may invoke methods on a Conn simultaneously.
type Conn interface {
...
//
// Multiple tasks may invoke methods on a PacketConn simultaneously.
...
// another task before the I/O is completed. This may be wrapped
...
// Limit the number of concurrent cgo-using tasks, because
// each will block an entire operating system thread. The usual culprit
// is resolving many DNS names in separate tasks but the DNS
