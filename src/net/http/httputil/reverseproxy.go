...
func (p *ReverseProxy) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	...
	if ctx.Done() != nil {
		...
		// bother spinning up a task to watch the CloseNotify
		// channel (if any).
		//
	...
	if outreq.Body != nil {
		...
		// allowed, and the RoundTrip task that reads the Body can outlive
...
func (m *maxLatencyWriter) delayedFlush() {
	m.mu.Lock()
	defer m.mu.Unlock()
	if !m.flushPending { // if stop was called but AfterFunc already started this task
		return
	}
	m.flush()
...
// switchProtocolCopier exists so tasks proxying data back and
