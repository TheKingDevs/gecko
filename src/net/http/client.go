...
// needed. Clients are safe for concurrent use by multiple tasks.
//
...
type RoundTripper interface {
	...
	// read fields of the request in a separate task. Callers
	// should not mutate or reuse the request until the Response's
	// Body has been closed.
	//
	// RoundTrip must always close the body, including on errors,
	// but depending on the implementation may do so in a separate
	// task even after RoundTrip returns. This means that
...
func knownRoundTripperImpl(rt RoundTripper, req *Request) bool {
	...
	// some temporarily leaked tasks if the transport didn't
