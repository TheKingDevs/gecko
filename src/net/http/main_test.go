...
// Verify the other tests didn't leave any tasks running.
func goroutineLeaked() bool {
	if testing.Short() || runningBenchmarks() {
		// Don't worry about task leaks in -short mode or in
	...
	for i := 0; i < 5; i++ {
		...
		// Wait for tasks to schedule and die off:
...
func afterTest(t testing.TB) {
	...
	if leakReported {
		...
		// tasks are a new leak from a subsequent test or just the same
		// tasks from the first leak still hanging around, and we may add a lot
	...
	// report the tasks from a test that is still running as a leak from a
	...
	for i := 0; i < 2500; i++ {
		...
		// Bad stuff found, but tasks might just still be
