...
// Transports are safe for concurrent use by multiple tasks.
//
...
type readTrackingBody struct {
	io.ReadCloser
	didRead  bool // not atomic.Bool because only one task (the user's) should be accessing
...
func (t *Transport) tryPutIdleConn(pconn *persistConn) error {
	...
	// because multiple tasks can use them simultaneously.
	// If this is an HTTP/2 connection being “returned,” we're done.
	if pconn.alt != nil && t.idleLRU.m[pconn] != nil {
		return nil
	}

	// Deliver pconn to task waiting for idle connection, if any.
...
func (t *Transport) queueForIdleConn(w *wantConn) (delivered bool) {
	...
	if list, ok := t.idleConn[w.key]; ok {
		...
		for len(list) > 0 && !stop {
			...
			if tooOld {
				// Async cleanup. Launch in its own task (as if a
...
type wantConn struct {
	...
	// beforeDial is called in the getConn task when the dial is queued.
...
// Once w receives permission to dial, it will do so in a separate task.
...
// startDialConnFor calls dialConn in a new task.
...
// which may in turn give a different waiting task permission to dial.
func (t *Transport) decConnsPerHost(key connectMethodKey) {
	...
	// Can we hand this count to a task still waiting to dial?
	// (Some tasks on the wait list may have timed out or
...
func (t *Transport) dialConn(ctx context.Context, cm connectMethod, isClientConn bool, internalStateHook func()) (pconn *persistConn, err error) {
	...
	case cm.targetScheme == "https":
		...
		// and leak a task if the connection stops replying after
		// the TCP connect.
...
type persistConn struct {
	...
	// from the writeLoop task to the readLoop which passes
...
func (pc *persistConn) mapRoundTripError(req *transportRequest, startBytesWritten int64, err error) error {
	...
	// Wait for the writeLoop task to terminate to avoid data
...
func (pc *persistConn) readLoop() {
	...
	// eofc is used to block caller tasks reading from Response.Body
	// at EOF until this tasks has (potentially) added the connection
	...
	for alive {
		...
		if !hasBody || bodyWritable {
			...
			// out of the select that also waits on this task to die, so
		...
		// the bufio.Reader, wait for the caller task to finish
...
func (pc *persistConn) wroteRequest() bool {
	...
	default:
		...
		// write task a bit of time to finish its send.
		//
...
// responseAndError is how the task reading from an HTTP/1 server
// communicates with the task doing the RoundTrip.
...
// A writeRequest is sent by the caller's task to the
// writeLoop's task to write a request while the read loop
