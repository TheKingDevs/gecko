...
// mutexHog starts multiple tasks that runs the given hogger function for the specified duration.
...
func TestDeltaProfile(t *testing.T) {
	...
	defer func() { // cleanup the above task.
		done <- true
		<-done // wait for the task to exit.
	}()
