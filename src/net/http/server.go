...
type conn struct {
	...
	// inside the Listener's Accept task, as some implementations block.
	// It is populated immediately inside the (*conn).serve task.
...
// call blocked in a background task to wait for activity and
...
func (cr *connReader) backgroundRead() {
	...
	if ne, ok := err.(net.Error); ok && cr.aborted && ne.Timeout() {
		...
		// another task calling abortPendingRead.
	} else if err != nil {
...
func (c *conn) serve(ctx context.Context) {
	...
	for {
		...
		// so we might as well run the handler in this task.
...
// creating a new service task for each. The service tasks
...
// new service task for each. The service tasks read requests and
...
// new service task for each. The service tasks perform TLS
