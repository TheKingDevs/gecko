...
type serverTester struct {
	...
	// the read and write tasks, which may hide data races.
...
// do executes f in the handler's task.
...
// sync waits for all tasks to idle.
...
func testServer_MaxQueuedControlFrames(t *testing.T) {
	// Task debugging makes this test very slow.
...
func testServerMaxHandlerGoroutines(t *testing.T) {
	...
	// Reset them all, but only after the handler tasks have started.
	var stops []chan bool
	streamID := uint32(1)
