...
// for concurrent use by multiple tasks.
...
type clientStream struct {
	...
	// task has exited without waiting for the stream to end, and
	// that cleanupWriteRequest should instead be run (on a new task)
...
// onIdleTimeout is called from a time.AfterFunc task. It will
// only be called when we're idle, but because we're coming from a new
// task, there could be a new request coming in at the same time,
...
func (cc *ClientConn) roundTrip(req *ClientRequest, streamf func(*clientStream)) (*ClientResponse, error) {
	...
	cancelRequest := func(cs *clientStream, err error) error {
		...
		// will have started a task to close it.
		//
		// Closing the body before returning avoids a race condition
		// with net/http checking its readTrackingBody to see if the
		// body was read from or closed. See golang/go#60041.
		//
		// The body is closed in a separate task without the
...
// letting the writeRequest task exit instead of parking until then.
//
...
// task and its stack for the stream's lifetime doing nothing but
// waiting.
//
...
func (cs *clientStream) detach(respHeaderTimeout time.Duration) bool {
	...
	// Watch for request context cancellation without parking a task
	// on ctx.Done(). If the context was canceled already, AfterFunc runs
	// the func in a new task, which blocks acquiring cc.mu until we
	// return.
	//
...
func (cs *clientStream) writeRequest(req *ClientRequest, streamf func(*clientStream)) (err error) {
	...
	// task to do but wait for the stream to end, detach from the
	// stream and exit rather than pinning this task (and its stack)
	...
	// task, so that (rare) case keeps the historical behavior of
	// waiting here.
...
// readLoop runs in its own task and reads and dispatches frames.
...
func (b transportResponseBody) Close() error {
	...
	if unread > 0 {
		...
		// Move flow control return to a task?
		cc.wmu.Lock()
		// Return connection-level flow control.
		if connAdd > 0 {
