...
// #15425: Transport task leak while the transport is still trying to
...
// the request body writer task had already read an EOF from the
...
func (rc *closeChecker) isClosed() error {
	...
	// but that it may do so in a separate task. Wait a reasonable
...
// TestTransportRequestGoroutineExits verifies that the task spawned to
...
// concurrent long-lived streams (long polls), a parked task and its
...
func testTransportRequestGoroutineExits(t *testing.T) {
	...
	// Count the tasks net/http has parked (the connection's read
	...
	// end in sight. The request-writing task should be gone,
	// leaving only the tasks that predate the request.
	synctest.Wait()
...
// bubbleNetHTTPGoroutines returns the number of tasks in the calling
...
func bubbleNetHTTPGoroutines(t *testing.T) int {
	...
	// The first record is the calling task, whose header names the
	// test's bubble: "task 8 [running, synctest bubble 3]:".
...
// task, and once response headers arrive the timer is disarmed and
...
func testTransportRequestGoroutineExitsRespHeaderTimeout(t *testing.T) {
	...
	// The request-writing task should be gone even before response
