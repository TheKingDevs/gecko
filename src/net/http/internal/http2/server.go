...
// TODO: turn off the serve task when idle, so
// an idle conn only has the readFrames task active. (which could
// also be optimized probably to pin less memory in crypto/tls). This
// would involve tracking when the serve task is active (atomic
...
type serverConn struct {
	...
	curHandlers                 uint32 // number of running handler tasks
	...
	writingFrame                bool              // started writing a frame (on serve task or separate)
	writingFrameAsync           bool              // started a frame on its own task but haven't heard back on wroteFrameCh
	...
	// Owned by the writeFrameAsync task:
...
// the serve task. Most of the actual stream state is owned by
// the http.Handler's task in the responseWriter. Because the
...
// frameWriteResult is the message passed from writeFrameAsync to the serve task.
...
// writeFrameAsync runs in its own task and writes a single frame
// and then reports when it's done.
// At most one task can be running writeFrameAsync at a time per
// serverConn.
...
func (sc *serverConn) writeDataFromHandler(stream *stream, data []byte, endStream bool) error {
	...
	*writeArg = writeData{stream.id, data, endStream}
	...
	case <-stream.cw:
		...
		// The writeFrameAsync and serve tasks guarantee
...
//
// This must not be run from the serve task itself, else it might
// deadlock writing to sc.wantWriteFrameCh (which is only mildly
// buffered and is read by serve itself). If you're on the serve
// task, call writeFrame instead.
...
//
// There is no pushback here (the serve task never blocks). It's
// the http.Handlers that block, waiting for their previous frames to
// make it onto the wire
//
// If you're not on the serve task, use writeFrameFromHandler instead.
...
// startFrameWrite starts a task to write wr (in a separate
// task since that might block on the network), and updates the
// serve task's state about the world, updated from info in wr.
func (sc *serverConn) startFrameWrite(wr FrameWriteRequest) {
	...
	if wr.write.staysWithinBuffer(sc.bw.Available()) {
		...
		// Encode the frame in the serve task, to ensure we don't have
...
// Request.Body when the main task panics. Since most handlers read in the
// main ServeHTTP task, this will show up rarely.
var errHandlerPanicked = errors.New("http2: handler panicked")

// wroteFrame is called on the serve task with the result of
...
func (sc *serverConn) wroteFrame(res frameWriteResult) {
	...
	// Reply (if requested) to unblock the ServeHTTP task.
...
func (sc *serverConn) processSettings(f *SettingsFrame) error {
	...
	if f.IsAck() {
		...
		if sc.pendingDecoderTableSize != 0 {
			...
			// the configured size to the decoder. The read task
...
// copyTrailersToHandlerRequest is run in the Handler's task in
...
// onReadTimeout is run on its own task (from time.AfterFunc)
...
// onWriteTimeout is run on its own task (from time.AfterFunc)
...
// scheduleHandler starts a handler task,
...
func (sc *serverConn) handlerDone() {
	...
	for ; i < len(sc.unstartedHandlers); i++ {
		...
		if sc.streams[u.streamID] == nil {
			// This stream was reset before its task had a chance to start.
			continue
		}
		if sc.curHandlers >= maxHandlers {
			break
