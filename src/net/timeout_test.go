...
func TestAcceptTimeoutMustNotReturn(t *testing.T) {
	...
	case <-max.C:
		ln.Close()
		<-ch // wait for tester task to stop
	}
}
...
func TestReadTimeoutMustNotReturn(t *testing.T) {
	...
	case <-max.C:
		c.Close()
		err := <-ch // wait for tester task to stop
		if perr := parseReadError(err); perr != nil {
			t.Error(perr)
		}
...
func TestReadTimeoutFluctuation(t *testing.T) {
	...
	for {
		...
		if want := timeoutUpperBound(d); actual > want {
			...
			// Maybe this machine is too slow to reliably schedule tasks within
...
func TestReadWriteDeadlineRace(t *testing.T) {
	...
	wg.Wait() // wait for tester task to stop
}
