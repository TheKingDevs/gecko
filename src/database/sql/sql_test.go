...
func init() {
	...
	putConnHook = func(db *DB, c *driverConn) {
		if slices.Contains(db.freeConn, c) {
			...
			// (all tasks asleep) elsewhere, since we might not unlock
...
func testConnectionLeak(t *testing.T, db *DB) {
	...
	// Wait until the task we've just created has started waiting.
	<-drv.waitingCh
	// Now close the busy connections. This provides a connection for
	// the blocked task and then fills up the idle queue.
	for _, v := range rows {
		v.Close()
	}
...
func testContextCancelDuringRawBytesScan(t *testing.T, mode string) {
	...
	// 2 or 3 depending on how the awaitDone task schedules.
	switch numRows {
	case 0, 1:
...
func testContextCancelBetweenNextAndErr(t *testing.T, db *DB) {
	...
	cancel() // wake up the awaitDone task
	synctest.Wait()
	if err := r.Err(); err != nil {
		t.Fatal(err)
	}
...
func testNilErrorAfterClose(t *testing.T, db *DB) {
	...
	// spawning a task when the query/transaction context cannot be
	// canceled, but this test tests a bug which is caused by said task.
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
...
func BenchmarkManyConcurrentQueries(b *testing.B) {
	b.ReportAllocs()
	// To see lock contention in Go 1.4, 16~ cores and 128~ tasks are required.
