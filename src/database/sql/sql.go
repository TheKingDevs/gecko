...
type DB struct {
	...
	// a task running connectionOpener() reads on this chan and
...
// [DB.Ping].
//
// The returned [DB] is safe for concurrent use by multiple tasks
...
// long-lived and shared between many tasks.
...
// Runs in a separate task, opens new connections when requested.
	...
	if db.maxOpen > 0 && db.numOpen >= db.maxOpen {
		...
		case <-ctx.Done():
			...
			if !deleted {
				...
				// should probably start a task to read from req. This best
				...
				// task) to get the connection back.
				select {
				default:
				case ret, ok := <-req:
					if ok && ret.conn != nil {
...
// A Stmt is safe for concurrent use by multiple tasks.
//
