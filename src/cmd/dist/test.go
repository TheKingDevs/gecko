...
func (t *tester) run() {
	...
	if !t.json {
		if t.failed {
			...
			fmt.Println()
			errprintf("gecko tool dist: warning: %q matched no tests; use the -list flag to list available tests\n", t.runRxStr)
		...
		for i := 1; i <= 4; i *= 2 {
			t.registerTest(fmt.Sprintf("GOMAXPROCS=2 runtime -cpu=%d -quick", i),
				&goTest{
					...
					// creation of first tasks and first garbage collections in the parallel setting.
...
func (t *tester) runPending(nextTest *distTest) {
	...
	for ended < len(worklist) {
		...
		if dt != last {
			...
			last = w.dt
			if vflag > 0 {
				fmt.Printf("# gecko tool dist test -run=^%s$\n", dt.name)
			}
		}
		if vflag > 1 {
			errprintf("%#q\n", w.cmd)
...
func (t *tester) registerRaceTests() {
	...
	// t.registerTest(hdr, &goTest{variant: "race", race: true, runTests: "TestParallelTest", pkg: "cmd/gecko"})
	if t.cgoEnabled {
