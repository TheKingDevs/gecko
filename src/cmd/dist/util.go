...
func runEnv(dir string, mode int, env []string, cmd ...string) string {
	...
	if err != nil && mode&CheckExit != 0 {
		...
		if mode&Background != 0 {
			// Prevent fatalf from waiting on our own task's
			// bghelper to exit:
			bghelpers.Done()
		}
...
// bgwait must be called from only a single task at a time.
...
func xworkdir() string {
	name, err := os.MkdirTemp(os.Getenv("GKTMPDIR"), "go-tool-dist-")
	if err != nil {
		fatalf("%v", err)
	}
	return name
...
func fatalf(format string, args ...any) {
	fmt.Fprintf(os.Stderr, "gecko tool dist: %s\n", fmt.Sprintf(format, args...))
