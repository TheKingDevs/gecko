...
// gState represents the trace viewer state of a task in a trace.
//
...
type gState[R resource] struct {
	...
	executing R // The resource this task is executing on. (Could be itself.)
	...
	// activeRanges is the set of all active ranges on the task.
	activeRanges map[string]activeRange

	// completedRanges is a list of ranges that completed since before the
	// task stopped executing. These are flushed on every stop or block.
	completedRanges []completedRange

	// startRunningTime is the most recent event that caused a task to
	// transition to GoRunning.
	startRunningTime trace.Time

	// startSyscall is the most recent event that caused a task to
	...
	// event that caused a task to transition to GoWaiting.
	startBlockReason string

	// startCause is the event that allowed this task to start running.
	// It's used to generate flow events. This is typically something like
	// an unblock event or a task creation event.
	//
...
// newGState constructs a new task state for the task
...
// augmentName attempts to use stk to augment the name of the task
// with stack information. This stack must be related to the task
...
// setLabel adds an additional label to the task's name.
func (gs *gState[R]) setLabel(label string) {
	gs.label = label
}

// name returns a name for the task.
...
// setStartCause sets the reason a task will be allowed to start soon.
...
// created indicates that this task was just created by the provided creator.
...
// start indicates that a task has started running on a proc.
...
// syscallBegin indicates that the task entered a syscall on a proc.
...
// task is no longer executing on the resource (e.g. a proc) whereas blockedSyscallEnd
// is the point at which the task actually exited the syscall regardless of which
...
// to emit a flow event from, indicating explicitly that this task was unblocked by the system.
...
// unblock indicates that the task gs represents has been unblocked.
...
// block indicates that the task has stopped executing on a proc -- specifically,
...
// stop indicates that the task has stopped executing on a proc.
...
// finalize writes out any in-progress slices as if the task stopped.
...
func (gs *gState[R]) rangeBegin(ts trace.Time, name string, stack trace.Stack) {
	if gs.executing != R(noResource) {
		...
		// If the task isn't executing, there's no place for
...
func (gs *gState[R]) rangeActive(name string) {
	if gs.executing != R(noResource) {
		...
		// from wherever the task started running from.
