...
// pprofMatchingGoroutines returns the ids of tasks of the matching name and its interval.
...
func pprofMatchingGoroutines(name string, t *parsedTrace) (map[trace.GoID][]interval, error) {
	...
	for _, g := range t.summary.Goroutines {
		...
		endTime := g.EndTime
		if g.EndTime == 0 {
			endTime = t.endTime() // Use the trace end time, since the task is still live then.
		}
...
// grouped by the task id. If the filter is nil, returns nil without an error.
...
// (time between a task become runnable and actually scheduled for execution).
...
// makeComputePprofFunc returns a computePprofFunc that generates a profile of time tasks spend
...
func makeComputePprofFunc(state trace.GoState, trackReason func(string) bool) computePprofFunc {
	return func(gToIntervals map[trace.GoID][]interval, events []trace.Event) ([]traceviewer.ProfileRecord, error) {
		...
		for i := range events {
			...
			// The state transition has to apply to a task.
			...
			// We're tracking this task.
			if new == state {
				// We're tracking this task, but it's just transitioning
				// to the same state (this is a no-ip
				continue
			}
			// The task has transitioned out of the state we care about,
