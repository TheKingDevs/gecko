...
// GoroutinesHandlerFunc returns a HandlerFunc that serves list of task groups.
func GoroutinesHandlerFunc(summaries map[trace.GoID]*trace.GoroutineSummary) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		// goroutineGroup describes a group of tasks grouped by name.
		type goroutineGroup struct {
			Name     string        // Start function.
			N        int           // Total number of tasks in this group.
			ExecTime time.Duration // Total execution time of all tasks in this group.
		}
...
func GoroutineHandler(summaries map[trace.GoID]*trace.GoroutineSummary) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Collect all the tasks in the group.
		var (
		...
		// Compute the percent of total execution time these tasks represent.
		execTimePercent := ""
		if totalExecTime > 0 {
