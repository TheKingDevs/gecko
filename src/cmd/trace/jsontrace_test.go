...
func TestJSONTraceHandler(t *testing.T) {
	...
	for _, testPath := range testPaths {
		t.Run(filepath.Base(testPath), func(t *testing.T) {
			...
			// TODO(mknyszek): Check that there's one at most task per proc at any given time.
			checkExecutionTimes(t, data)
			checkPlausibleHeapMetrics(t, data)
			// TODO(mknyszek): Check for plausible thread and task metrics.
...
// parseGoroutineName returns the task name from the event's name field.
...
func checkNetworkUnblock(t *testing.T, data format.Data) {
	...
	// TODO(mknyszek): Check for the flow of this event to some slice event of a task running.
}
