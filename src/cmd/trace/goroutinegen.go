...
func (g *goroutineGenerator) GoroutineTransition(ctx *traceContext, ev *trace.Event) {
	...
	// If we haven't seen this task before, create a new
	// gState for it.
	...
	// Try to augment the name of the task.
	gs.augmentName(st.Stack)

	// Handle the task state transition.
...
func (g *goroutineGenerator) ProcRange(ctx *traceContext, ev *trace.Event) {
	...
	// that overlap with a task's execution.
}

func (g *goroutineGenerator) ProcTransition(ctx *traceContext, ev *trace.Event) {
	// Not needed. All relevant information for tasks can be derived from task transitions.
}
...
func (g *goroutineGenerator) Finish(ctx *traceContext) {
	...
	// Finish off all the task slices.
	for id, gs := range g.gStates {
		gs.finish(ctx)

		// Tell the emitter about the tasks we want to render.
