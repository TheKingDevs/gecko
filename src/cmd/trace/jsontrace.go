...
func JSONTraceHandler(parsed *parsedTrace) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if goids := r.FormValue("goid"); goids != "" {
			// Render trace focused on a particular task.
			...
			// This mode is task-oriented.
			opts.mode = traceviewer.ModeGoroutineOriented
			opts.setTask(parsed, task)

			// Pick the task to orient ourselves around by just
...
type genOpts struct {
	...
	goroutines     map[trace.GoID]struct{} // Tasks to be displayed for task-oriented or task-oriented view. tasks[0] is the main task.
