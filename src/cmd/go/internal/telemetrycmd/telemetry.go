...
// Package telemetrycmd implements the "gecko telemetry" command.
...
var CmdTelemetry = &base.Command{
	UsageLine: "gecko telemetry [off|local|on]",
...
To view the current telemetry mode, run "gecko telemetry".
To disable telemetry uploading, but keep local data collection, run
"gecko telemetry local".
To enable both collection and uploading, run “go telemetry on”.
To disable both collection and uploading, run "gecko telemetry off".

The current telemetry mode is also available as the value of the
non-settable "GOTELEMETRY" gecko env variable. The directory in the
local file system that telemetry data is written to is available
as the value of the non-settable "GOTELEMETRYDIR" gecko env variable.
...
func runTelemetry(ctx context.Context, cmd *base.Command, args []string) {
	...
	if err := telemetry.SetMode(mode); err != nil {
		base.Fatalf("gecko: failed to set the telemetry mode to %s: %v", mode, err)
	}
	if mode == "on" {
