...
type scanner struct {
	...
	gk     bool // gecko syntax: recognize the while, class, new, and export keywords
...
func (s *scanner) ident() {
	...
	if len(lit) >= 2 {
		if tok := keywordMap[hash(lit)]; tok != 0 && tokStrFast(tok) == string(lit) {
			if tok == _While && !s.gk {
				// 'while' is a keyword only for gecko source files.
				s.nlsemi = true
				s.lit = string(lit)
				s.tok = _Name
				return
		...
		// 'class' and 'new' are keywords only for gecko source files.
		if s.gk {
			switch string(lit) {
			case "class":
				s.nlsemi = false
