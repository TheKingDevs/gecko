...
type (
	...
	//              Path
	// LocalPkgName Path
	// The gecko TypeScript-style forms (only in .gk files):
	//   LocalPkgName "from" Path           (TS default import)
	//   "{" TSMembers "}" "from" Path      (TS named imports)
	//   LocalPkgName "," "{" TSMembers "}" "from" Path  (TS mixed import)
	ImportDecl struct {
	...
	// TsMember is a single named member of a gecko TypeScript-style
	// import: either a plain member "a" (Qualifier == nil) or a
	// qualified member "x.y" (Qualifier == x, Name == y).
	TsMember struct {
		Qualifier *Name // nil for plain members
		Name      *Name
		node
	}
	...
	ConstDecl struct {
		...
		Export   bool // gecko: exported via the export keyword
		decl
	}
	...
	FuncDecl struct {
		...
		Export     bool       // gecko: exported via the export keyword
		// GeckoSynthCtor marks a `constructor` synthesized by the parser to
	...
	// export { X, Y } / export X
	// exports the package-scope names X and Y.
	ExportDecl struct {
		NameList []*Name
		decl
	}
...
type (
	...
	// new Type(ArgList[0], ArgList[1], ...)
	// gecko: instantiates a class, running its constructor.
	NewExpr struct {
		Type    Expr
		ArgList []Expr
		HasDots bool // last argument is followed by ...
	...
	// class Name { MethodList[0]; MethodList[1]; ... }
	// gecko: the underlying type of a class is a struct whose fields are
	// inferred from `this.<field>` references inside the class methods.
	// Methods become regular FuncDecls with an implicit `this` receiver.
	// Fields/Consts hold `var x = v` / `const X = v` members declared in
