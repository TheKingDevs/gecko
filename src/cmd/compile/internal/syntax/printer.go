...
type printer struct {
	...
	classInits []Stmt // parsed class field initializers (for constructor body pruning)
}
...
func (p *printer) printRawNode(n Node) {
	...
	case *TemplateLit:
		p.print(_Name, "`") // _Name requires actual value following immediately
		for _, part := range n.Parts {
			if s, ok := part.(string); ok {
				p.print(_Name, s)
			} else {
				p.printNode(part.(Expr))
			}
		}
		p.print(_Name, "`")
	...
	case *NewExpr:
		p.print(_New, blank, n.Type, _Lparen)
		p.printExprList(n.ArgList)
		if n.HasDots {
			p.print(_DotDotDot)
		}
	...
	case *ClassType:
		...
		sort.SliceStable(members, func(i, j int) bool {
			pi, pj := members[i].Pos(), members[j].Pos()
			return pi.Line() < pj.Line() || (pi.Line() == pj.Line() && pi.Col() < pj.Col())
		})
		if len(members) > 0 {
			if p.linebreaks {
				p.print(newline, indent)
	...
	case *ImportDecl:
		if n.TsFrom {
			// gecko TypeScript-style import
			if n.Group == nil {
				p.print(_Import, blank)
			}
			if n.LocalPkgName != nil {
				if len(n.TSMembers) > 0 {
					p.print(n.LocalPkgName, blank, _Comma, blank)
				} else {
					p.print(n.LocalPkgName, blank)
				}
			}
			if len(n.TSMembers) > 0 {
				p.print(_Lbrace, blank)
				for i, m := range n.TSMembers {
					if i > 0 {
						p.print(_Comma, blank)
					}
					if m.Qualifier != nil {
						p.print(m.Qualifier, _Dot)
					}
					p.print(m.Name)
				}
				p.print(blank, _Rbrace, blank)
			}
			p.print(NewName(Pos{}, "from"), blank)
			p.print(n.Path)
			break
		}
		if n.Group == nil {
	...
	case *TypeDecl:
		...
		if _, ok := n.Type.(*ClassType); ok {
			// gecko class declaration, e.g. "class Greeter() { ... }"
			p.print(n.Name)
			if n.TParamList != nil {
	...
	case *FuncDecl:
		...
		if body := p.classMethodBodyPruned(n); body != nil {
			p.print(blank, body)
		}
...
// should be printed.
func (p *printer) classMethodBodyPruned(n *FuncDecl) *BlockStmt {
	if n.Body == nil || len(p.classInits) == 0 {
		return n.Body
	}
	if n.Name == nil || n.Name.Value != "constructor" {
		return n.Body
	}
	k := 0
	for k < len(n.Body.List) && k < len(p.classInits) && n.Body.List[k] == p.classInits[k] {
		k++
	}
	if k == 0 {
		return n.Body
