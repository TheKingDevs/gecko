...
func walkPrint(nn *ir.CallExpr, init *ir.Nodes) ir.Node {
	...
	calls := []ir.Node{
		mkcall("printlock", nil, init),
		mkcall("geckoprintbegin", nil, init),
	}
	...
	calls = append(calls,
		mkcall("geckoprintend", nil, init),
		mkcall("printunlock", nil, init))
...
// walkInput walks an OINPUT node, rewriting input(...) into a call to
// runtime.geckoinput. The formatted prompt is written to stdout and a line
// is read from stdin.
func walkInput(nn *ir.CallExpr, init *ir.Nodes) ir.Node {
	init.Append(ir.TakeInit(nn)...)
	walkExprListCheap(nn.Args, init)
	call := ir.NewCallExpr(nn.Pos(), ir.OCALL, typecheck.LookupRuntime("geckoinput"), nn.Args)
	call = typecheck.Expr(call).(*ir.CallExpr)
	call.SetType(nn.Type())
