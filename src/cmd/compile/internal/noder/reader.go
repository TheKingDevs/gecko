...
// geckoExported reports the gecko `export` visibility override recorded
// for the object with the given index, if any.
func (pr *pkgReader) geckoExported(idx index) (exported bool, hasOverride bool) {
	r := pr.newReader(pkgbits.SectionObj, idx, pkgbits.SyncObject1)
	if !r.Version().Has(pkgbits.GeckoExport) {
		return false, false
	}
	if !r.Bool() {
		return false, false
...
func (r *reader) doPkg() *types.Pkg {
	...
	if r.Version().Has(pkgbits.GeckoExport) {
		r.Bool() // gecko flag; visibility is enforced in types2
	}
...
func (pr *pkgReader) objIdxMayFail(idx index, implicits, explicits []*types.Type, shaped bool) (ir.Node, error) {
	...
	// gecko: o override de visibilidade do `export` não interessa ao
	// noder (a checagem acontece no types2), apenas é consumido.
	if r.Version().Has(pkgbits.GeckoExport) {
		if r.Bool() {
			r.Bool()
		}
...
// geckoNullSafeChain rebuilds a null-safe print chain. tmp is an
// interface{}-typed temp initialized to nil; if every guard (checked base
// first) is non-nil then tmp is assigned final. It returns the temp to
// print and the initialization statements that must run before it is used.
func (r *reader) geckoNullSafeChain(pos src.XPos, guards []ir.Node, final ir.Node) (ir.Node, ir.Nodes) {
	tmp := typecheck.TempAt(pos, r.curfn, final.Type())

	init := ir.Nodes{ir.NewDecl(pos, ir.ODCL, tmp)}
	init.Append(typecheck.Stmt(ir.NewAssignStmt(pos, tmp, typecheck.Expr(ir.NewZero(pos, tmp.Type())))))

	body := ir.Nodes{typecheck.Stmt(ir.NewAssignStmt(pos, tmp, final))}
	for i := len(guards) - 1; i >= 0; i-- {
		guard := guards[i]
		cond := typecheck.Expr(ir.NewBinaryExpr(pos, ir.ONE, guard, ir.NewNilExpr(pos, guard.Type())))
		body = ir.Nodes{typecheck.Stmt(ir.NewIfStmt(pos, cond, body, nil))}
	}
	init.Append(body...)
	return tmp, init
}
...
func (r *reader) expr() (res ir.Node) {
	...
	case exprPrintTemplate:
		...
		for i := 0; i < n; i++ {
			if r.Bool() {
				str := r.String()
				args.Append(typecheck.Expr(ir.NewString(pos, str)))
				continue
			}
			if !r.Bool() { // plain expression
				args.Append(typecheck.Expr(r.expr()))
				continue
			}
			// Null-safe chain part: flush pending arguments, then print the
			// guarded temp through a runtime geckoprint call so that a nil
			// result renders as "null" (an OPRINT argument of interface type
			// would be rendered as a printeface "(type, data)" pair instead).
			if len(args) > 0 {
				flush := make(ir.Nodes, len(args))
				copy(flush, args)
				stmts = append(stmts, typecheck.Stmt(ir.NewCallExpr(pos, ir.OPRINT, nil, flush)))
				args = args[:0]
			}
			guards := make([]ir.Node, 0, r.Len())
			for j := 0; j < cap(guards); j++ {
				guards = append(guards, typecheck.Expr(r.expr()))
			}
			final := typecheck.Expr(r.expr())
			tmp, init := r.geckoNullSafeChain(pos, guards, final)
			call := ir.NewCallExpr(pos, ir.OCALL, typecheck.LookupRuntime("geckoprint"), []ir.Node{tmp})
			call.PtrInit().Append(init...)
			stmts = append(stmts, typecheck.Stmt(call))
		}
		if isprintln {
			args = append(args, typecheck.Expr(ir.NewString(pos, "\n")))
		}
		if len(stmts) > 0 {
			if len(args) > 0 {
				stmts = append(stmts, typecheck.Stmt(ir.NewCallExpr(pos, ir.OPRINT, nil, args)))
			}
			return typecheck.Stmt(block(stmts))
		}
		call := ir.NewCallExpr(pos, ir.OPRINT, nil, args)
	...
	case exprGeckoPrint:
		...
		isprintln := r.Bool()
		n := r.Len()
		args := make(ir.Nodes, 0, n)
		var init ir.Nodes
		for i := 0; i < n; i++ {
			if r.Bool() { // null-safe chain
				guards := make([]ir.Node, 0, r.Len())
				for j := 0; j < cap(guards); j++ {
					guards = append(guards, typecheck.Expr(r.expr()))
				}
				final := typecheck.Expr(r.expr())
				tmp, tmpInit := r.geckoNullSafeChain(pos, guards, final)
				init.Append(tmpInit...)
				args.Append(tmp)
			} else {
				args.Append(typecheck.Expr(r.expr()))
		...
		sym := typecheck.LookupRuntime("geckoprint")
		if isprintln {
			sym = typecheck.LookupRuntime("geckoprintln")
		}
		call := ir.NewCallExpr(pos, ir.OCALL, sym, args)
		call.PtrInit().Append(init...)
		return typecheck.Stmt(call)
	...
	case exprTemplate:
		...
		var init ir.Nodes
		for i := 0; i < n; i++ {
			if r.Bool() { // literal string segment
				args.Append(typecheck.Expr(ir.NewString(pos, r.String())))
				continue
			}
			if r.Bool() { // null-safe chain
				guards := make([]ir.Node, 0, r.Len())
		...
		call := ir.NewCallExpr(pos, ir.OCALL, typecheck.LookupRuntime("geckotemplate"), args)
		call.PtrInit().Append(init...)
	...
	case exprNewClass:
		pos := r.pos()
		classTyp := r.typ()
		ptr := types.NewPtr(classTyp)

		if !r.Bool() { // no constructor
			return typecheck.Expr(ir.NewUnaryExpr(pos, ir.ONEW, ir.TypeNode(classTyp)))
		}

		tmp := typecheck.TempAt(pos, r.curfn, ptr)
		init := ir.Nodes{ir.NewDecl(pos, ir.ODCL, tmp)}

		unew := typecheck.Expr(ir.NewUnaryExpr(pos, ir.ONEW, ir.TypeNode(classTyp)))
		init.Append(typecheck.Stmt(ir.NewAssignStmt(pos, tmp, unew)))

		_, method, dictPtr := r.methodExpr()
		args := []ir.Node{tmp}
		if dictPtr != nil {
			args = append(args, dictPtr)
		}
		args = append(args, r.multiExpr()...)
		dots := r.Bool()
		call := typecheck.Call(pos, method, args, dots)
		init.Append(typecheck.Stmt(call))
...
func (r *reader) multiExpr() []ir.Node {
	...
	if r.Bool() { // N:1
		...
		for i := range results {
			...
			res := ir.Node(tmp)
			if r.Bool() { // some conversion/fix
				if r.Bool() { // gecko pointer fix
					if r.Bool() { // address-of
						res = typecheck.Expr(typecheck.NodAddrAt(pos, res))
					} else { // dereference
						res = typecheck.Expr(ir.NewStarExpr(pos, res))
					}
