...
func LoadPackage(filenames []string) {
	...
	// Move the entire syntax processing logic into a separate task to avoid blocking on the "sem".
	go func() {
...
// Note that cmd/gecko ignores files whose names start with underscore,
// so the only _cgo_ files we will see from cmd/gecko are generated by cgo.
// It's easy to bypass this check by calling the compiler directly;
// we only protect against uses by cmd/gecko.
...
// This is copied from SafeArg in cmd/gecko/internal/load/pkg.go.
