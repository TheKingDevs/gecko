		...
		case types2.UntypedInt, types2.UntypedFloat, types2.UntypedComplex:
			...
			if tv.Value != nil {
				...
				if s.Kind() != constant.Int {
					// The value is not an exact integer (e.g. 2.5 or 1+2i),
					// which happens for untyped literals in gecko template
					// strings. Use the Go default type for the constant.
					switch basic.Kind() {
					case types2.UntypedFloat:
						typ = types2.Typ[types2.Float64]
					case types2.UntypedComplex:
						typ = types2.Typ[types2.Complex128]
					}
				} else if constant.Sign(s) < 0 {
