...
// writeGeckoExport writes the gecko `export` keyword visibility
// override for obj, if any.
func (w *writer) writeGeckoExport(obj types2.Object) {
	if ov := obj.GeckoExported(); ov != nil {
		w.Bool(true)
		w.Bool(*ov)
		return
	}
...
func (pw *pkgWriter) pkgIdx(pkg *types2.Package) index {
	...
	default:
		...
		if w.Version().Has(pkgbits.GeckoExport) {
			w.Bool(pkg.IsGecko())
		}
...
func (w *writer) doObj(wext *writer, obj types2.Object) pkgbits.CodeObj {
	...
	// gecko: persiste o override de visibilidade do `export`.
	w.writeGeckoExport(obj)
...
func (w *writer) stmt1(stmt syntax.Stmt) {
	...
	case *syntax.AssignStmt:
		...
		case stmt.Op != 0 && stmt.Op != syntax.Def:
			...
			if w.geckoFile(stmt) && typ != nil && stmt.Op != syntax.Shl && stmt.Op != syntax.Shr {
				if ct := types2.GeckoBinaryCommonType(typ, w.p.typeOf(stmt.Rhs), stmt.Op); ct != nil {
					w.geckoConvert(ct, stmt.Rhs)
					break
				}
			}
	...
	if rang, ok := stmt.Init.(*syntax.RangeClause); w.Bool(ok) {
		...
		}
		{
			...
			assign := func(i int, src types2.Type) {
				...
				if name, ok := dst.(*syntax.Name); ok {
					// Names introduced by `:=`, or by a gecko `=` declare-or-assign
					// in a range clause, only appear in Defs, not in Types.
					if def, ok := w.p.info.Defs[name].(*types2.Var); ok {
						dstType = def.Type()
					} else {
...
func (w *writer) expr(expr syntax.Expr) {
	...
	case *syntax.TemplateLit:
		// A template string outside print/println lowers to a runtime
		// geckotemplate call whose arguments are the flattened literal
		// segments and interpolations (converted to interface{}), mirroring
		// the part encoding of exprPrintTemplate.
		w.Code(exprTemplate)
		w.pos(expr)
		...
		for _, part := range expr.Parts {
			...
			e := part.(syntax.Expr)
			w.Bool(false)
			if guards, _, ok := w.geckoNullChain(e); ok {
				w.Bool(true)
				w.writeGeckoChain(guards, e)
			} else {
				w.Bool(false)
				w.expr(e)
			}
	...
	case *syntax.NewExpr:
		classTyp := types2.Unalias(w.p.typeAndValue(expr.Type).Type)
		named, ok := classTyp.(*types2.Named)
		assert(ok)
		...
		ctorObj, _, _ := types2.LookupFieldOrMethod(named, true, w.p.curpkg, "constructor")
		w.Bool(ctorObj != nil)
		if ctorObj == nil {
			return
		}

		ctor := ctorObj.(*types2.Func)
		sig := ctor.Type().(*types2.Signature)

		w.typ(sig.Recv().Type())
		w.Bool(false) // not a generic method
		w.typ(sig)
		w.pos(expr)
		w.selector(ctor)

		w.Bool(false) // not a method on a type parameter
		w.Bool(false) // no dynamic subdictionary
		w.Bool(false) // no static dictionary

		paramTypes := sig.Params()
		w.multiExpr(expr, func(i int) types2.Type {
	...
	case *syntax.Operation:
		...
		if expr.Op != syntax.Shl && expr.Op != syntax.Shr {
			...
			if w.geckoFile(expr) {
				if ct := types2.GeckoBinaryCommonType(xtyp, ytyp, expr.Op); ct != nil {
					commonType = ct
					geckoForce = true
				}
			}
		...
		w.op(binOps[expr.Op])
		if geckoForce {
			w.geckoConvert(commonType, expr.X)
		} else {
			w.implicitConvExpr(commonType, expr.X)
		}
		w.pos(expr)
		if geckoForce {
			w.geckoConvert(commonType, expr.Y)
		} else {
	...
	case *syntax.CallExpr:
		...
		if tv.IsBuiltin() {
			obj, _ := lookupObj(w.p, syntax.Unparen(expr.Fun))
			if obj.Name() == "print" || obj.Name() == "println" {
				isprintln := obj.Name() == "println"
				if w.geckoFile(expr) && (w.printTemplate(expr, isprintln, expr.ArgList) || w.geckoDynPrint(expr, isprintln, expr.ArgList)) {
					return
				}
			}
			switch obj.Name() {
...
// geckoFile reports whether the expression belongs to a gecko (.gk)
// source file.
func (w *writer) geckoFile(p poser) bool {
	base := p.Pos().FileBase()
	return base != nil && strings.HasSuffix(base.Filename(), ".gk")
}

// geckoConvert writes expr coerced to dst. Unlike implicitConvExpr,
// gecko's implicit byte conversions (byte→int, byte→string taken as a
// character) are not Go-assignable conversions, so the conversion is
// forced rather than asserted assignable.
func (w *writer) geckoConvert(dst types2.Type, expr syntax.Expr) {
	if dst == nil || types2.Identical(w.p.typeOf(expr), dst) {
		w.expr(expr)
		return
	}
	w.convertExpr(dst, expr, false)
}

// geckoNullChain reports whether e is a null-safe chain expression: a
// chain of field selections, ending optionally in a single method call,
// whose receivers (base first) may need nil checks. It returns the base-
// first guard expressions to check non-nil and the expression to evaluate
// once every guard passes. If e is also a nullable (pointer- or interface-
// typed) pure field selection, it is appended as an innermost guard so
// that a nil result prints as "null" too; a method call is never re-
// evaluated as a guard, since it may have side effects.
func (w *writer) geckoNullChain(e syntax.Expr) (guards []syntax.Expr, final syntax.Expr, ok bool) {
	var steps []syntax.Expr // receivers, outermost first
	seenCall := false
	for cur := e; ; {
		switch n := syntax.Unparen(cur).(type) {
		case *syntax.SelectorExpr:
			steps = append(steps, n.X)
			cur = n.X
		case *syntax.CallExpr:
			if seenCall {
				return nil, nil, false
			}
			sel, isSel := syntax.Unparen(n.Fun).(*syntax.SelectorExpr)
			if !isSel {
				return nil, nil, false // plain function call, not a chain
			}
			steps = append(steps, sel.X)
			cur = sel.X
			seenCall = true
		case *syntax.Name:
			tv, ok := w.p.maybeTypeAndValue(cur)
			if !ok || !tv.IsValue() {
				return nil, nil, false // e.g. a package-qualified name
			}
			if len(steps) == 0 {
				return nil, nil, false // bare identifier, not a chain
			}
			for i := len(steps) - 1; i >= 0; i-- { // base first
				if isGeckoChainType(w.p.typeOf(steps[i])) {
					guards = append(guards, steps[i])
				}
			}
			if len(guards) == 0 {
				return nil, nil, false
			}
			if !seenCall && isGeckoChainType(w.p.typeOf(e)) {
				guards = append(guards, e)
			}
			return guards, e, true

		case *syntax.IndexExpr:
			// Terminal base such as "tasks[i]" in "tasks[i].next.label()".
			if len(steps) == 0 {
				return nil, nil, false // bare index expression, not a chain
			}
			for i := len(steps) - 1; i >= 0; i-- { // base first
				if isGeckoChainType(w.p.typeOf(steps[i])) {
					guards = append(guards, steps[i])
...
// isGeckoChainType reports whether a chain receiver of this type can be
// null and therefore needs a nil check before being dereferenced.
func isGeckoChainType(t types2.Type) bool {
	switch types2.Unalias(t).(type) {
	case *types2.Pointer, *types2.Interface:
		return true
	}
	return false
}

// writeGeckoChain writes the guard list of a null-safe chain followed by
// the chain expression itself (converted to interface{}).
func (w *writer) writeGeckoChain(guards []syntax.Expr, expr syntax.Expr) {
	w.Len(len(guards))
	for _, g := range guards {
		w.expr(g)
	}
	w.implicitConvExpr(anyTypeName.Type(), expr)
}

// printTemplate flattens the arguments of a gecko print/println call
// that contains a template string into a sequence of literal string
// segments and sub-expressions, and writes it with the
// exprPrintTemplate code. It reports whether any argument was a
// template string.
func (w *writer) printTemplate(pos poser, isprintln bool, args []syntax.Expr) bool {
	var parts []any
	template := false
	for i, arg := range args {
		...
		if tpl, ok := syntax.Unparen(arg).(*syntax.TemplateLit); ok {
			template = true
			parts = append(parts, tpl.Parts...)
		} else {
	...
	for _, part := range parts {
		if s, ok := part.(string); ok {
			...
			expr := part.(syntax.Expr)
			if guards, _, ok := w.geckoNullChain(expr); ok {
				w.Bool(false)
				w.Bool(true)
				w.writeGeckoChain(guards, expr)
			} else {
				w.Bool(false)
				w.Bool(false)
				w.expr(expr)
...
// geckoDynPrint writes an exprGeckoPrint code for a print/println call that
// has no template strings but at least one argument whose type is a slice or
// a map (both of which the Go println builtin would render raw, e.g.
// "[3/3]0x..."), or that is a null-safe chain expression (which must print
// "null" instead of panicking on a nil receiver). Every argument is converted
// to interface{} and printed by the geckoprint/geckoprintln runtime helpers.
// It reports whether it handled the call.
func (w *writer) geckoDynPrint(pos poser, isprintln bool, args []syntax.Expr) bool {
	chains := make([][]syntax.Expr, len(args))
	ok := false
	for i, arg := range args {
		if t := w.p.typeOf(arg); t != nil {
			switch types2.CoreType(t).(type) {
			case *types2.Slice, *types2.Map:
				ok = true
			}
		}
		if guards, _, chainOk := w.geckoNullChain(arg); chainOk {
			chains[i] = guards
			ok = true
		}
	}
	...
	w.Code(exprGeckoPrint)
	w.pos(pos)
	w.Bool(isprintln)
	w.Len(len(args))
	for i, arg := range args {
		if guards := chains[i]; guards != nil {
			w.Bool(true)
			w.writeGeckoChain(guards, arg)
		} else {
			w.Bool(false)
			w.implicitConvExpr(anyTypeName.Type(), arg)
		}
	}
	return true
}
...
func (w *writer) implicitConvExpr(dst types2.Type, expr syntax.Expr) {
	// Gecko: implicit address-of and dereference of pointers. types2
	// accepts a value of type E where *E is expected (the value must be
	// addressable), and a value of type *E where E is expected, so mirror
	// that by wrapping the expression in an implicit & or * node.
	if dst != nil && w.geckoFile(expr) {
		src := w.p.typeOf(expr)
		if p, _ := types2.Unalias(dst).(*types2.Pointer); p != nil && types2.Identical(types2.Unalias(p.Elem()), src) {
			w.Code(exprUnaryOp)
			w.op(ir.OADDR)
...
func (w *writer) pkgDecl(decl syntax.Decl) {
	...
	case *syntax.ExportDecl:
		// gecko: export lists only affect types2 visibility.
