...
func (r *reader) doPkg() *types2.Package {
	...
	if r.Version().Has(pkgbits.GeckoExport) && r.Bool() {
		// gecko package: fields and methods are visible to importers
...
func (pr *pkgReader) objIdx(idx pkgbits.Index) (*types2.Package, string) {
	...
	objPkg.Scope().InsertLazy(objName, func() types2.Object {
		...
		// gecko: records the `export` visibility override for this object.
		geckoExported, hasGeckoExport := func() (bool, bool) {
		...
		applyGeckoExport := func(obj types2.Object) types2.Object {
			if hasGeckoExport {
				obj.SetGeckoExported(geckoExported)
			}
			return obj
		}
		...
		case pkgbits.ObjAlias:
			...
			typ := r.typ()
			return applyGeckoExport(newAliasTypeName(pr.enableAlias, pos, objPkg, objName, typ, tparams))

		case pkgbits.ObjConst:
			pos := r.pos()
			typ := r.typ()
			val := r.Value()
			return applyGeckoExport(types2.NewConst(pos, objPkg, objName, typ, val))
		...
		case pkgbits.ObjFunc:
			...
			return applyGeckoExport(types2.NewFunc(pos, objPkg, objName, sig))

		case pkgbits.ObjType:
			pos := r.pos()

			return applyGeckoExport(types2.NewTypeNameLazy(pos, objPkg, objName, func(_ *types2.Named) ([]*types2.TypeParam, types2.Type, []*types2.Func, []func()) {
				...
				if r.Version().Has(pkgbits.GenericMethods) {
					for range r.Len() {
						...
						// gecko: generic methods also carry the export override prefix.
						if t.Version().Has(pkgbits.GeckoExport) && t.Bool() {
							t.Bool()
						}
		...
		case pkgbits.ObjVar:
			pos := r.pos()
			typ := r.typ()
			return applyGeckoExport(types2.NewVar(pos, objPkg, objName, typ))
		}
	})
