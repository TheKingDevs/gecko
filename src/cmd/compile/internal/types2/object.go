...
type Object interface {
	...
	Exported() bool  // reports whether the object is exported (see object.Exported)
	...
	// SetGeckoExported sets the gecko `export` keyword visibility
	// override for the object.
	SetGeckoExported(bool)

	// GeckoExported reports the gecko export visibility override, or
	// nil if none is set.
	GeckoExported() *bool
}
...
type object struct {
	...
	// geckoExported overrides the capital-letter visibility rule for
	// objects annotated with the gecko `export` keyword. nil means no
	// override.
	geckoExported *bool
}
...
// Exported reports whether the object is exported outside of its
// package. In Go, an object is exported if its name starts with a
// capital letter. In gecko (.gk) source files, package-level
// declarations are exported only if annotated with the `export`
// keyword, and struct fields and methods of gecko packages are
// exported regardless of capitalization. The result doesn't take
// into account whether the object is in a local (function) scope
// or not.
...
// SetGeckoExported sets the gecko export override for this object.
func (obj *object) SetGeckoExported(b bool) { obj.geckoExported = &b }

// GeckoExported reports the gecko export override for this object,
// or nil if no override is set.
func (obj *object) GeckoExported() *bool { return obj.geckoExported }
...
func NewPkgName(pos syntax.Pos, pkg *Package, name string, imported *Package) *PkgName {
	return &PkgName{object{nil, pos, pkg, name, Typ[Invalid], 0, nopos, nil}, imported}
}
...
func NewConst(pos syntax.Pos, pkg *Package, name string, typ Type, val constant.Value) *Const {
	return &Const{object{nil, pos, pkg, name, typ, 0, nopos, nil}, val}
}
...
func NewTypeName(pos syntax.Pos, pkg *Package, name string, typ Type) *TypeName {
	return &TypeName{object{nil, pos, pkg, name, typ, 0, nopos, nil}}
}
...
func newVar(kind VarKind, pos syntax.Pos, pkg *Package, name string, typ Type) *Var {
	return &Var{object: object{nil, pos, pkg, name, typ, 0, nopos, nil}, kind: kind}
}
...
func NewFunc(pos syntax.Pos, pkg *Package, name string, sig *Signature) *Func {
	...
	return &Func{object{nil, pos, pkg, name, typ, 0, nopos, nil}, nil, false, false}
}
