...
func (x *operand) assignableTo(check *Checker, T Type, cause *string) (bool, Code) {
	...
	// Gecko: a value of type E is assignable to a variable of type *E
	// (implicit address-of), and a value of type *E is assignable to a
	// variable of type E (implicit dereference). Explicit & and * remain
	// supported, but gecko programs do not require them. Address-of is
	// only permitted for addressable (variable) values.
	if check != nil && x.expr != nil && check.geckoFile(x.expr.Pos()) && !check.conf.GeckoTyped {
		if tp, _ := T.(*Pointer); tp != nil {
			if Identical(Unalias(tp.Elem()), V) {
				if x.mode() != variable {
					if cause != nil {
						*cause = "cannot take the address of a non-addressable value"
					}
		...
		if vp, _ := V.(*Pointer); vp != nil {
			if Identical(Unalias(vp.Elem()), T) {
				return true, 0
			}
		}
	}
