...
func (check *Checker) collectParams(kind VarKind, list []*syntax.Field) (names []*syntax.Name, params []*Var, variadic bool) {
	...
	for i, field := range list {
		ftype := field.Type
		if ftype == nil {
			// gecko: class method parameters without an explicit type
			// default to `any`. The Go parser never produces nil param
			// types, so this only happens for desugared class methods.
			typ = universeAny.Type()
			ftype = prev // nil param types never vary; skip the grouped logic
		}
