...
func (check *Checker) shortVarDecl(pos poser, lhs, rhs []syntax.Expr) {
	check.declareVars(pos, lhs, rhs, false)
}

// geckoVarDecl is like shortVarDecl but resolves already visible names
// across the whole lexical chain instead of only the innermost block.
// It implements the gecko `x = e` declare-or-assign rule: the name is
// assigned to if it is visible anywhere in the environment and only
// declared in the current block otherwise.
func (check *Checker) geckoVarDecl(pos poser, lhs, rhs []syntax.Expr) {
	check.declareVars(pos, lhs, rhs, true)
}

func (check *Checker) declareVars(pos poser, lhs, rhs []syntax.Expr, chain bool) {
	...
	for i, lhs := range lhs {
		...
		var alt Object
		if chain {
			// gecko: an existing name anywhere in the visible environment is
			// an assignment target, not a shadowed redeclaration.
			alt = check.lookup(name)
		} else {
