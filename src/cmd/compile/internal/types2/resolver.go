...
func (check *Checker) collectObjects() {
	...
	// geckoExports collects export { x, y } / export x declarations so
	// they can be applied after all package-level declarations have been
	// collected, regardless of where they appear in the files.
	var geckoExports []*syntax.ExportDecl
	...
	for fileNo, file := range check.files {
		...
		for index, decl := range file.DeclList {
			...
			case *syntax.ImportDecl:
				...
				if s.TsFrom {
					// gecko TypeScript-style import (import x from "y",
					// import { a, x.y } from "b", import x, { a } from "b").
					check.geckoTSImport(s, fileScope, imp, pkgImports, pkg)
					continue
				}
				...
				if name == "." {
					...
					for name, obj := range imp.scope.elems {
						...
						// do not import them!
						if isExported(name) || resolve(name, obj).Exported() {
			...
			case *syntax.ConstDecl:
				...
				for i, name := range s.NameList {
					...
					if check.geckoFile(name.Pos()) {
						obj.SetGeckoExported(s.Export)
					}
				}
			...
			case *syntax.ExportDecl:
				// gecko: export { x, y } / export x. The named package-scope
				// objects are marked as exported. Processing happens after all
				// declarations have been collected, so the export list may
				// appear anywhere in the file.
				geckoExports = append(geckoExports, s)
	...
	// gecko: apply export { x, y } / export x annotations.
	for _, d := range geckoExports {
		for _, name := range d.NameList {
			obj := pkg.scope.Lookup(name.Value)
			if obj == nil {
				check.errorf(name, UndeclaredName, "cannot export undefined name %s", name.Value)
				continue
			}
			obj.SetGeckoExported(true)
		}
	}
