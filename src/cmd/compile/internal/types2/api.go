...
type Config struct {
	...
	// GeckoTyped enables gecko's strict ("typed") type checking mode for
	// .gk files: the gecko dynamic-typing features — type inference for
	// type-less composite literals, `x = e` declare-or-assign, and the
	// implicit pointer address-of/dereference conversions — are rejected,
	// forcing explicit type annotations and explicit & and * operators (a
	// "dynamic" project keeps them). It has no effect on .go files.
	GeckoTyped bool
...
func (info *Info) PkgNameOf(imp *syntax.ImportDecl) *PkgName {
	if imp.LocalPkgName != nil {
		if pkgname, _ := info.Defs[imp.LocalPkgName].(*PkgName); pkgname != nil {
			return pkgname
		}
		// Gecko: a default import may bind an exported symbol instead of the
		// package itself; fall back to the implicit package name recorded on
		// the import declaration.
		if pkgname, _ := info.Implicits[imp].(*PkgName); pkgname != nil {
			return pkgname
		}
		return nil
	}
