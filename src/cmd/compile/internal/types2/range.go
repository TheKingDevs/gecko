...
func (check *Checker) rangeStmt(inner stmtContext, rangeStmt *syntax.ForStmt, noNewVarPos poser, sKey, sValue, sExtra, rangeVar syntax.Expr, isDef bool) {
	...
	if isDef {
		...
		// gecko: `for (i = range x)` assigns to an existing i but declares
		// it when it is not visible anywhere in the environment.
		if check.geckoDynamicFile(sKey.Pos()) && check.geckoNewVar([]syntax.Expr{sKey, sValue}) {
			check.geckoRangeVars(rangeStmt, noNewVarPos, sKey, sValue, rangeOverInt, &x, key, val)
		} else {
			// ordinary assignment
			for i, lhs := range lhs {
				if lhs == nil {
...
// geckoRangeVars implements the gecko declare-or-assign rule for the
// iteration variables of a `for (i = range x)` loop: an iteration
// variable that is visible anywhere in the environment is assigned to,
// and a new one is declared in the loop's body scope.
func (check *Checker) geckoRangeVars(rangeStmt *syntax.ForStmt, noNewVarPos poser, sKey, sValue syntax.Expr, rangeOverInt bool, x *operand, key, val Type) {
	...
	for i, l := range lhs {
		...
		if ident, ok := syntax.Unparen(l).(*syntax.Name); ok && ident.Value != "_" && check.lookup(ident.Value) == nil {
			// declare a new iteration variable
			obj := newVar(LocalVar, ident.Pos(), check.pkg, ident.Value, nil)
			check.recordDef(ident, obj)
			vars = append(vars, obj)
			assert(obj.typ == nil)
			if rangeOverInt {
				...
				var y operand
				y.mode_ = value
				y.expr = l // we don't have a better rhs expression to use here
				y.typ_ = typ
		...
		// ordinary assignment to an existing iteration variable
		if rangeOverInt {
			assert(i == 0) // at most one iteration variable
			check.assignVar(l, nil, x, "range clause")
			if x.isValid() && !isInteger(x.typ()) {
				check.softErrorf(l, InvalidRangeExpr, "cannot use iteration variable of type %s", x.typ())
			}
		} else {
			var y operand
			y.mode_ = value
			y.expr = l // we don't have a better rhs expression to use here
			y.typ_ = typ
			check.assignVar(l, nil, &y, "assignment")
		}
	}
