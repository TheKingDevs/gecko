...
type Checker struct {
	...
	*Info
	...
	geckoClass map[*syntax.ClassType]*Struct
	// geckoConstFields maps a class's underlying struct to its read-only
	// (const) field names; geckoConstRhs records the initializer RHS
	// expressions that are allowed to write those fields (the parser emits
	// them as the first constructor assignments).
	geckoConstFields map[*Struct]map[string]bool
	geckoConstRhs    map[syntax.Expr]bool
...
func (check *Checker) initFiles(files []*syntax.File) {
	...
	for _, file := range files {
		...
		case name:
			...
			if check.geckoFile(file.PkgName.Pos()) {
				pkg.MarkGecko()
			}
