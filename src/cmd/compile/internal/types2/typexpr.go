...
func (check *Checker) ident(x *operand, e *syntax.Name, wantType bool) {
	...
	case nil:
		...
		} else if e.Value == "null" && check.geckoFile(e.Pos()) {
			// gecko: null is the gecko name of the nil value; resolve it
			// to the predeclared nil object so the rest of this function
			// runs normally (see the *Nil case below).
			obj = Universe.Lookup("nil")
	...
	case *Nil:
		if e.Value == "nil" && check.geckoFile(e.Pos()) {
			check.error(e, UnsupportedFeature, "nil is not allowed in gecko; use null instead")
			return
		}
		x.mode_ = nilvalue
