...
/*
   ...
   keeping task stacks in linear memory and unwinding
   the Wasm stack each time we want to switch tasks.
   ...
   is left on the top of the Go stack indicating where the task
   should be resumed.

   Stack pointer:

   There is a single global stack pointer which records the stack pointer
   used by the currently active task. This is just an address in
   ...
   normal execution, they are used only when resuming tasks.
   ...
   The main loop that executes tasks is wasm_pc_f_loop, in
   ...
   Note that when resuming a task, only the most recent function
   invocation of that task appears on the Wasm stack. When that
