...
/*
Compile, typically invoked as ``gecko tool compile,'' compiles a single Go package
...
or passed directly to the linker (``gecko tool link''). If invoked with -pack, the compiler
...
Flags:
	...
	-goversion string
		Specify required gecko tool version of the runtime.
		Exits when the runtime gecko version does not match goversion.
	-h
...
at times when it is unsafe for the calling task to be preempted.
