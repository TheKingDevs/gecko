...
//go:generate go test cmd/gecko -v -run=TestScript/README --fixreadme
...
func TestScript(t *testing.T) {
	...
	for _, file := range files {
		...
		if usesModCommand(file) {
			// gecko removes the 'mod' subcommand: scripts that exercise module
			// maintenance via 'gecko mod ...' are skipped instead of failing.
			t.Run(name, func(t *testing.T) {
				t.Skip("mod command removed in gecko")
			})
			continue
		}
		t.Run(name, func(t *testing.T) {
			...
			// will work better seeing the full path relative to cmd/gecko
...
// usesModCommand reports whether the given script directs the gecko command's
// removed 'mod' subcommand (or its help topics). TestScript skips such scripts
// rather than failing: gecko does not provide a 'mod' command.
func usesModCommand(file string) bool {
	a, err := txtar.ParseFile(file)
	if err != nil {
		return false
	}
	for _, line := range strings.Split(string(a.Comment), "\n") {
		// Ignore leading conditionals such as "[GOOS:windows]" and
		// the "!"/"?" command markers.
		for {
		...
		if len(fields) == 0 || (fields[0] != "go" && fields[0] != "gecko") {
			continue
		}
		// Walk the subcommand, allowing the '-C <dir>' prefix.
		i := 1
		for i < len(fields) {
			switch fields[i] {
			case "-C":
				i += 2 // Skip '-C' and its directory argument.
			case "help":
				if i+1 < len(fields) && fields[i+1] == "mod" {
					return true
				}
				i = len(fields) // 'help <topic>' is not the mod command.
			case "mod":
				return true
			default:
				i = len(fields) // Some other subcommand; inspect the next line.
			}
		}
	}
	return false
...
func scriptEnv(srv *vcstest.Server, srvCertFile string) ([]string, error) {
	...
	if os.Getenv("CGO_ENABLED") != "" || runtime.GOOS != goHostOS || runtime.GOARCH != goHostArch {
		// If the actual CGO_ENABLED might not match the cmd/gecko default, set it
