...
func checkScriptReadme(t *testing.T, engine *script.Engine, env []string) {
	...
	if *fixReadme {
		...
		t.Logf("\n%s", diff)
		t.Errorf("%s is stale. To update, run 'gecko generate cmd/gecko'.", readmePath)
	}
}

const readmeTmpl = `
This file is generated by 'gecko generate cmd/gecko'. DO NOT EDIT.

This directory holds test scripts *.txt run during 'gecko test cmd/gecko'.
To run a specific script foo.txt

	gecko test cmd/gecko -run=Script/^foo$
...
Scripts also have access to other environment variables, including:
	...
	GKOS=<target GKOS>
	GKPATH=$WORK/gopath
	GKPROXY=<local module proxy serving from cmd/gecko/testdata/mod>
	GKROOT=<actual GKROOT>
	TESTGO_GOROOT=<GKROOT used to build cmd/gecko, for use in tests that may change GKROOT>
	HOME=/no-home
	PATH=<actual PATH>
	TMPDIR=$WORK/tmp
...
is shown next to the phase heading. To see the entire execution, use "gecko test -v",
which also adds an initial environment dump to the beginning of the log.

Note also that in reported output, the actual name of the per-script temporary directory
has been consistently replaced with the literal string $WORK.

The cmd/gecko test flag -testwork (which must appear on the "gecko test" command line after
