...
//go:generate gecko test cmd/gecko -v -run=^TestDocsUpToDate$ -fixdocs
...
import (
	...
	"cmd/gecko/internal/bug"
	"cmd/gecko/internal/cfg"
	"cmd/gecko/internal/clean"
	"cmd/gecko/internal/doc"
	"cmd/gecko/internal/envcmd"
	"cmd/gecko/internal/fmtcmd"
	"cmd/gecko/internal/generate"
	"cmd/gecko/internal/help"
	"cmd/gecko/internal/list"
	"cmd/gecko/internal/modfetch"
	"cmd/gecko/internal/modget"
	"cmd/gecko/internal/run"
	"cmd/gecko/internal/telemetrycmd"
	"cmd/gecko/internal/telemetrystats"
	"cmd/gecko/internal/test"
	"cmd/gecko/internal/tool"
	"cmd/gecko/internal/toolchain"
	"cmd/gecko/internal/trace"
	"cmd/gecko/internal/version"
	"cmd/gecko/internal/vet"
	"cmd/gecko/internal/work"
	"cmd/gecko/internal/workcmd"
...
func main() {
	...
	if len(cmd.Commands) > 0 {
		...
		if args[used] == "help" {
			// Accept 'gecko mod help' and 'gecko mod help foo' for 'gecko help mod' and 'gecko help mod foo'.
		...
		fmt.Fprintf(os.Stderr, "gecko %s: unknown command\nRun 'gecko help%s' for usage.\n", cmdName, helpArg)
		base.SetExitStatus(2)
		base.Exit()
	}
...
// cmdIsGoTelemetryOff reports whether the command is "gecko telemetry off". This
...
func cmdIsGoTelemetryOff() bool {
	...
	// in a valid 'gecko telemetry off' command, and which hasn't been processed
	// yet. We need to determine if the command is 'gecko telemetry off' before we open
...
func invoke(cmd *base.Command, args []string) {
	// 'gecko env' handles checking the build config
...
//
//  2. A toolchain switch later on reinvokes the new gecko command with the same arguments.
