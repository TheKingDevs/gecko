...
import (
	...
	"cmd/gecko/internal/cache"
	"cmd/gecko/internal/cfg"
	"cmd/gecko/internal/gover"
	"cmd/gecko/internal/search"
	"cmd/gecko/internal/toolchain"
	"cmd/gecko/internal/vcs"
	"cmd/gecko/internal/vcweb/vcstest"
	"cmd/gecko/internal/web/intercept"
	"cmd/gecko/internal/work"
...
var (
	...
	cgoEnabled           string // raw value from 'gecko env CGO_ENABLED'
)
...
// testGOROOT is the GOROOT to use when running testgo, a cmd/gecko binary
...
func TestMain(m *testing.M) {
	// When CMDGO_TEST_RUN_MAIN is set, we're reusing the test binary as cmd/gecko.
	// Enable the special behavior needed in cmd/gecko/internal/work,
	...
	if os.Getenv("CMDGO_TEST_RUN_MAIN") != "" {
		...
		if testGOROOT := os.Getenv("TESTGO_GOROOT"); testGOROOT != "" {
			...
			work.AllowInstall = func(a *work.Action) error {
				...
				notice := "This error error can occur if GKROOT is stale, in which case rerunning make.bash will fix it."
				return fmt.Errorf("%stestgo must not write to GKROOT (installing to %s) (%v)", callerPos, filepath.Join("GKROOT", rel), notice)
			}
		}
	...
	if os.Getenv("GK_GCFLAGS") != "" {
		fmt.Fprintf(os.Stderr, "testing: warning: no tests to run\n") // magic string for cmd/gecko
		fmt.Printf("cmd/gecko test is not compatible with $GK_GCFLAGS being set\n")
		fmt.Printf("SKIP\n")
		return
	}
	...
	if testenv.HasGoBuild() {
		...
		goEnv := func(name string) string {
			...
			if err != nil {
				fmt.Fprintf(os.Stderr, "gecko env %s: %v\n%s", name, err, out)
				os.Exit(2)
			}
			return strings.TrimSpace(string(out))
		}
		testGOROOT = goEnv("GKROOT")
		os.Setenv("TESTGO_GOROOT", testGOROOT)
		os.Setenv("GKROOT", testGOROOT)
		...
		goHostOS = goEnv("GKHOSTOS")
		os.Setenv("TESTGO_GOHOSTOS", goHostOS)
		goHostArch = goEnv("GKHOSTARCH")
		...
		out, err := exec.Command(gotool, "env", "GKCACHE").CombinedOutput()
		if err != nil {
			fmt.Fprintf(os.Stderr, "could not find testing GKCACHE: %v\n%s", err, out)
			os.Exit(2)
		}
	...
	if cfg.Getenv("GKCACHE") == "" {
		os.Setenv("GKCACHE", testGOCACHE) // because $HOME is gone
	}
...
func (tg *testgoData) parallel() {
	...
	for _, e := range tg.env {
		if strings.HasPrefix(e, "GKROOT=") || strings.HasPrefix(e, "GKPATH=") || strings.HasPrefix(e, "GKBIN=") {
...
func (tg *testgoData) unsetenv(name string) {
	if tg.env == nil {
		...
		tg.env = append(tg.env, "GK111MODULE=off", "TESTGONETWORK=panic")
	}
...
func (tg *testgoData) run(args ...string) {
	...
	if status := tg.doRun(args); status != nil {
		wd, _ := os.Getwd()
		tg.t.Logf("gecko %v failed unexpectedly in %s: %v", args, wd, status)
		tg.t.FailNow()
	}
}
...
func TestNewReleaseRebuildsStalePackagesInGOPATH(t *testing.T) {
	...
	tg.tempDir("gocache")
	tg.setenv("GKCACHE", tg.path("gocache"))
	...
	tg.setenv("GKROOT", tg.path("goroot"))
...
func TestGoTestWithPackageListedMultipleTimes(t *testing.T) {
	...
	if strings.Contains(strings.TrimSpace(tg.getStdout()), "\n") {
		t.Error("gecko test errors errors errors errors errors tested the same package multiple times")
	}
}

func TestGoListHasAConsistentOrder(t *testing.T) {
	tooSlow(t, "walks all of GKROOT/src twice")
	...
	if first != tg.getStdout() {
		t.Error("gecko list std ordering is inconsistent")
	}
}

func TestGoListStdDoesNotIncludeCommands(t *testing.T) {
	tooSlow(t, "walks all of GKROOT/src")

	tg := testgo(t)
	defer tg.cleanup()
	tg.parallel()
	tg.run("list", "std")
	tg.grepStdoutNot("cmd/", "gecko list std shows commands")
}

func TestGoListCmdOnlyShowsCommands(t *testing.T) {
	skipIfGccgo(t, "gccgo does not have GKROOT")
	tooSlow(t, "walks all of GKROOT/src/cmd")
	...
	for _, line := range strings.Split(out, "\n") {
		if !strings.Contains(line, "cmd/") {
			t.Error("gecko list cmd shows non-commands")
			break
		}
	}
}
...
func TestGoListExport(t *testing.T) {
	...
	if buildID != toolBuildID {
		t.Fatalf(".BuildID with -export %q disagrees with 'gecko tool buildid' %q", buildID, toolBuildID)
	}
}
...
func TestGOROOTSearchFailureReporting(t *testing.T) {
	...
	tg.parallel()
	tg.runFail("install", "foo/quxx")
	if tg.grepCountBoth(regexp.QuoteMeta(filepath.Join("foo", "quxx"))+` \(from \$GKROOT\)$`) != 1 {
		t.Error(`go install foo/quxx expected error: .*foo/quxx (from $GKROOT)`)
	}
}
...
func TestMultipleGOPATHEntriesReportedSeparately(t *testing.T) {
	...
	tg.setenv("GKPATH", filepath.Join(tg.pwd(), "testdata", "a")+sep+filepath.Join(tg.pwd(), "testdata", "b"))
	tg.runFail("install", "foo/quxx")
	if tg.grepCountBoth(`testdata[/\\].[/\\]src[/\\]foo[/\\]quxx`) != 2 {
		t.Error(`go install foo/quxx expected error: .*testdata/a/src/foo/quxx (from $GKPATH)\n.*testdata/b/src/foo/quxx`)
	}
}
...
func TestMentionGOPATHInFirstGOPATHEntry(t *testing.T) {
	...
	tg.setenv("GKPATH", filepath.Join(tg.pwd(), "testdata", "a")+sep+filepath.Join(tg.pwd(), "testdata", "b"))
	tg.runFail("install", "foo/quxx")
	if tg.grepCountBoth(regexp.QuoteMeta(filepath.Join("testdata", "a", "src", "foo", "quxx"))+` \(from \$GKPATH\)$`) != 1 {
		t.Error(`go install foo/quxx expected error: .*testdata/a/src/foo/quxx (from $GKPATH)`)
	}
}
...
func TestDefaultGOPATH(t *testing.T) {
	...
	tg.run("env", "GKPATH")
	tg.grepStdout(regexp.QuoteMeta(tg.path("home/go")), "want GKPATH=$HOME/go")

	tg.setenv("GKROOT", tg.path("home/go"))
	tg.run("env", "GKPATH")
	tg.grepStdoutNot(".", "want unset GKPATH because GKROOT=$HOME/go")
...
func TestDefaultGOPATHPrintedSearchList(t *testing.T) {
	...
	tg.grepStderr(regexp.QuoteMeta(tg.path("home/go/src/github.com/golang/example/hello"))+`.*from \$GKPATH`, "expected default GKPATH")
}
...
func TestLdflagsArgumentsWithSpacesIssue3941(t *testing.T) {
	...
	defer tg.cleanup()
	tg.parallel()
	tg.tempFile("main.gk", `package main
		var extern: string
		func main() {
			println(extern)
		}`)
	tg.run("run", "-ldflags", `-X "main.extern=hello world"`, tg.path("main.gk"))
	tg.grepStdout("^hello world", `ldflags -X "main.extern=hello world"' failed`)
}
...
func TestLdFlagsLongArgumentsIssue42295(t *testing.T) {
	...
	tg.run("run", "-ldflags", fmt.Sprintf(`-X "main.extern=%s"`, buf.String()), tg.path("main.gk"))
	if tg.stdout.String() != buf.String() {
...
func TestGoTestDashCDashOControlsBinaryLocation(t *testing.T) {
	...
	tg.wantExecutable(tg.path("myerrors.test"+exeSuffix), "gecko test -c -o myerrors.test did not create myerrors.test")
}
...
func TestGoTestDashOWritesBinary(t *testing.T) {
	...
	tg.wantExecutable(tg.path("myerrors.test"+exeSuffix), "gecko test -o myerrors.test did not create myerrors.test")
}
...
func TestInstallWithTags(t *testing.T) {
	...
	tg.wantExecutable(tg.path("bin/a"+exeSuffix), "gecko install example/a example/b did not install binaries")
	tg.wantExecutable(tg.path("bin/b"+exeSuffix), "gecko install example/a example/b did not install binaries")
	tg.must(os.Remove(tg.path("bin/a" + exeSuffix)))
	tg.must(os.Remove(tg.path("bin/b" + exeSuffix)))
	tg.run("install", "-tags", "mytag", "example/...")
	tg.wantExecutable(tg.path("bin/a"+exeSuffix), "gecko install example/... did not install binaries")
	tg.wantExecutable(tg.path("bin/b"+exeSuffix), "gecko install example/... did not install binaries")
	tg.run("list", "-tags", "mytag", "example/b...")
	if strings.TrimSpace(tg.getStdout()) != "example/b" {
		t.Error("gecko list example/b did not find example/b")
	}
}
...
func TestGenerateUsesBuildContext(t *testing.T) {
	...
	tg.parallel()
	tg.tempDir("src/gen")
	tg.tempFile("src/gen/gen.go", "package gen\n//go:generate echo $GKOS $GKARCH\n")
	tg.setenv("GKPATH", tg.path("."))

	tg.setenv("GKOS", "linux")
	tg.setenv("GKARCH", "amd64")
	tg.run("generate", "gen")
	tg.grepStdout("linux amd64", "unexpected GKOS/GKARCH combination")

	tg.setenv("GKOS", "darwin")
	tg.setenv("GKARCH", "arm64")
	tg.run("generate", "gen")
	tg.grepStdout("darwin arm64", "unexpected GKOS/GKARCH combination")
}
...
func TestGoEnv(t *testing.T) {
	...
	tg.parallel()
	defer tg.cleanup()
	tg.setenv("GKOS", "freebsd") // to avoid invalid pair errors
	tg.setenv("GKARCH", "arm")
	tg.run("env", "GKARCH")
	tg.grepStdout("^arm$", "GKARCH not honored")
	...
	// If 'gecko version' is "gecko version <version> <goos>/<goarch>", then
	// 'gecko env GOVERSION' is just "<version>".
	if cmdVersion == envVersion || !strings.Contains(cmdVersion, envVersion) {
		t.Fatalf("'gecko env GOVERSION' %q should be a shorter substring of 'gecko version' %q", envVersion, cmdVersion)
	}
}
...
func testBuildmodePIE(t *testing.T, useCgo, setBuildmodeToPIE bool) {
	...
	tg.tempFile("main.gk", fmt.Sprintf(`package main;%s func main() { print("hello") }`, s))
	src := tg.path("main.gk")
...
func TestUpxCompression(t *testing.T) {
	...
	tg.tempFile("main.gk", `package main; import "fmt"; func main() { fmt.Print("hello upx") }`)
	src := tg.path("main.gk")
	obj := tg.path("main")
...
func TestCacheListStale(t *testing.T) {
	...
	tg.parallel()
	tg.makeTempdir()
	tg.setenv("GKCACHE", tg.path("cache"))
	...
	tg.setenv("GKPATH", tg.path("gopath"))
	tg.run("install", "p", "m")
...
func TestCacheCoverage(t *testing.T) {
	...
	defer tg.cleanup()
	tg.parallel()
	tg.setenv("GKPATH", filepath.Join(tg.pwd(), "testdata"))
	tg.makeTempdir()

	tg.setenv("GKCACHE", tg.path("c1"))
...
func TestBadCommandLines(t *testing.T) {
	...
	tg.setenv("GKPATH", tg.path("."))
	tg.runFail("build", "@x")
	tg.grepStderr("invalid input directory name \"@x\"|can only use path@version syntax with 'gecko get' and 'gecko install' in module-aware mode", "did not reject @x directory")

	tg.tempFile("src/@x/y/y.go", "package y\n")
	tg.setenv("GKPATH", tg.path("."))
	tg.runFail("build", "@x/y")
	tg.grepStderr("invalid import path \"@x/y\"|can only use path@version syntax with 'gecko get' and 'gecko install' in module-aware mode", "did not reject @x/y import path")
...
func TestLinkerTmpDirIsDeleted(t *testing.T) {
	...
	defer tg.cleanup()
	tg.parallel()
	tg.tempFile("a.gk", `package main; import "C"; func main() {}`)
	tg.run("build", "-ldflags", "-v", "-o", os.DevNull, tg.path("a.gk"))
