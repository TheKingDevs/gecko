...
// It is also safe for simultaneous use by multiple tasks
	...
	info, err := r.statCache.Do(rev, func() (*RevInfo, error) {
		...
		if err == nil {
			...
			if err := writeDiskStat(ctx, file, info); err != nil && !isErrReadOnlyFS(err) {
				fmt.Fprintf(os.Stderr, "gecko: writing stat cache: %v\n", err)
			}
		}
		return info, err
	})
	...
	text, err := r.gomodCache.Do(version, func() ([]byte, error) {
		...
		if err == nil {
			...
			if err := writeDiskGoMod(ctx, file, text); err != nil && !isErrReadOnlyFS(err) {
				fmt.Fprintf(os.Stderr, "gecko: writing go.mod cache: %v\n", err)
			}
		}
		return text, err
	})
...
func readDiskStat(ctx context.Context, path, rev string) (file string, info *RevInfo, err error) {
	...
	if err != nil {
		...
		// tag to the repository, 'gecko get' would not pick up the effect of that new
		// tag on the existing commits, and 'gecko' commands that referred to those
		...
		// legacy config and during an explicit 'gecko get', and a little extra latency
...
func rewriteVersionList(ctx context.Context, dir string) (err error) {
	if filepath.Base(dir) != "@v" {
		base.Fatalf("gecko: internal error: misuse of rewriteVersionList")
	}
