...
func newGitRepo(ctx context.Context, remote string, local bool) (Repo, error) {
	...
	if _, err := os.Stat(filepath.Join(r.dir, "objects")); err != nil {
		...
		if runtime.GOOS == "windows" {
			...
			// in some Windows programs. However, cmd/gecko should be able to handle
			// long paths just fine, and we expect people to use 'gecko clean' to
...
func (r *gitRepo) fetchRefsLocked(ctx context.Context) error {
	if r.local {
		panic("gecko: fetchRefsLocked called in local only mode.")
	}
...
func (r *gitRepo) RecentTag(ctx context.Context, rev, prefix string, allowed func(tag string) bool) (tag string, err error) {
	...
	//
	// Try one last time in case some other task fetched rev while we were
	// waiting on the lock.
	describe()
	return tag, err
}
...
func (r *gitRepo) DescendsFrom(ctx context.Context, rev, tag string) (bool, error) {
	...
	// this won't work with Git 1.7.1. According to golang.org/issue/28550, cmd/gecko
