...
// Package doc implements the “gecko doc” command.
package doc
...
var CmdDoc = &base.Command{
	Run:         runDoc,
	UsageLine:   "gecko doc [doc flags] [package|[package.]symbol[.methodOrField]]",
	CustomFlags: true,
...
which is schematically one of these:
	...
	gecko doc <pkg>
	gecko doc <sym>[.<methodOrField>]
	gecko doc [<pkg>.]<sym>[.<methodOrField>]
	gecko doc [<pkg>.][<sym>.]<methodOrField>
...
path. The gecko tool's usual package mechanism does not apply: package path
elements like . and ... are not implemented by gecko doc.
...
Examples:
	...
	gecko doc Foo
		...
		a package path.)
	gecko doc encoding/json
		Show documentation for the encoding/json package.
	gecko doc json
		Shorthand for encoding/json.
	gecko doc json.Number (or gecko doc json.number)
		Show documentation and method summary for json.Number.
	gecko doc json.Number.Int64 (or gecko doc json.number.int64)
		Show documentation for json.Number's Int64 method.
	gecko doc cmd/doc
		Show package docs for the doc command.
	gecko doc -cmd cmd/doc
		Show package docs and exported symbols within the doc command.
	gecko doc template.new
		Show documentation for html/template's New function.
		(html/template is lexically before text/template)
	gecko doc text/template.new # One argument
		Show documentation for text/template's New function.
	gecko doc text/template new # Two arguments
		Show documentation for text/template's New function.

	At least in the current tree, these invocations all print the
	documentation for json.Decoder's Decode method:

	gecko doc json.Decoder.Decode
	gecko doc json.decoder.decode
	gecko doc json.decode
	cd go/src/encoding/json; gecko doc decode
...
func usage(flagSet *flag.FlagSet) {
	fmt.Fprintf(os.Stderr, "Usage of [gecko] doc:\n")
	fmt.Fprintf(os.Stderr, "\tgecko doc\n")
	fmt.Fprintf(os.Stderr, "\tgecko doc <pkg>\n")
	fmt.Fprintf(os.Stderr, "\tgecko doc <sym>[.<methodOrField>]\n")
	fmt.Fprintf(os.Stderr, "\tgecko doc [<pkg>.]<sym>[.<methodOrField>]\n")
	fmt.Fprintf(os.Stderr, "\tgecko doc [<pkg>.][<sym>.]<methodOrField>\n")
	fmt.Fprintf(os.Stderr, "\tgecko doc <pkg> <sym>[.<methodOrField>]\n")
	fmt.Fprintf(os.Stderr, "For more information run\n")
	fmt.Fprintf(os.Stderr, "\tgecko help doc\n")
...
func objectPath(userPath string, pkg *Package, symbol, method string) (string, string, error) {
	...
	if path == "." {
		...
		// gecko list to get the import path.
...
func parseArgs(ctx context.Context, flagSet *flag.FlagSet, args []string) (pkg *load.Package, path, symbol string, more bool) {
	...
	if slash >= 0 {
		...
		// invokes 'gecko list' and returns the error text from the loaded package.
