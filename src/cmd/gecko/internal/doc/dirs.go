	...
	for len(next) > 0 {
		...
		for _, dir := range this {
			...
			for _, entry := range entries {
				...
				// The gecko tool ignores directories starting with ., _, or named "testdata".
...
func findCodeRoots() []Dir {
	...
	if !testGOPATH {
		...
		if state := modload.NewLoader(); state.WillBeEnabled() {
			...
			if !usingModules {
				...
				// Avoid 'gecko list -m all' below, since it will not work.
				return list
			}
		}
	}
	...
	// Find module root directories from gecko list.
...
// gecko command in use is 1.14+. This is the information needed to figure out
