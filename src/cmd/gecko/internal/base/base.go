...
type Command struct {
	...
	// Short is the short description shown in the 'gecko help' output.
	Short string

	// Long is the long message shown in the 'gecko help <this-command>' output.
	Long string
	...
	// The order here is the order in which they are printed by 'gecko help'.
...
var Go = &Command{
	UsageLine: "gecko",
	Long:      `Gecko is a tool for managing Gecko source code.`,
...
func (c *Command) LongName() string {
	...
	if name == "go" || name == "gecko" {
		return ""
	}
	return strings.TrimPrefix(strings.TrimPrefix(name, "gecko "), "go ")
}
...
func Error(err error) {
	...
	// handle each one separately so that they all have the leading "gecko: " prefix.
...
func RunStdin(cmdline []string) {
	...
	for try := range 3 {
		...
		// (*runTestActor).Act in cmd/gecko/internal/test, which does something similar.
