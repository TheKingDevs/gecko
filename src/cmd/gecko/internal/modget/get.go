...
// Package modget implements the module-aware “gecko get” command.
package modget

// The arguments to 'gecko get' are patterns with optional version queries, with
...
var CmdGet = &base.Command{
	...
	UsageLine: "gecko get [-t] [-u] [-tool] [build flags] [packages]",
...
var HelpVCS = &base.Command{
	UsageLine: "vcs",
	Short:     "controlling version control with GKVCS",
	Long: `
...
To balance the functionality and security concerns, the gecko command
...
the gecko command uses the proxy protocol instead, which is always permitted.
By default, the gecko command uses the Go module mirror (proxy.golang.org)
...
var (
	...
	getFix      = CmdGet.Flag.Bool("fix", false, "no-op; formerly ran 'gecko fix' on downloaded packages")
	...
	getInsecure = CmdGet.Flag.Bool("insecure", false, "no-op; use GKINSECURE instead")
)
...
func runGet(ctx context.Context, cmd *base.Command, args []string) {
	...
	case "", "upgrade", "patch":
		// ok
	default:
		base.Fatalf("gecko: unknown upgrade flag -u=%s", getU.rawVersion)
	}
	if getD.set {
		if !getD.value {
			base.Fatalf("gecko: -d flag may not be set to false")
		}
		fmt.Fprintf(os.Stderr, "gecko: -d flag is deprecated. -d=true is a no-op\n")
	}
	if *getF {
		fmt.Fprintf(os.Stderr, "gecko: -f flag is a no-op\n")
	}
	if *getFix {
		fmt.Fprintf(os.Stderr, "gecko: -fix flag is a no-op\n")
	}
	if *getM {
		base.Fatalf("gecko: -m flag is no longer supported")
	}
	if *getInsecure {
		base.Fatalf("gecko: -insecure flag is no longer supported; use GKINSECURE instead")
	}
	...
	// 'gecko get' is expected to do this, unlike other commands.
	moduleLoader.AllowMissingModuleImports()

	// 'gecko get' no longer builds or installs packages, so there's nothing to do
	...
	if !moduleLoader.HasModRoot() {
		base.Fatalf("gecko: go.mod file not found in current directory or any parent directory.\n" +
			"\t'gecko get' is no longer supported outside a module.\n" +
			"\tTo build and install a command, use 'gecko install' with a version,\n" +
			"\tlike 'gecko install example.com/cmd@latest'\n" +
			"\tFor more information, see https://go.dev/doc/go-get-install-deprecation\n" +
			"\tor run 'gecko help get' or 'gecko help install'.")
	}
	...
	for {
		...
		if changed := r.resolveQueries(moduleLoader, ctx, queries); changed {
			// 'gecko get' arguments can be (and often are) package patterns rather than
			...
			// the 'gecko get' command, not an intermediate state, and is therefore
	...
	if err := modload.WriteGoMod(moduleLoader, ctx, opts); err != nil {
		// A TooNewError can happen for 'gecko get go@newversion'
...
func updateTools(ld *modload.Loader, ctx context.Context, r *resolver, queries []*query, opts *modload.WriteOpts) {
	...
	for _, q := range queries {
		if search.IsMetaPackage(q.pattern) || q.pattern == "toolchain" {
			base.Fatalf("gecko: gecko get -tool does not work with \"%s\".", q.pattern)
		}
...
func parseArgs(ld *modload.Loader, ctx context.Context, rawArgs []string) (dropToolchain bool, queries []*query) {
	...
	for _, arg := range search.CleanPatterns(rawArgs) {
		...
		if q.version == "none" {
			switch q.pattern {
			case "go":
				base.Errorf("gecko: cannot use go@none")
				continue
			case "toolchain":
				dropToolchain = true
				continue
		...
		// Guard against 'gecko get x.go', a common mistake.
		...
		if strings.HasSuffix(q.raw, ".go") && q.rawVersion == "" {
			if !strings.Contains(q.raw, "/") {
				base.Errorf("gecko: %s: arguments must be package or module paths", q.raw)
				continue
			}
			if fi, err := os.Stat(q.raw); err == nil && !fi.IsDir() {
				base.Errorf("gecko: %s exists as a file, but 'gecko get' requires package arguments", q.raw)
				continue
			}
		}
...
type resolver struct {
	...
	initialVersion map[string]string // index of the initial build list at the start of 'gecko get'
...
// was selected at the start of this 'gecko get' invocation.
...
//
// We interpret arguments to 'gecko get' as packages first, and fall back to
	...
	for _, q := range r.workQueries {
		q.pathOnce(q.pattern, func() pathSet {
			...
			if len(ld.MainModules.Versions()) != 1 {
				panic("internal error: number of main modules is not exactly one in resolution phase of gecko get")
			}
...
func (r *resolver) performPatternAllQueries(ld *modload.Loader, ctx context.Context) {
	...
	// nondeterministic order. We want 'gecko get' to be fully deterministic,
...
func (r *resolver) loadPackages(ld *modload.Loader, ctx context.Context, patterns []string, findPackage func(ctx context.Context, path string, m module.Version) (versionOk bool)) {
	opts := modload.PackageOpts{
		...
		VendorModulesInGOROOTSrc: true,
		LoadTests:                *getT,
		AssumeRootsImported:      true, // After 'gecko get foo', imports of foo should build.
...
func (r *resolver) resolveQueries(ld *modload.Loader, ctx context.Context, queries []*query) (changed bool) {
	...
	//
	// If that results in conflicting versions, the user can re-run 'gecko get'
...
func (r *resolver) disambiguate(s *modload.Loader, cs pathSet) (filtered pathSet, isPackage bool, m module.Version, unique bool) {
	...
	for _, m := range cs.pkgMods {
		...
		if vr.version != m.Version {
			...
			//
			// 	gecko get example.com/foo/bar@none example.com/foo/bar/baz@latest
			//
		...
		//
		// 	gecko get example.com/foo@latest example.com/foo/bar/baz@latest
		//
...
// the start of 'gecko get', prefers modules that provide packages over those that
...
func (r *resolver) checkPackageProblems(ld *modload.Loader, ctx context.Context, pkgPatterns []string) {
	...
	if r.workspace != nil && r.workspace.hasModule(ld.MainModules.Versions()[0].Path) {
		...
		if err != nil {
			...
			// gecko get go@newversion when all the required modules
			// are old enough but the gecko command itself is not new
	...
	type modFlags int
	const (
		resolved modFlags = 1 << iota // version resolved by 'gecko get'
	...
	// package in 'all' that wasn't loaded as part of this 'gecko get' command.
	...
	for _, mm := range deprecations {
		if mm.message != "" {
			fmt.Fprintf(os.Stderr, "gecko: module %s is deprecated: %s\n", mm.m.Path, mm.message)
		}
	}
	var retractPath string
	for _, mm := range retractions {
		if mm.message != "" {
			fmt.Fprintf(os.Stderr, "gecko: warning: %v\n", mm.message)
			if retractPath == "" {
	...
	if retractPath != "" {
		fmt.Fprintf(os.Stderr, "gecko: to switch to the latest unretracted version, run:\n\tgecko get %s@latest\n", retractPath)
	}
...
func (r *resolver) reportChanges(oldReqs, newReqs []module.Version, mainHadGoDirective bool) {
	...
	// gover.DefaultGoModVersion. The gecko command synthesizes its own version
	...
	// whichever version of the gecko command happened to run.
	// See go.dev/issue/63507.
	...
	for _, c := range sortedChanges {
		// An implicit gecko version was never written in go.mod, so say so rather
		...
		if c.old == "" {
			fmt.Fprintf(os.Stderr, "gecko: added %s %s\n", what, c.new)
		} else if c.new == "none" || c.new == "" {
			fmt.Fprintf(os.Stderr, "gecko: removed %s %s\n", what, c.old)
		} else if gover.ModCompare(c.path, c.new, c.old) > 0 {
			fmt.Fprintf(os.Stderr, "gecko: upgraded %s %s => %s\n", what, c.old, c.new)
			if c.path == "go" && gover.Compare(c.old, gover.ExplicitIndirectVersion) < 0 && gover.Compare(c.new, gover.ExplicitIndirectVersion) >= 0 {
				fmt.Fprintf(os.Stderr, "\tnote: expanded dependencies to upgrade to go %s or higher; run 'gecko mod tidy' to clean up\n", gover.ExplicitIndirectVersion)
			}

		} else {
			fmt.Fprintf(os.Stderr, "gecko: downgraded %s %s => %s\n", what, c.old, c.new)
		}
	}
	...
	if err != nil {
		...
		if cfg.BuildV {
			...
			for _, c := range constraint.Conflicts {
				fmt.Fprintf(os.Stderr, "gecko: %v\n", c.String())
			}
		}

		// modload.EditBuildList reports constraint errors at
		// the module level, but 'gecko get' operates on packages.
		...
		for _, c := range constraint.Conflicts {
			...
			if c.Err != nil {
				base.Errorf("gecko: %v %srequires %v: %v", firstReason, adverb, last, c.UnwrapModuleError())
			} else {
				base.Errorf("gecko: %v %srequires %v, not %v", firstReason, adverb, last, reason(c.Constraint))
			}
		}
		return false
	}
