...
func getFileInfo(dir, name string, fset *token.FileSet) (*fileInfo, error) {
	...
	if !fsys.IsGoFileName(name) && fileListForExt(&dummyPkg, ext) == nil {
		// skip
	...
	if fsys.IsGoFileName(name) {
		...
		if strings.HasSuffix(name, "_test.go") || strings.HasSuffix(name, "_test.gk") {
			ignoreBinaryOnly = true // ignore //go:binary-only-package comments in _test files
		}
	} else {
