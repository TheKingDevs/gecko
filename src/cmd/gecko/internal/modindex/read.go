...
func dirHash(modroot, pkgdir string) (cache.ActionID, error) {
	...
	for _, d := range dirs {
		...
		// See hashOpen in cmd/gecko/internal/test/test.go for the
		// corresponding code.
		info, err := d.Info()
		if err != nil {
			return cache.ActionID{}, ErrNotIndexed
...
Files:
	for _, sf := range rp.sourceFiles {
		name := sf.name()
		if strings.HasPrefix(name, "_") || strings.HasPrefix(name, ".") || !fsys.IsGoFileName(name) || !imports.MatchFile(name, tags) {
			continue
		}
