...
var cmdUse = &base.Command{
	UsageLine: "gecko work use [-r] [moddirs]",
...
thing that gecko work use does.
...
The gecko command matches use paths to module directories without resolving
...
func workUse(ctx context.Context, s *modload.Loader, gowork string, wf *modfile.WorkFile, args []string) {
	...
	lookDir := func(dir string) {
		...
		if dup := keepDirs[absDir]; dup != "" && dup != dir {
			base.Errorf(`gecko: already added "%s" as "%s"`, dir, dup)
		}
		keepDirs[absDir] = dir
	}
