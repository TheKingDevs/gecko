...
var cmdEdit = &base.Command{
	UsageLine: "gecko work edit [editing flags] [go.work]",
...
flags are specified, as in 'gecko work edit -fmt'.
...
func runEditwork(ctx context.Context, cmd *base.Command, args []string) {
	...
	if len(args) > 1 {
		base.Fatalf("gecko: 'gecko help work edit' accepts at most one argument")
	}
	var gowork string
	if len(args) == 1 {
		gowork = args[0]
	...
	if *editGo != "" && *editGo != "none" {
		if !modfile.GoVersionRE.MatchString(*editGo) {
			base.Fatalf(`gecko work: invalid -go option; expecting something like "-go %s"`, gover.Local())
		}
	}
	if *editToolchain != "" && *editToolchain != "none" {
		if !modfile.ToolchainRE.MatchString(*editToolchain) {
			base.Fatalf(`gecko work: invalid -toolchain option; expecting something like "-toolchain go%s"`, gover.Local())
		}
	}
	...
	if !anyFlags {
		base.Fatalf("gecko: no flags specified (see 'gecko help work edit').")
	}
...
func flagEditworkUse(arg string) {
	workedits = append(workedits, func(f *modfile.WorkFile) {
		...
		if err := f.AddUse(modload.ToDirectoryPath(arg), ""); err != nil {
			base.Fatalf("gecko: -use=%s: %v", arg, err)
		}
	})
}
...
func flagEditworkDropUse(arg string) {
	workedits = append(workedits, func(f *modfile.WorkFile) {
		if err := f.DropUse(modload.ToDirectoryPath(arg)); err != nil {
			base.Fatalf("gecko: -dropuse=%s: %v", arg, err)
		}
	})
}
