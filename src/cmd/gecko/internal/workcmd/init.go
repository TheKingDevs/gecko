...
var cmdInit = &base.Command{
	UsageLine: "gecko work init [moddirs]",
...
gecko work init optionally accepts paths to the workspace modules as
arguments. If the argument is omitted, an empty workspace with no
modules will be created.

Each argument path is added to a use directive in the go.work file. The
current gecko version will also be listed in the go.work file.
...
func runInit(ctx context.Context, cmd *base.Command, args []string) {
	...
	if _, err := fsys.Stat(gowork); err == nil {
		base.Fatalf("gecko: %s already exists", gowork)
	}
