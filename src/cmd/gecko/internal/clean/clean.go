...
// Package clean implements the “gecko clean” command.
package clean
...
var CmdClean = &base.Command{
	UsageLine: "gecko clean [-i] [-r] [-cache] [-testcache] [-modcache] [-fuzzcache] [build flags] [packages]",
	Short:     "remove object files and cached files",
	Long: `
Clean removes object files from package source directories.
The gecko command builds most objects in a temporary directory,
so gecko clean is mainly concerned with object files left by other
tools or by manual invocations of gecko build.
...
source directories corresponding to the import paths:
	...
	*.[568ao]        object files, left from Makefiles
	...
	DIR(.exe)        from gecko build
	DIR.test(.exe)   from gecko test -c
	MAINFILE(.exe)   from gecko build MAINFILE.go
	*.so             from SWIG
...
archive or binary (what 'gecko install' would create).
...
The -cache flag causes clean to remove the entire gecko build cache.
...
func init() {
	...
	CmdClean.Flag.BoolVar(&cleanI, "i", false, "remove the corresponding installed archive or binary (what 'gecko install' would create)")
	...
	CmdClean.Flag.BoolVar(&cleanCache, "cache", false, "remove the entire gecko build cache")
	...
	CmdClean.Flag.BoolVar(&cleanTestcache, "testcache", false, "expire all test results in the gecko build cache")
...
func runClean(ctx context.Context, cmd *base.Command, args []string) {
	...
	if len(args) > 0 {
		...
		if cacheFlag != "" {
			base.Fatalf("gecko: clean %s cannot be used with package arguments", cacheFlag)
		}
	}
	...
	if cleanModcache {
		if cfg.GOMODCACHE == "" {
			base.Fatalf("gecko: cannot clean -modcache without a module cache")
		}
...
func clean(p *load.Package) {
	...
	if err != nil {
		base.Errorf("gecko: %s: %v", p.Dir, err)
		return
	}
