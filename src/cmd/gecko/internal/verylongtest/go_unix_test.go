...
func TestGoBuildUmask(t *testing.T) {
	...
	// we compare the file created by "gecko build" against a file written explicitly
	// by os.WriteFile.
	//
...
//
// If the whole process group for a 'gecko test' invocation receives
...
func TestTestInterrupt(t *testing.T) {
	...
	if !ee.Exited() {
		t.Fatalf("'gecko test' did not exit after interrupt: %v", err)
	}
