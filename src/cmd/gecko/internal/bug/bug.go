...
func runBug(ctx context.Context, cmd *base.Command, args []string) {
	...
	if len(args) > 0 {
		base.Fatalf("gecko: bug takes no arguments")
	}
...
func printGoVersion(w io.Writer) {
	fmt.Fprintf(w, "### What version of Go are you using (`gecko version`)?\n\n")
	fmt.Fprintf(w, "<pre>\n")
	fmt.Fprintf(w, "$ gecko version\n")
	fmt.Fprintf(w, "gecko version %s %s/%s\n", runtime.Version(), runtime.GOOS, runtime.GOARCH)
	fmt.Fprintf(w, "</pre>\n")
	fmt.Fprintf(w, "\n")
}

func printEnvDetails(ld *modload.Loader, w io.Writer) {
	fmt.Fprintf(w, "### What operating system and processor architecture are you using (`gecko env`)?\n\n")
	fmt.Fprintf(w, "<details><summary><code>gecko env</code> Output</summary><br><pre>\n")
	fmt.Fprintf(w, "$ gecko env\n")
	printGoEnv(ld, w)
	printGoDetails(w)
	printOSDetails(w)
	printCDetails(w)
...
// It ignores failures; 'gecko bug' is best effort.
