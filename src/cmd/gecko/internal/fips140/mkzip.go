...
//	go run ../../src/cmd/gecko/internal/fips140/mkzip.go [-b branch] v1.2.3
//
...
func usage() {
	fmt.Fprintf(os.Stderr, "usage: gecko run mkzip.go [-b branch] vX.Y.Z\n")
	os.Exit(2)
}
...
func main() {
	...
	// but otherwise the path is not used. The cmd/gecko code using these zips
