...
// Package list implements the “gecko list” command.
package list
...
var CmdList = &base.Command{
	...
	UsageLine: "gecko list [-f format] [-json] [-m] [list flags] [build flags] [packages]",
...
applied to a Go struct, but now a Module struct:
    ...
    type Module struct {
        ...
        GoVersion  string        // gecko version used in module
...
For example, 'gecko list -m all' might print:
...
For example, 'gecko list -m -u all' might print:

    my/main/module
    golang.org/x/text v0.3.0 [v0.4.0] => /tmp/text
    rsc.io/pdf v0.1.1 (retracted) [v0.1.2]

(For tools, 'gecko list -m -u -json all' may be more convenient to parse.)
...
the JSON output of a previous 'gecko list -m -json' invocation with the
same set of modifier flags (such as -u, -retracted, and -versions).
The gecko command may use this file to determine that a module is unchanged
...
func runList(ctx context.Context, cmd *base.Command, args []string) {
	...
	for _, arg := range args {
		if arg == "" {
			base.Fatalf("gecko: invalid package: %q", arg)
		}
	}
	...
	if *listFmt != "" && listJson {
		base.Fatalf("gecko list -f cannot be used with -json")
	}
	if *listReuse != "" && !*listM {
		base.Fatalf("gecko list -reuse cannot be used without -m")
	}
	if *listReuse != "" && moduleLoader.HasModRoot() {
		base.Fatalf("gecko list -reuse cannot be used inside a module")
	}
	...
	if *listRetracted {
		if cfg.BuildMod == "vendor" {
			base.Fatalf("gecko list -retracted cannot be used when vendoring is enabled")
		}
		if !moduleLoader.Enabled() {
			base.Fatalf("gecko list -retracted can only be used in module-aware mode")
		}
	}
	...
	if *listM {
		// Module mode.
		if *listCompiled {
			base.Fatalf("gecko list -compiled cannot be used with -m")
		}
		if *listDeps {
			// TODO(rsc): Could make this mean something with -m.
			base.Fatalf("gecko list -deps cannot be used with -m")
		}
		if *listExport {
			base.Fatalf("gecko list -export cannot be used with -m")
		}
		if *listFind {
			base.Fatalf("gecko list -find cannot be used with -m")
		}
		if *listTest {
			base.Fatalf("gecko list -test cannot be used with -m")
		}
		...
		if cfg.BuildMod == "vendor" {
			const actionDisabledFormat = "gecko: can't %s using the vendor directory\n\t(Use -mod=mod or -mod=readonly to bypass.)"
		...
		if *listReuse != "" && len(args) == 0 {
			base.Fatalf("gecko: list -m -reuse only has an effect with module@version arguments")
		}
	...
	// Package mode (not -m).
	if *listU {
		base.Fatalf("gecko list -u can only be used with -m")
	}
	if *listVersions {
		base.Fatalf("gecko list -versions can only be used with -m")
	}

	// These pairings make no sense.
	if *listFind && *listDeps {
		base.Fatalf("gecko list -deps cannot be used with -find")
	}
	if *listFind && *listTest {
		base.Fatalf("gecko list -test cannot be used with -find")
	}
	if *listFind && *listExport {
		base.Fatalf("gecko list -export cannot be used with -find")
	}
		...
		for _, p := range pkgs {
			if len(p.TestGoFiles)+len(p.XTestGoFiles) > 0 {
				...
				if *listE {
					...
					if perr != nil {
						base.Fatalf("gecko: can't load test package: %s", perr.Error)
					}
				}
