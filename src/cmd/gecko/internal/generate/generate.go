...
// Package generate implements the “gecko generate” command.
package generate
...
var CmdGenerate = &base.Command{
	Run:       runGenerate,
	UsageLine: "gecko generate [-run regexp] [-n] [-v] [-x] [build flags] [file.go... | packages]",
	Short:     "generate gecko files by processing source",
	Long: `
Generate runs commands described by directives within existing
files. Those commands can run any process but the intent is to
create or update Go source files.

Go generate is never run automatically by gecko build, gecko test,
...
(fmt), a fully qualified path (/usr/you/bin/mytool), or a
command alias, described below.

Note that gecko generate does not parse the file, so lines that look
...
For example,
	...
	//go:generate -command foo gecko tool foo
...
in the file, one at a time. The gecko generate tool also sets the build
tag "generate" so that files may be examined by gecko generate but ignored
during build.

For packages with invalid code, generate processes only source files with a
valid package clause.

If any generator returns an error exit status, "gecko generate" skips
...
func runGenerate(ctx context.Context, cmd *base.Command, args []string) {
	...
	for _, pkg := range load.PackagesAndErrors(moduleLoader, ctx, pkgOpts, args) {
		if moduleLoader.Enabled() && pkg.Module != nil && !pkg.Module.Main {
			if !printed {
				fmt.Fprintf(os.Stderr, "gecko: not generating in packages in dependency modules\n")
				printed = true
			}
			continue
		}
...
func (g *Generator) exec(words []string) {
	...
	if path != "" && !strings.Contains(path, string(os.PathSeparator)) {
		// If a generator says '//go:generate gecko run <blah>' it almost certainly
		// intends to use the same 'gecko' as 'gecko generate' itself.
