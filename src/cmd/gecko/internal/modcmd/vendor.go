...
var cmdVendor = &base.Command{
	UsageLine: "gecko mod vendor [-e] [-v] [-o outdir]",
...
path instead of "vendor". The gecko command can only use a vendor directory
named "vendor" within the module root directory, so this flag is
primarily useful for other tools.

See https://go.dev/ref/mod#go-mod-vendor for more about 'gecko mod vendor'.
	`,
	Run: runVendor,
}
...
func RunVendor(ld *modload.Loader, ctx context.Context, vendorE bool, vendorO string, args []string) {
	if len(args) != 0 {
		base.Fatalf("gecko: 'gecko mod vendor' accepts no arguments")
	}
	...
	if buf.Len() == 0 {
		fmt.Fprintf(os.Stderr, "gecko: no dependencies to vendor\n")
		return
	}
...
func moduleLine(m, r module.Version) string {
	...
	if r.Path != "" {
		if str.HasFilePathPrefix(filepath.Clean(r.Path), "vendor") {
			base.Fatalf("gecko: replacement path %s inside vendor directory", r.Path)
		}
		b.WriteString(" => ")
...
func vendorPkg(s *modload.Loader, vdir, pkg string) {
	...
	if gover.Compare(s.MainModules.GoVersion(s), "1.22") >= 0 {
		...
		// so that we continue to agree with older versions of the gecko command
		// about the contents of vendor directories in existing modules
		embedPatterns = str.StringList(bp.EmbedPatterns, bp.TestEmbedPatterns, bp.XTestEmbedPatterns)
	}
	embeds, err := load.ResolveEmbed(bp.Dir, embedPatterns)
	if err != nil {
		format := "gecko: resolving embeds in %s: %v\n"
		if vendorE {
...
func matchPotentialSourceFile(dir string, info fs.DirEntry, goVersion string) bool {
	...
	if info.Name() == "go.mod" || info.Name() == "go.sum" {
		if gover.Compare(goVersion, "1.17") >= 0 {
			...
			// Otherwise, 'gecko' commands invoked within the vendor subtree may misidentify
			// an arbitrary directory within the vendor tree as a module root.
			// (See https://golang.org/issue/42970.)
			return false
		}
	}
	if fsys.IsGoFileName(info.Name()) {
...
// The reason why we do this check in gecko mod vendor is to keep consistency
// with gecko build. If modifying, consider changing load() in
// src/cmd/gecko/internal/load/pkg.go
