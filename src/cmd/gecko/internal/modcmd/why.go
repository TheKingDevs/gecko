...
var cmdWhy = &base.Command{
	UsageLine: "gecko mod why [-m] [-vendor] packages...",
...
By default, why queries the graph of packages matched by "gecko list all",
...
For example:
	...
	$ gecko mod why golang.org/x/text/language golang.org/x/text/encoding
...
See https://go.dev/ref/mod#go-mod-why for more about 'gecko mod why'.
	`,
}
	...
	if *whyM {
		for _, arg := range args {
			if strings.Contains(arg, "@") {
				base.Fatalf("gecko: %s: 'gecko mod why' requires a module path, not a version query", arg)
			}
			if err := checkModulePathPattern(arg); err != nil {
				base.Errorf("gecko mod why: %v", err)
			}
		}
		base.ExitIfErrors()
