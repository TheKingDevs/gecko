...
var cmdEdit = &base.Command{
	UsageLine: "gecko mod edit [editing flags] [-fmt|-print|-json] [go.mod]",
...
flags are specified, as in 'gecko mod edit -fmt'.
...
Users should prefer 'gecko get path@version' or 'gecko get path@none',
...
use 'gecko list -m -json all'.

Edit also provides the -C, -n, and -x build flags.

See https://go.dev/ref/mod#go-mod-edit for more about 'gecko mod edit'.
	`,
}
...
func runEdit(ctx context.Context, cmd *base.Command, args []string) {
	...
	if !anyFlags {
		base.Fatalf("gecko: no flags specified (see 'gecko help mod edit').")
	}

	if *editJSON && *editPrint {
		base.Fatalf("gecko: cannot use both -json and -print")
	}

	if len(args) > 1 {
		base.Fatalf("gecko: too many arguments")
	}
	var gomod string
	if len(args) == 1 {
		gomod = args[0]
	...
	if *editModule != "" {
		...
		if err != nil {
			base.Fatalf("gecko: invalid -module: %v", err)
		}
	}

	if *editGo != "" && *editGo != "none" {
		if !modfile.GoVersionRE.MatchString(*editGo) {
			base.Fatalf(`gecko mod: invalid -go option; expecting something like "-go %s"`, gover.Local())
		}
	}
	if *editToolchain != "" && *editToolchain != "none" {
		if !modfile.ToolchainRE.MatchString(*editToolchain) {
			base.Fatalf(`gecko mod: invalid -toolchain option; expecting something like "-toolchain go%s"`, gover.Local())
		}
	}
	...
	if err != nil {
		base.Fatalf("gecko: errors parsing %s:\n%s", base.ShortPath(gomod), err)
	}
...
func parsePathVersion(flag, arg string) (path, version string) {
	...
	if err != nil {
		base.Fatalf("gecko: -%s=%s: %v", flag, arg, err)
	}
	if !found {
		base.Fatalf("gecko: -%s=%s: need path@version", flag, arg)
	}
	path, version = strings.TrimSpace(before), strings.TrimSpace(after)
	if err := module.CheckImportPath(path); err != nil {
		base.Fatalf("gecko: -%s=%s: invalid path: %v", flag, arg, err)
	}

	if !allowedVersionArg(version) {
		base.Fatalf("gecko: -%s=%s: invalid version %q", flag, arg, version)
	}
...
func parsePath(flag, arg string) (path string) {
	if strings.Contains(arg, "@") {
		base.Fatalf("gecko: -%s=%s: need just path, not path@version", flag, arg)
	}
	path = arg
	if err := module.CheckImportPath(path); err != nil {
		base.Fatalf("gecko: -%s=%s: invalid path: %v", flag, arg, err)
...
// "1234abcdef", which the gecko command will resolve the next time it runs (or
...
func flagGodebug(arg string) {
	...
	if !ok || strings.ContainsAny(arg, "\"`',") {
		base.Fatalf("gecko: -godebug=%s: need key=value", arg)
	}
	edits = append(edits, func(f *modfile.File) {
		if err := f.AddGodebug(key, value); err != nil {
			base.Fatalf("gecko: -godebug=%s: %v", arg, err)
		}
	})
}
...
func flagDropGodebug(arg string) {
	edits = append(edits, func(f *modfile.File) {
		if err := f.DropGodebug(arg); err != nil {
			base.Fatalf("gecko: -dropgodebug=%s: %v", arg, err)
		}
	})
}
...
func flagRequire(arg string) {
	...
	edits = append(edits, func(f *modfile.File) {
		if err := f.AddRequire(path, version); err != nil {
			base.Fatalf("gecko: -require=%s: %v", arg, err)
		}
	})
}
...
func flagDropRequire(arg string) {
	...
	edits = append(edits, func(f *modfile.File) {
		if err := f.DropRequire(path); err != nil {
			base.Fatalf("gecko: -droprequire=%s: %v", arg, err)
		}
	})
}
...
func flagExclude(arg string) {
	...
	edits = append(edits, func(f *modfile.File) {
		if err := f.AddExclude(path, version); err != nil {
			base.Fatalf("gecko: -exclude=%s: %v", arg, err)
		}
	})
}
...
func flagDropExclude(arg string) {
	...
	edits = append(edits, func(f *modfile.File) {
		if err := f.DropExclude(path, version); err != nil {
			base.Fatalf("gecko: -dropexclude=%s: %v", arg, err)
		}
	})
}
...
func flagReplace(arg string) {
	...
	if !found {
		base.Fatalf("gecko: -replace=%s: need old[@v]=new[@w] (missing =)", arg)
	}
	old, new := strings.TrimSpace(before), strings.TrimSpace(after)
	if strings.HasPrefix(new, ">") {
		base.Fatalf("gecko: -replace=%s: separator between old and new is =, not =>", arg)
	}
	oldPath, oldVersion, err := parsePathVersionOptional("old", old, false)
	if err != nil {
		base.Fatalf("gecko: -replace=%s: %v", arg, err)
	}
	...
	if newPath == new && !modfile.IsDirectoryPath(new) {
		base.Fatalf("gecko: -replace=%s: unversioned new path must be local directory", arg)
	}
...
func flagDropReplace(arg string) {
	...
	if err != nil {
		base.Fatalf("gecko: -dropreplace=%s: %v", arg, err)
	}
	edits = append(edits, func(f *modfile.File) {
...
func flagRetract(arg string) {
	...
	if err != nil {
		base.Fatalf("gecko: -retract=%s: %v", arg, err)
	}
	edits = append(edits, func(f *modfile.File) {
...
func flagDropRetract(arg string) {
	vi, err := parseVersionInterval(arg)
	if err != nil {
		base.Fatalf("gecko: -dropretract=%s: %v", arg, err)
	}
	edits = append(edits, func(f *modfile.File) {
...
func flagTool(arg string) {
	...
	edits = append(edits, func(f *modfile.File) {
		if err := f.AddTool(path); err != nil {
			base.Fatalf("gecko: -tool=%s: %v", arg, err)
		}
	})
}
...
func flagDropTool(arg string) {
	...
	edits = append(edits, func(f *modfile.File) {
		if err := f.DropTool(path); err != nil {
			base.Fatalf("gecko: -droptool=%s: %v", arg, err)
		}
	})
}
...
func flagIgnore(arg string) {
	edits = append(edits, func(f *modfile.File) {
		if err := f.AddIgnore(arg); err != nil {
			base.Fatalf("gecko: -ignore=%s: %v", arg, err)
		}
	})
}
...
func flagDropIgnore(arg string) {
	edits = append(edits, func(f *modfile.File) {
		if err := f.DropIgnore(arg); err != nil {
			base.Fatalf("gecko: -dropignore=%s: %v", arg, err)
		}
	})
}
