...
import (
	...
	"cmd/gecko/internal/modfetch/codehost"
...
var cmdDownload = &base.Command{
	UsageLine: "gecko mod download [-x] [-json] [-reuse=old.json] [modules]",
...
module if it is at 'gecko 1.17' or higher, or all transitively-required modules
if at 'gecko 1.16' or lower.

The gecko command will automatically download modules as needed during ordinary
execution. The "gecko mod download" command is useful mainly for pre-filling
...
previous 'gecko mod download -json' invocation. The gecko command may use this
...
See https://go.dev/ref/mod#go-mod-download for more about 'gecko mod download'.
...
func init() {
	...
	// TODO(jayconrod): https://golang.org/issue/35849 Apply -x to other 'gecko mod' commands.
	cmdDownload.Flag.BoolVar(&cfg.BuildX, "x", false, "print the commands")
	base.AddChdirFlag(&cmdDownload.Flag)
	base.AddModCommonFlags(&cmdDownload.Flag)
}

// A ModuleJSON describes the result of gecko mod download.
...
func runDownload(ctx context.Context, cmd *base.Command, args []string) {
	...
	if moduleLoader.HasModRoot() || modload.WorkFilePath(moduleLoader) != "" {
		...
		if haveExplicitArgs {
			for _, mainModule := range moduleLoader.MainModules.Versions() {
				...
				for _, arg := range args {
					...
					case mainModule.Path, targetAtUpgrade, targetAtPatch:
						os.Stderr.WriteString("gecko: skipping download of " + arg + " that resolves to the main module\n")
					}
				}
			}
			...
			if modFile.Go == nil || gover.Compare(modFile.Go.Version, gover.ExplicitIndirectVersion) < 0 {
				...
				// 'gecko mod download' is typically run before testing packages in the
				...
				// we have downloaded enough of the module graph to run 'gecko list all',
				// 'gecko mod graph', and similar commands.
	...
	if len(args) == 0 {
		if moduleLoader.HasModRoot() {
			os.Stderr.WriteString("gecko: no module dependencies to download\n")
		} else {
			base.Errorf("gecko: no modules specified (see 'gecko help mod download')")
		}
		base.Exit()
	}

	if *downloadReuse != "" && moduleLoader.HasModRoot() {
		base.Fatalf("gecko mod download -reuse cannot be used inside a module")
	}
	...
	if !haveExplicitArgs && modload.WorkFilePath(moduleLoader) == "" {
		// 'gecko mod download' is sometimes run without arguments to pre-populate the
	...
	// Fill semaphore channel to wait for tasks to finish.
	for n := cap(sem); n > 0; n-- {
		sem <- token{}
	}

	// If there were explicit arguments
	// (like 'gecko mod download golang.org/x/tools@latest'),
	...
	// be tidy and the go.mod file to declare a 'gecko' version that satisfies
	...
	// 'gecko mod download mod@version' is a useful way to add a sum without using
	// 'gecko get mod@version', which may have other side effects. We print this in
	...
	//
	// Don't save sums for 'gecko mod download' without arguments unless we're in
...
// DownloadModule runs 'gecko mod download' for m.Path@m.Version,
