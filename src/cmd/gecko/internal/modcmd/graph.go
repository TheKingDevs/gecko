...
var cmdGraph = &base.Command{
	UsageLine: "gecko mod graph [-go=version] [-x]",
...
given Go version, instead of the version indicated by the 'gecko' directive
in the go.mod file.

The -x flag causes graph to print the commands graph executes.

See https://go.dev/ref/mod#go-mod-graph for more about 'gecko mod graph'.
	`,
	Run: runGraph,
}
...
func runGraph(ctx context.Context, cmd *base.Command, args []string) {
	...
	if len(args) > 0 {
		base.Fatalf("gecko: 'gecko mod graph' accepts no arguments")
	}
