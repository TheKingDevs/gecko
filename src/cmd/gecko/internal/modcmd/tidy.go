...
var cmdTidy = &base.Command{
	UsageLine: "gecko mod tidy [-e] [-v] [-x] [-diff] [-go=version] [-compat=version]",
...
The -go flag causes tidy to update the 'gecko' directive in the go.mod
...
'gecko' command from the indicated major Go release to successfully load
the module graph, and causes tidy to error out if that version of the
'gecko' command would load any imported package from a different module
version. By default, tidy acts as if the -compat flag were set to the
version prior to the one indicated by the 'gecko' directive in the go.mod
file.

The -x flag causes tidy to print the commands download executes.

See https://go.dev/ref/mod#go-mod-tidy for more about 'gecko mod tidy'.
	`,
	Run: runTidy,
}
...
var (
	...
	tidyGo     goVersionFlag // gecko version to write to the tidied go.mod file (toggles lazy loading)
	tidyCompat goVersionFlag // gecko version for which the tidied go.mod and go.sum files should be “compatible”
)
...
//
// (Note that the -go argument to 'gecko mod edit' is *not* a goVersionFlag.
...
func runTidy(ctx context.Context, cmd *base.Command, args []string) {
	...
	if len(args) > 0 {
		base.Fatalf("gecko: 'gecko mod tidy' accepts no arguments")
	}

	// Tidy aims to make 'gecko test' reproducible for any package in 'all', so we
	...
	// those packages. In order to make 'gecko test' reproducible for the packages
