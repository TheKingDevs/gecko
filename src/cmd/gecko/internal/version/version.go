...
// Package version implements the “gecko version” command.
package version
...
// GeckoVersion is the gecko toolchain version reported by
// "gecko version" when invoked without file arguments.
const GeckoVersion = "v1.0.0"

var CmdVersion = &base.Command{
	UsageLine: "gecko version [-m] [-v] [-json] [file ...]",
	Short:     "print gecko version",
	Long: `Version prints the build information for Go binary files.

Go version reports the Go version used to build each of the named files.

If no files are named on the command line, gecko version prints its own
version information.

If a directory is named, gecko version walks that directory, recursively,
looking for recognized Go binaries and reporting their versions.
By default, gecko version does not report unrecognized files found
during a directory scan. The -v flag causes it to report unrecognized files.

The -m flag causes gecko version to print each file's embedded
module version information, when available. In the output, the module
information consists of multiple lines following the version line, each
indented by a leading tab character.

The -json flag is similar to -m but outputs the runtime/debug.BuildInfo in JSON format.
If flag -json is specified without -m, gecko version reports an error.

See also: gecko doc runtime/debug.BuildInfo.
`,
}
...
func runVersion(ctx context.Context, cmd *base.Command, args []string) {
	if len(args) == 0 {
		...
		if argOnlyFlag != "" {
			fmt.Fprintf(os.Stderr, "gecko: 'gecko version' only accepts %s flag with arguments\n", argOnlyFlag)
			base.SetExitStatus(2)
			return
		}
		fmt.Printf("gecko %s (%s/%s)\n", GeckoVersion, runtime.GOOS, runtime.GOARCH)
		return
	}

	if !*versionM && *versionJson {
		fmt.Fprintf(os.Stderr, "gecko: 'gecko version' with -json flag requires -m flag\n")
		base.SetExitStatus(2)
		return
	}
...
func scanFile(file string, info fs.FileInfo, mustPrint bool) bool {
	...
	fmt.Printf("%s: %s\n", file, bi.GoVersion)
	bi.GoVersion = "" // suppress printing gecko version again
	mod := bi.String()
