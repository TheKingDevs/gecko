...
import "cmd/gecko/internal/base"
...
The first is the cgecko tool, which is part of the Go distribution. For
information on how to use it see the cgecko documentation (gecko doc cmd/cgo).

The second is the SWIG program, which is a general tool for
interfacing between languages. For information on SWIG see
https://swig.org/. When running gecko build, any file with a .swig
...
When either cgo or SWIG is used, gecko build will pass any .c, .m, .s, .S
...
"gecko test ./utf8" instead of needing to specify the full path.
Similarly, in the reverse situation, "gecko test .." will test "unicode" from
the "unicode/utf8" directory. Relative patterns are also allowed, such as
"gecko test ./..." to test all subdirectories.
...
the gecko command synthesizes a virtual package named "command-line-arguments"
containing just the given files. In most cases, it is an error
to do so (e.g. "gecko build main.go" or "gecko build *.go").
...
As the gecko command primarily operates on directories,
...
package. See "gecko help test" and "gecko help testflag" for details.
...
identifiers. See "gecko doc testing" for details.

For more information about import paths, see "gecko help importpath".
`,
}
...
and "test" for testing. These modules cannot be fetched by the gecko command.
...
with the version control type, or the gecko tool can dynamically fetch
...
version control qualifier, the gecko tool attempts to fetch the import
...
fetched with "gecko get". If it's not an exact match, another http
request is made at the prefix to verify the <meta> tags match.

The meta tag should appear as early in the file as possible.
In particular, it should appear before any raw JavaScript or CSS,
to avoid confusing the gecko command's restricted parser.
...
the gecko tool will verify that https://example.org/?go-get=1 contains the
...
the gecko tool will verify that https://example.org/?go-get=1 contains the same meta
...
The gecko command and the tools it invokes consult environment variables
for configuration. If an environment variable is unset or empty, the go
command uses a sensible default setting. To see the effective setting of
the variable <NAME>, run 'gecko env <NAME>'. To change the default setting,
run 'gecko env -w <NAME>=<VALUE>'. Defaults changed using 'gecko env -w'
...
General-purpose environment variables:
	...
	GKCACHEPROG
		...
		See 'gecko doc cmd/gecko/internal/cacheprog'.
	GODEBUG
...
Environment variables for use with cgo:
	...
	CGO_ENABLED
		Whether the cgecko command is supported. Either 0 or 1.
...
Environment variables for use with code coverage:
	...
	GOCOVERDIR
		...
		generated by running a "gecko build -cover" binary.
...
var HelpFileType = &base.Command{
	...
	Short:     "file types",
	Long: `
The gecko command examines the contents of a restricted set of files
...
constraints, but the gecko command stops scanning for build constraints
...
var HelpBuildmode = &base.Command{
	...
	Short:     "build modes",
	Long: `
The 'gecko build' and 'gecko install' commands take a -buildmode argument which
...
(see 'gecko help build') to force rebuilding of packages that
depend on the updated C libraries.

The gecko command also caches successful package test results.
See 'gecko help test' for details. Running 'gecko clean -testcache' removes
all cached test results (but not cached build results).

The gecko command also caches values used in fuzzing with 'gecko test -fuzz',
...
Running 'gecko clean -fuzzcache' removes all cached fuzzing values.
This may make fuzzing less effective, temporarily.

The GODEBUG environment variable can enable printing of debugging
information about the state of the cache:

GODEBUG=gocacheverify=1 causes the gecko command to bypass the
use of any cache entries and instead rebuild everything and check
that the results match existing cache entries.

GODEBUG=gocachehash=1 causes the gecko command to print the inputs
...
"gecko doc cmd/gecko/internal/cacheprog".
`,
}
...
During a particular build, the following build tags are satisfied:
	...
	- any additional tags given by the -tags flag (see 'gecko help build').
...
with a "// +build" prefix. The fmt command will add an equivalent //go:build
...
git dir
	...
	gecko command will run 'git credential approve/reject' to update
...
command
	...
	If the server responds with any 4xx code, the gecko command will write the
...
var HelpBuildJSON = &base.Command{
	...
	Long: `
The 'gecko build', 'gecko install', and 'gecko test' commands take a -json flag that
...
This matches the Package.ImportPath field of gecko list -json and the
TestEvent.FailedBuild field of gecko test -json. Note that it does not
...
field produced by gecko test -json.

For gecko test -json, this struct is designed so that parsers can distinguish
...
have appeared from gecko build without the -json flag.
