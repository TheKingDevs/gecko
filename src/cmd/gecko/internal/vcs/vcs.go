...
var (
	...
	// cmd/gecko/internal/vcweb.Server.
...
func (v *Cmd) run1(dir string, cmdline string, keyval []string, verbose bool) ([]byte, error) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr,
			"gecko: missing %s command. See https://go.dev/s/gogetcmd\n",
			v.Name)
		return nil, err
	}
...
func interceptVCSTest(repo string, vcs *Cmd, security web.SecurityMode) (repoURL string, ok bool) {
	...
	if vcs == vcsMod {
		...
		// requests will be intercepted at a lower level (in cmd/gecko/internal/web).
		return "", false
	}
...
// on cmd/gecko/internal/load. It just needs to satisfy load.ImportPathError.
