...
// Cache is the interface as used by the cmd/gecko.
...
func (c *DiskCache) putIndexEntry(id ActionID, out OutputID, size int64, allowVerify bool) error {
	...
	if verify && allowVerify {
		...
		if err == nil && (old.OutputID != out || old.Size != size) {
			...
			msg := fmt.Sprintf("gecko: internal cache error: cache verify failed: id=%x changed:<<<\n%s\n>>>\nold: %x %d\nnew: %x %d", id, reverseHash(id), out, size, old.OutputID, old.Size)
			panic(msg)
		}
	}
...
// directory aren't removed by the 'gecko clean -cache' command or by Trim.
// They may be removed with 'gecko clean -fuzzcache'.
//
