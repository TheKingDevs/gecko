...
// but when testing a cross-compiled cmd/gecko they will
// indicate the GOOS and GOARCH of the installed cmd/gecko
...
func SetGOROOT(goroot string, isTestGo bool) {
	...
	if runtime.Compiler != "gccgo" {
		if goroot == "" {
			...
			// are testing a cross-compiled cmd/gecko all of the installed packages and
...
// x/tools/cmd/geckodoc/goroot.go.
...
//
// There is a copy of this code in x/tools/cmd/geckodoc/goroot.go.
func findGOROOT(env string) string {
	...
	if err == nil {
		...
		if err == nil {
			// cmd/gecko may be installed in GOROOT/bin or GOROOT/bin/GOOS_GOARCH,
...
// which is necessary for useful operation of the cmd/gecko tool,
