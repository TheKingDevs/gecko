...
import (
	...
	"cmd/gecko/internal/test/internal/genflags"
	"internal/testenv"
	"maps"
	"os"
	"testing"
...
// consistent with output of "gecko tool vet -flags", using the installed
...
func TestPassFlagToTest(t *testing.T) {
	...
	if t.Failed() {
		t.Logf("To regenerate:\n\tgecko generate cmd/gecko/internal/test")
	}
}

func TestPassAnalyzersToVet(t *testing.T) {
	testenv.MustHaveGoBuild(t) // runs 'gecko tool vet -flags'
