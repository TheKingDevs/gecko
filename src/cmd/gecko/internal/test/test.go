...
import (
	...
	"cmd/gecko/internal/load"
	"cmd/gecko/internal/lockedfile"
	"cmd/gecko/internal/modload"
	"cmd/gecko/internal/search"
	"cmd/gecko/internal/str"
	"cmd/gecko/internal/trace"
...
const testUsage = "gecko test [build/test flags] [packages] [build/test flags & test binary flags]"
...
tests and example functions. See 'gecko help testfunc' for more.
...
The gecko tool will ignore a directory named "testdata", making it available
to hold ancillary data needed by the tests.

As part of building a test binary, gecko test runs gecko vet on the package
and its test source files to identify significant problems. If gecko vet
finds any problems, gecko test reports those and does not run the test
binary. Only a high-confidence subset of the default gecko vet checks are
...
"gecko doc cmd/vet". To disable the running of gecko vet, use the -vet=off flag.
To run all checks, use the -vet=all flag.

All test output and summary lines are printed to the gecko command's
...
The first, called local directory mode, occurs when gecko test is
invoked with no package arguments (for example, 'gecko test' or 'gecko
test -v'). In this mode, gecko test compiles the package sources and
tests found in the current directory and then runs the resulting
test binary. In this mode, caching (discussed below) is disabled.
After the package test finishes, gecko test prints a summary line
showing the test status ('ok' or 'FAIL'), package name, and elapsed
time.

The second, called package list mode, occurs when gecko test is invoked
with explicit package arguments (for example 'gecko test math', 'gecko
test ./...', and even 'gecko test .'). In this mode, gecko test compiles
and tests each of the packages listed on the command line. If a
package test passes, gecko test prints only the final 'ok' summary
line. If a package test fails, gecko test prints the full test output.
If invoked with the -bench or -v flag, gecko test prints the full
output even for passing package tests, in order to display the
requested benchmark results or verbose logging. After the package
tests for all of the listed packages finish, and their output is
printed, gecko test prints a final 'FAIL' status if any package test
has failed.

In package list mode only, gecko test caches successful package test
results to avoid unnecessary repeated running of tests. When the
result of a test can be recovered from the cache, gecko test will
redisplay the previous output instead of running the test binary
again. When this happens, gecko test prints '(cached)' in place of the
...
If a run of gecko test has any test or non-test flags outside this set,
...
In addition to the build flags, the flags handled by 'gecko test' itself are:
	...
	-exec xprog
	    ...
	    in 'gecko run'. See 'gecko help run' for details.

	-json
	    Convert test output to JSON suitable for automated processing.
	    See 'gecko doc test2json' for the encoding details.
	    Also emits build output in JSON. See 'gecko help buildjson'.
...
flags are also accessible by 'gecko test'. See 'gecko help testflag' for details.

For more about build flags, see 'gecko help build'.
For more about specifying packages, see 'gecko help packages'.

See also: gecko build, gecko vet.
`,
}
...
var HelpTestflag = &base.Command{
	...
	Short:     "testing flags",
	Long: `
The 'gecko test' command takes both flags that apply to 'gecko test' itself
and flags that apply to the resulting test binary.

Several of the flags control profiling and write an execution profile
suitable for "gecko tool pprof"; run "gecko tool pprof -h" for more
...
The following flags are recognized by the 'gecko test' command and
control the execution of any test:

	-artifacts
	    Save test artifacts in the directory specified by -outputdir.
	    See 'gecko doc testing.T.ArtifactDir'.
	...
	-coverpkg pattern1,pattern2,pattern3
	    ...
	    the package being tested. See 'gecko help packages' for a description
	...
	-outputdir directory
	    ...
	    specified directory, by default the directory in which "gecko test" is running.
	...
	-parallel n
	    ...
	    The 'gecko test' command may run tests for different packages
	    in parallel as well, according to the setting of the -p flag
	    (see 'gecko help build').
	...
	-vet list
	    Configure the invocation of "gecko vet" during "gecko test"
	    to use the comma-separated list of vet checks.
	    If list is empty, "gecko test" runs "gecko vet" with a curated list of
	    checks believed to be always worth addressing.
	    If list is "off", "gecko test" does not run "gecko vet" at all.

The following flags are also recognized by 'gecko test' and can be used to
profile the tests during execution:
	...
	-blockprofilerate n
	    ...
	    See 'gecko doc runtime.SetBlockProfileRate'.
	...
	-memprofilerate n
	    ...
	    setting runtime.MemProfileRate. See 'gecko doc runtime.MemProfileRate'.
...
'gecko test -c') directly, however, the prefix is mandatory.

The 'gecko test' command rewrites or removes recognized flags,
as appropriate, both before and after the optional package list,
before invoking the test binary.

For instance, the command

	gecko test -v -myflag testdata -cpuprofile=prof.out -x

will compile the test binary and then run it as

	pkg.test -test.v -myflag testdata -test.cpuprofile=prof.out

(The -x flag is removed because it applies only to the gecko command's
execution, not to the test itself.)

The test flags that generate profiles (other than for coverage) also
leave the test binary in pkg.test for use when analyzing the profiles.

When 'gecko test' runs a test binary, it does so from within the
...
flag not known to the gecko test command. Continuing the example above,
the package list would have to appear before -myflag, but could appear
on either side of -v.

When 'gecko test' runs in package list mode, 'gecko test' caches successful
...
known flag or a package name, use -args (see 'gecko help test') which
passes the remainder of the command line through to the test binary
uninterpreted and unaltered.

For instance, the command

	gecko test -v -args -x -v
...
test binary unchanged and with no effect on the gecko command itself.
...
var HelpTestfunc = &base.Command{
	...
	Long: `
The 'gecko test' command expects to find test, benchmark, and example functions
...
func runTest(ctx context.Context, cmd *base.Command, args []string) {
	...
	if testFuzz != "" {
		...
		if m := pkgs[0].Module; m != nil && m.Path != "" {
			...
			// they are only treated as part of a module in 'gecko mod' subcommands and
			// 'gecko get'. However, we still don't want to accidentally corrupt their
	...
	// the test wedges with a task spinning and its background
	...
	if testTimeout > 0 && testFuzz == "" && testBench == "" {
		...
		// timeout on the assumption that the time to write and read a task
		...
		// external signal: if the process has a lot of tasks, dumping stacks
	...
	// (We implement gecko clean -testcache by writing an expiration date
	...
	if cfg.BuildCoverPkg != nil {
		...
		if len(testCoverPkgs) > 0 {
			...
			// from the top level, a user runs "gecko test -coverpkg=./... ./...".
		...
		reportSetupFailed := func(perr *load.Package, err error) {
			...
			if testJSON {
				...
				if gotestjsonbuildtext.Value() == "1" {
					// While this flag is about gecko build -json, the other effect
	...
	root := &work.Action{Mode: "gecko test", Actor: work.ActorFunc(printExitStatus), Deps: prints}
...
func builderTest(ld *modload.Loader, b *work.Builder, ctx context.Context, pkgOpts load.PackageOpts, p *load.Package, imported bool, writeCoverMetaAct *work.Action) (buildAction, runAction, printAction *work.Action, perr *load.Package, err error) {
	...
	if cfg.Goos == "windows" {
		...
		// First, you can actually run 'gecko test' in directories that
		// have names that Windows thinks are installer-like,
		// without getting a dialog box asking for more permissions.
		// Second, in the Windows process listing during gecko test,
	...
	if testC {
		...
		if writeCoverMetaAct != nil {
			...
			// "gecko test -coverpkg" run actions will need to read the
...
// tasks, so that we can have multiple JSON streams writing
...
func (r *runTestActor) Act(b *work.Builder, ctx context.Context, a *work.Action) error {
	...
	// the exact details of output buffering are up to the gecko command and
	...
	if cfg.BuildCover {
		...
		// intended to help with tests that run "gecko build" to build
	...
	//
	// If the -o flag is set, or if at some point we change cmd/gecko to start
...
func (c *runCache) tryCacheWithID(b *work.Builder, a *work.Action, id string) bool {
	if len(pkgArgs) == 0 {
		// Caching does not apply to "gecko test",
		// only to "gecko test foo" (including "gecko test .").
		if cache.DebugTest {
	...
	if entry.Time.Before(testCacheExpire) {
		if cache.DebugTest {
			fmt.Fprintf(os.Stderr, "testcache: %s: test output expired due to gecko clean -testcache\n", a.Package.ImportPath)
		}
		return false
	}
...
func coveragePercentage(out []byte) string {
	...
	if matches == nil {
		// Probably running "gecko test -cover" not "gecko test -cover fmt".
...
// like it is during 'gecko run', use the package name.
