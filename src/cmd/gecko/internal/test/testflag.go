...
//go:generate gecko run ./genflags.go

// The flag handling part of gecko test is large and distracting.
...
func init() {
	...
	cf.Var((*base.StringsFlag)(&work.ExecCmd), "exec", "run the test binary using `xprog`; see 'gecko help run' for details")
	...
	// some of them so that cmd/gecko knows what to do with the test output, or knows
...
// It interprets an empty value as the working directory of the 'gecko' command.
...
type vetFlag struct {
	explicit bool
	off      bool
	flags    []string // passed to vet when invoked automatically during 'gecko test'
}
...
// Unfortunately for us, we need to do our own flag processing because gecko test
// grabs some flags but otherwise its command line is just a holding place for
// pkg.test's arguments.
// We allow known flags both before and after the package name list,
// to allow both
//
//	gecko test fmt -custom-flag-for-fmt-test
//	gecko test -x math
func testFlags(args []string) (packageNames, passToTest []string) {
	...
	// firstUnknownFlag helps us report an error when flags not known to 'gecko
	...
	for len(args) > 0 {
		...
		if errors.Is(err, cmdflag.ErrFlagTerminator) {
			// 'gecko list' allows package arguments to be named either before or after
			// the terminator, but 'gecko test' has historically allowed them only
		...
		if nf, ok := errors.AsType[cmdflag.NonFlagError](err); ok {
			if !inPkgList && packageNames != nil {
				...
				if wasAfterFlagWithoutValue {
					...
					// optimistically assume that it is and keep looking for gecko command
					// flags after it.
					//
	...
	if firstUnknownFlag != "" && testC {
		fmt.Fprintf(os.Stderr, "gecko: unknown flag %s cannot be used with -c\n", firstUnknownFlag)
		exitWithUsage()
	}
	...
	// 'gecko test' has a default timeout, but the test binary itself does not.
	...
	// directory, but 'gecko test' defaults it to the working directory of the 'gecko'
...
func exitWithUsage() {
	...
	fmt.Fprintf(os.Stderr, "\nRun 'gecko help %s' and 'gecko help %s' for details.\n", CmdTest.LongName(), HelpTestflag.LongName())
