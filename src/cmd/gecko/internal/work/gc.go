...
func (gcToolchain) gc(b *Builder, a *Action, export string, importcfg, embedcfg []byte, symabis string, asmhdr bool, pgoProfile, coverCfg string, gofiles []string) (ofile string, output []byte, err error) {
	...
	if typ := load.GeckoProjectType(p.Dir); typ == "typed" {
		defaultGcFlags = append(defaultGcFlags, "-geckotype=typed")
	}
...
func compilerConcurrency() (int, func()) {
	...
	// potentially have GOMAXPROCS^2 running compile tasks) In the past, we'd limit
	...
	// We'll set aside one token for each of the action tasks that aren't currently running a compile.
