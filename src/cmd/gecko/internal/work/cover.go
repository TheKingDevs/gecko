...
// CovData invokes "gecko tool covdata" with the specified arguments
...
func (b *Builder) CovData(a *Action, cmdargs ...any) ([]byte, error) {
	...
	args = append(args, filepath.Join(cfg.GOROOTbin, "gecko"), "tool", "covdata")
	args = append(args, cmdline...)
	return b.Shell(a).runOut(a.Objdir, nil, args)
}

// BuildActionCoverMetaFile locates and returns the path of the
// meta-data file written by the "gecko tool cover" step as part of the
// build action for the "gecko test -cover" run action 'runAct'. Note
...
// cases where a user runs "gecko test -cover" on a package that has
...
// 'mf'. This helper is used in cases where a user runs "gecko test
...
// pseudo action employed during "gecko test -coverpkg" runs where there
