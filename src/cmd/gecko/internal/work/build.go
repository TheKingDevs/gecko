...
var CmdBuild = &base.Command{
	UsageLine: "gecko build [-o output] [build flags] [packages]",
...
So 'gecko build example/sam' writes 'sam' or 'sam.exe'.
'gecko build example.com/foo/v2' writes 'foo' or 'foo.exe', not 'v2.exe'.

When compiling a package from a list of .go files, the executable
is named after the first source file.
'gecko build ed.go rx.go' writes 'ed' or 'ed.exe'.
...
and test commands:
	...
	-coverpkg pattern1,pattern2,pattern3
		...
		'gecko help packages' for a description of package patterns.
		Sets -cover.
	-v
	...
	-asmflags '[pattern=]arg list'
		arguments to pass on each gecko tool asm invocation.
	-buildmode mode
		build mode to use. See 'gecko help buildmode' for more.
	-buildvcs
	...
	-gcflags '[pattern=]arg list'
		arguments to pass on each gecko tool compile invocation.
	...
	-json
		...
		See 'gecko help buildjson' for the encoding details.
	-ldflags '[pattern=]arg list'
		arguments to pass on each gecko tool link invocation.
	-linkshared
		build code that will be linked against shared libraries previously
		created with -buildmode=shared.
	-mod mode
		module download mode to use: readonly, vendor, or mod.
		By default, if a vendor directory is present and the gecko version in go.mod
		is 1.14 or higher, the gecko command acts as if -mod=vendor were set.
		Otherwise, the gecko command acts as if -mod=readonly were set.
	...
	-pgo file
		...
		build, the gecko command selects a file named "default.pgo" in the package's
	...
	-tags tag,list
		...
		'gecko help buildconstraint'. (Earlier versions of Go used a
	...
	-toolexec 'cmd args'
		...
		matching 'gecko list -f {{.ImportPath}}' for the package being built.
...
that pattern (see 'gecko help packages' for a description of package
patterns). Without a pattern, the argument list applies only to the
packages named on the command line. The flags may be repeated
with different patterns in order to specify different arguments for
different sets of packages. If a package matches patterns given in
multiple flags, the latest match on the command line wins.
For example, 'gecko build -gcflags=-S fmt' prints the disassembly
only for package fmt, while 'gecko build -gcflags=all=-S fmt'
...
// (for example, buildV) are in cmd/gecko/internal/cfg.
...
func AddBuildFlags(cmd *base.Command, mask BuildFlagMask) {
	...
	cmd.Flag.Var(&load.BuildAsmflags, "asmflags", "`arguments` to pass on each gecko tool asm invocation")
	cmd.Flag.Var(buildCompiler{}, "compiler", "`name` of compiler to use, as in runtime.Compiler: gccgo, gc")
	cmd.Flag.StringVar(&cfg.BuildBuildmode, "buildmode", "default", "build `mode` to use; see 'gecko help buildmode' for details: archive, c-archive, c-shared, default, shared, exe, pie, plugin")
	...
	cmd.Flag.Var(&load.BuildGcflags, "gcflags", "`arguments` to pass on each gecko tool compile invocation")
	...
	if mask&OmitModCommonFlags == 0 {
		...
		// This only affects 'gecko get' in GOPATH mode, but add the flag anyway for
		// consistency.
	...
	if mask&(OmitBuildOnlyFlags|OmitJSONFlag) == 0 {
		...
		cmd.Flag.BoolVar(&cfg.BuildJSON, "json", false, "emit build output in JSON suitable for automated processing; see 'gecko help buildjson'")
	}
	cmd.Flag.Var(&load.BuildLdflags, "ldflags", "`arguments` to pass on each gecko tool link invocation")
	...
	cmd.Flag.Var((*base.StringsFlag)(&cfg.BuildToolexec), "toolexec", "a `program` to use to invoke toolchain programs like vet and asm; see 'gecko help build'")
...
func runBuild(ctx context.Context, cmd *base.Command, args []string) {
	...
	case "gccgo":
		if load.BuildGcflags.Present() {
			fmt.Println("gecko build: when using gccgecko toolchain, please pass compiler flags using -gccgoflags, not -gcflags")
		}
		if load.BuildLdflags.Present() {
			fmt.Println("gecko build: when using gccgecko toolchain, please pass linker flags using -gccgoflags, not -ldflags")
		}
	case "gc":
		if load.BuildGccgoflags.Present() {
			fmt.Println("gecko build: when using gc toolchain, please pass compile flags using -gcflags, and linker flags using -ldflags")
		}
	}
	...
	if cfg.BuildO != "" {
		...
		if fi, err := os.Stat(cfg.BuildO); (err == nil && fi.IsDir()) ||
			...
			if !explicitO {
				base.Fatalf("gecko: build output %q already exists and is a directory", cfg.BuildO)
			}
			a := &Action{Mode: "gecko build"}
			for _, p := range pkgs {
				if p.Name != "main" {
					continue
				}
			...
			if len(a.Deps) == 0 {
				base.Fatalf("gecko: no main packages to build")
			}
			b.Do(ctx, a)
			return
		}
		if len(pkgs) > 1 {
			base.Fatalf("gecko: cannot write multiple packages to non-directory %s", cfg.BuildO)
		} else if len(pkgs) == 0 {
...
var CmdInstall = &base.Command{
	UsageLine: "gecko install [build flags] [packages]",
...
func runInstall(ctx context.Context, cmd *base.Command, args []string) {
	...
	if cfg.ModulesEnabled && !moduleLoader.HasModRoot() {
		...
		if haveErrors && allMissingErrors {
			...
			base.Fatalf("gecko: 'gecko install' requires a version when current directory is not in a module\n\tTry 'gecko install %s' to install the latest version", hint)
		}
	}
	load.CheckPackageErrors(pkgs)
...
func InstallPackages(ld *modload.Loader, ctx context.Context, patterns []string, pkgs []*load.Package) {
	...
	// Success. If this command is 'gecko install' with no arguments
	// and the current directory (the implicit argument) is a command,
	// remove any leftover command binary from a previous 'gecko build'.
	...
	// One way to view this behavior is that it is as if 'gecko install' first
	// runs 'gecko build' and the moves the generated file to the install dir.
	// See issue 9645.
	if len(patterns) == 0 && len(pkgs) == 1 && pkgs[0].Name == "main" {
		// Compute file 'gecko build' would have created.
...
// installOutsideModule implements 'gecko install pkg@version'. It builds and
