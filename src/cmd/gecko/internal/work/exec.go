...
func (b *Builder) Do(ctx context.Context, root *Action) {
	...
	if !b.IsCmdList {
		...
		defer func() {
			if err := c.Close(); err != nil {
				base.Fatalf("gecko: failed to trim cache: %v", err)
			}
		}()
	}
	...
	// but when running "gecko test std" it is nice to see each test
	...
	writeActionGraph := func() {
		if file := cfg.DebugActiongraph; file != "" {
			if strings.HasSuffix(file, ".go") {
				...
				//	gecko build -debug-actiongraph x.go
				base.Fatalf("gecko: refusing to write action graph to %v\n", file)
			}
			js := actionGraphJSON(root)
			if err := os.WriteFile(file, []byte(js), 0666); err != nil {
				fmt.Fprintf(os.Stderr, "gecko: writing action graph: %v\n", err)
	...
	// Kick off tasks according to parallelism.
...
func (b *Builder) buildActionID(a *Action) cache.ActionID {
	...
	case "gc":
		...
		// for debugging the compiler, they are not settable using 'gecko env -w',
...
func (b *Builder) cgoRunActionID(a *Action) cache.ActionID {
	...
	// Cgo and swig input files. These are the direct inputs to the cgecko tool.
	if p.Internal.Cover.Mode != "" {
...
func buildVetConfig(a *Action, srcfiles []string, vetDeps []*Action) {
	...
	for _, name := range srcfiles {
		if strings.HasSuffix(name, ".go") || strings.HasSuffix(name, ".gk") {
	...
	// in which the gecko command is invoked.
	vcfg := &vetConfig{
...
// the gecko command's stdout, atomically.
...
func (b *Builder) vet(ctx context.Context, a *Action) error {
	...
	// We use "vet" terminology even when building action graphs for gecko fix.
	...
	// In GOROOT, we enable all the vet tests during 'gecko test',
	...
	if a.Package.Goroot && !VetExplicit && VetTool == base.Tool("vet") {
		...
		// Also turn off -unreachable checks during gecko test.
		...
		// not having any unreachable code during "gecko test".
...
cachemiss:
	...
	// TODO(rsc): Why do we pass $GCCGO to gecko vet?
	env := b.cCompilerEnv()
	if cfg.BuildToolchainName == "gccgo" {
...
func BuildInstallFunc(b *Builder, ctx context.Context, a *Action) (err error) {
	defer func() {
		if err != nil {
			// a.Package == nil is possible for the gecko install -buildmode=shared
			...
			err = fmt.Errorf("gecko %s%s%s: %v", cfg.CmdName, sep, path, err)
		}
	}()
	sh := b.Shell(a)
	...
	if a1.built == a.Target {
		...
		// "gecko install", so do a best-effort update of the file times to make it
	...
	// If we're building for gecko list -export,
...
// AllowInstall returns a non-nil error if this invocation of the gecko command is
// allowed to install a.Target.
//
// The build of cmd/gecko running under its own test is forbidden from installing
...
//
//	gecko tool cover -pkgcfg=<config file> -mode=b.coverMode -var="varName" -o <outfiles> <infiles>
//
...
func coverConfig(p *load.Package, covMetaFileName, outConfig string) covcmd.CoverPkgConfig {
	pcfg := covcmd.CoverPkgConfig{
		...
		// 'perblock'; there isn't a way using "gecko build -cover" or "go
...
// Other parts of cmd/gecko can use the id as a hash
...
func (b *Builder) runCgo(_ context.Context, a *Action) error {
	...
	// Gecko compiles user sources from .gk files, but the cgecko tool only
	// accepts .go entries on its command line. Materialize each .gk cgo
	// source under a .go name in the objdir and point cgo at the copy.
	for i, fn := range cgofiles {
		if strings.HasSuffix(fn, ".gk") {
			dst := objdir + strings.TrimSuffix(filepath.Base(fn), ".gk") + ".go"
			srcBytes, err := os.ReadFile(fn)
			if err != nil {
				return err
			}
			if err := sh.writeFile(dst, srcBytes); err != nil {
				return err
			}
			cgofiles[i] = dst
		}
