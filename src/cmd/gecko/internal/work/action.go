...
type Builder struct {
	...
	IsCmdList           bool // running as part of gecko list; set p.Stale and additional fields below
...
func actionGraphJSON(a *Action) string {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "gecko: writing debug action graph: %v\n", err)
		return ""
	}
	return string(js)
}
...
func NewBuilder(workDir string, getVendorDir func() string) *Builder {
	...
	for _, tag := range cfg.BuildContext.BuildTags {
		if strings.Contains(tag, ",") {
			fmt.Fprintf(os.Stderr, "gecko: -tags space-separated list contains comma\n")
			base.SetExitStatus(2)
			base.Exit()
		}
	}
...
func (b *Builder) Close() error {
	...
	if b.WorkDir != wd.(string) {
		base.Errorf("gecko: internal error: Builder WorkDir unexpectedly changed from %s to %s", wd, b.WorkDir)
	}
...
func closeBuilders() {
	...
	if leakedBuilders > 0 && base.GetExitStatus() == 0 {
		fmt.Fprintf(os.Stderr, "gecko: internal error: Builder leaked on successful exit\n")
...
//
// NewObjdir must be called only from a single task at a time,
...
// AutoAction returns the "right" action for gecko build or gecko install of p.
...
// runCgoActor implements the Actor interface for running the cgecko command for the package.
...
func (b *Builder) CompileAction(mode, depMode BuildMode, p *load.Package) *Action {
	...
	a := b.cacheAction("build", p, func() *Action {
		...
		// The cover action always runs in the same gecko build invocation as the build,
		...
		if p.Internal.Cover.Mode != "" {
			coverAction = b.cacheAction("cover", p, func() *Action {
				return &Action{
					Mode:    "cover",
					Package: p,
					Actor:   ActorFunc((*Builder).runCover),
					Objdir:  a.Objdir,
				}
			})
...
func (b *Builder) cgoAction(p *load.Package, objdir string, deps []*Action, hasCover bool) *Action {
	cgoCollectAction := b.cacheAction("cgo collect", p, func() *Action {
		// Run cgo
		runCgo := b.cacheAction("cgecko run", p, func() *Action {
			return &Action{
				Package: p,
				Mode:    "cgecko run",
				Actor:   &runCgoActor{},
		...
		// Determine which files swig will produce in the cgecko run action. We'll need to create
		...
		for _, fn := range slices.Concat(cgoFiles, swigGo) {
			cfiles = append(cfiles, strings.TrimSuffix(strings.TrimSuffix(filepath.Base(fn), ".go"), ".gk")+".cgo2.c")
		}
		...
		return &Action{
			...
			Actor: ActorFunc(func(b *Builder, ctx context.Context, a *Action) error {
				// Use the cgecko run action's provider as our provider output,
...
// VetAction returns the action for running gecko vet on package p.
...
func (b *Builder) LinkAction(s *modload.Loader, mode, depMode BuildMode, p *load.Package) *Action {
	...
	a := b.cacheAction("link", p, func() *Action {
		...
		// Because we run the temporary file in 'gecko run' and 'gecko test',
...
func (b *Builder) addTransitiveLinkDeps(s *modload.Loader, a, a1 *Action, shlib string) {
	...
	// If this is gecko build -linkshared, then the link depends on the shared libraries
...
// buildmodeShared takes the "gecko build" action a1 into the building of a shared library of a1.Deps.
// That is, the input a1 represents "gecko build pkgs" and the result represents "gecko build -buildmode=shared pkgs".
...
func (b *Builder) linkSharedAction(s *modload.Loader, mode, depMode BuildMode, shlib string, a1 *Action) *Action {
	...
	a := b.cacheAction("build-shlib "+shlib, nil, func() *Action {
		...
		a := &Action{
			Mode:    "gecko build -buildmode=shared",
			Package: p,
			Objdir:  b.NewObjdir(),
	...
	if (mode == ModeInstall || mode == ModeBuggyInstall) && a.Actor != nil {
		...
		a = b.cacheAction("install-shlib "+shlib, nil, func() *Action {
			...
			a := &Action{
				Mode:   "gecko install -buildmode=shared",
				Objdir: buildAction.Objdir,
				Actor:  ActorFunc(BuildInstallFunc),
				Deps:   []*Action{buildAction},
				Target: target,
