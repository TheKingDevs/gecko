...
// used by the build artifact cache (see cmd/gecko/internal/cache), but
...
// binaries. For a binary, like cmd/geckofmt, there are two steps: compile
// cmd/geckofmt/*.go into main.a, and then link main.a into the gofmt binary.
...
func (b *Builder) toolID(name string) string {
	return b.toolIDCache.Do(name, func() string {
		...
		if err := cmd.Run(); err != nil {
			...
			base.Fatalf("gecko: error obtaining buildID for %s: %v", desc, err)
		}

		line := stdout.String()
		f := strings.Fields(line)
		if len(f) < 3 || f[0] != name && path != VetTool || f[1] != "version" || strings.Contains(f[2], "devel") && !strings.HasPrefix(f[len(f)-1], "buildID=") {
			base.Fatalf("gecko: parsing buildID from %s -V=full: unexpected output:\n\t%s", desc, line)
		}
...
func (b *Builder) useCache(a *Action, actionHash cache.ActionID, target string, printOutput bool) (ok bool) {
	...
	// we only cache executables produced for 'gecko run' (and soon, for 'gecko tool').
	//
