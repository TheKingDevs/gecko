...
// the gecko command. They may be different because gecko tool <tool> for builtin
...
func BuildInit(ld *modload.Loader) {
	if buildInitStarted {
		base.Fatalf("gecko: internal error: work.BuildInit called more than once")
	}
	...
	if cfg.BuildPkgdir != "" && !filepath.IsAbs(cfg.BuildPkgdir) {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "gecko: evaluating -pkgdir: %v\n", err)
	...
	if cfg.BuildP <= 0 {
		base.Fatalf("gecko: -p must be a positive integer: %v\n", cfg.BuildP)
	}
	...
	for _, key := range []string{"CC", "CXX", "FC"} {
		...
		if err != nil {
			base.Fatalf("gecko: %s environment variable could not be parsed: %v", key, err)
		}
		if len(args) == 0 {
			continue
		}
		...
		if !filepath.IsAbs(path) && path != filepath.Base(path) {
			base.Fatalf("gecko: %s environment variable is relative; must be absolute path: %s\n", key, path)
		}
	}
...
// instrumentation is added. 'gecko test -fuzz' still works without coverage,
...
func instrumentInit() {
	...
	if cfg.BuildRace && cfg.BuildMSan {
		fmt.Fprintf(os.Stderr, "gecko: may not use -race and -msan simultaneously\n")
		base.SetExitStatus(2)
		base.Exit()
	}
	if cfg.BuildRace && cfg.BuildASan {
		fmt.Fprintf(os.Stderr, "gecko: may not use -race and -asan simultaneously\n")
		base.SetExitStatus(2)
		base.Exit()
	}
	if cfg.BuildMSan && cfg.BuildASan {
		fmt.Fprintf(os.Stderr, "gecko: may not use -msan and -asan simultaneously\n")
		base.SetExitStatus(2)
		base.Exit()
	}
	...
	if !cfg.BuildContext.CgoEnabled && (cfg.Goos != "darwin" || cfg.BuildASan || cfg.BuildMSan) {
		if runtime.GOOS != cfg.Goos || runtime.GOARCH != cfg.Goarch {
			fmt.Fprintf(os.Stderr, "gecko: %s requires cgo\n", modeFlag)
		} else {
			fmt.Fprintf(os.Stderr, "gecko: %s requires cgo; enable cgo by setting CGO_ENABLED=1\n", modeFlag)
		}
...
// compilerVersion detects the version of $(gecko env CC).
// It returns a non-nil error if the compiler matches a known version schema but
// the version could not be parsed, or if $(gecko env CC) could not be determined.
...
func compilerRequiredAsanVersion() error {
	...
	if err != nil {
		return fmt.Errorf("-asan: the version of $(gecko env CC) could not be parsed")
	}
