...
// IsGoFileName reports whether name is a Go source file accepted by gecko.
// Gecko reads user source files with the ".gk" extension, while the Go
// standard library it is built on remains written in ".go"; both contain
// Go syntax and are treated the same way by the build.
...
func IsGoDir(name string) (bool, error) {
	...
	var firstErr error
	for _, d := range fis {
		if d.IsDir() || !IsGoFileName(d.Name()) {
			continue
		}
		if d.Type().IsRegular() {
			return true, nil
