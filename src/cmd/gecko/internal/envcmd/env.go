...
// Package envcmd implements the “gecko env” command.
package envcmd
...
var CmdEnv = &base.Command{
	UsageLine: "gecko env [-json] [-changed] [-u] [-w] [var ...]",
	Short:     "print gecko environment information",
	Long: `
...
if one has been set with 'gecko env -w'.
...
For more about environment variables, see 'gecko help environment'.
	`,
}
...
var (
	...
	envU       = CmdEnv.Flag.Bool("u", false, "unsets the default setting for the named environment variables, if one has been set with 'gecko env -w'")
...
func MkEnv() []cfg.EnvVar {
	...
	env := []cfg.EnvVar{
		...
		// GOPACKAGESDRIVER isn't read or used by cmd/gecko, so it can only
...
func ExtraEnvVarsCostly(ld *modload.Loader) []cfg.EnvVar {
	...
	if err != nil {
		...
		fmt.Fprintf(os.Stderr, "gecko: invalid cflags: %v\n", err)
		return nil
	}
	cmd := b.GccCmd(".", "")
...
func runEnv(ctx context.Context, cmd *base.Command, args []string) {
	...
	if *envJson && *envU {
		base.Fatalf("gecko: cannot use -json with -u")
	}
	if *envJson && *envW {
		base.Fatalf("gecko: cannot use -json with -w")
	}
	if *envU && *envW {
		base.Fatalf("gecko: cannot use -u with -w")
	}

	// Handle 'gecko env -w' and 'gecko env -u' before calling buildcfg.Check,
	...
	for _, arg := range args {
		if strings.Contains(arg, "=") {
			base.Fatalf("gecko: invalid variable name %q (use -w to set variable)", arg)
		}
	}
	...
	needCostly := false
	if len(args) == 0 {
		// We're listing all environment variables ("gecko env"),
...
func runEnvW(args []string) {
	...
	if len(args) == 0 {
		base.Fatalf("gecko: no KEY=VALUE arguments given")
	}
	...
	for _, arg := range args {
		...
		if !found {
			base.Fatalf("gecko: arguments must be KEY=VALUE: invalid argument: %s", arg)
		}
		if err := checkEnvWrite(key, val); err != nil {
			base.Fatal(err)
		}
		if _, ok := add[key]; ok {
			base.Fatalf("gecko: multiple values for key: %s", key)
		}
		add[key] = val
		if osVal := osEnv[key]; osVal != "" && osVal != val {
			fmt.Fprintf(os.Stderr, "warning: gecko env -w %s=... does not override conflicting OS environment variable\n", key)
		}
	}
...
func runEnvU(args []string) {
	// Process and sanity-check command line.
	if len(args) == 0 {
		base.Fatalf("gecko: 'gecko env -u' requires an argument")
	}
	...
	for _, e := range env {
		if e.Name != "TERM" {
			if runtime.GOOS != "plan9" && bytes.Contains([]byte(e.Value), []byte{0}) {
				base.Fatalf("gecko: internal error: encountered null byte in environment variable %s on non-plan9 platform", e.Name)
			}
			...
			case "windows":
				if hasNonGraphic(e.Value) {
					base.Errorf("gecko: stripping unprintable or unescapable characters from %%%q%%", e.Name)
				}
...
func checkEnvWrite(key, val string) error {
	...
	if !cfg.CanGetenv(key) {
		return fmt.Errorf("unknown gecko command variable %s", key)
	}

	// Some variables can only have one of a few valid values. If set to an
	// invalid value, the next cmd/gecko invocation might fail immediately,
...
func readEnvFileLines(mustExist bool) []string {
	...
	if file == "" {
		if mustExist {
			base.Fatalf("gecko: cannot find gecko env config: %v", err)
		}
		return nil
	}
	data, err := os.ReadFile(file)
	if err != nil && (!os.IsNotExist(err) || mustExist) {
		base.Fatalf("gecko: reading gecko env config: %v", err)
	}
...
func updateEnvFile(add map[string]string, del map[string]bool) {
	...
	// Add variables (gecko env -w). Update existing lines in file if present, add to end otherwise.
	...
	// Delete requested variables (gecko env -u).
	for key := range del {
		if p, ok := prev[key]; ok {
			lines[p] = ""
		}
	...
	if err != nil {
		...
		err = os.WriteFile(file, data, 0666)
		if err != nil {
			base.Fatalf("gecko: writing gecko env config: %v", err)
		}
	}
}
