...
import (
	...
	"cmd/gecko/internal/modindex"
	"cmd/gecko/internal/modinfo"
...
type Package struct {
	PackagePublic                 // visible in 'gecko list'
...
type PackagePublic struct {
	...
	Stale       bool   `json:",omitempty"` // would 'gecko install' do anything for this package?
...
// and because the eventual real load of the test imports (during 'gecko test')
...
const (
	...
	// ResolveModule is for download (part of "gecko get") and indicates
	// that the module adjustment should be done, but not vendor adjustment.
	ResolveModule

	// GetTestDeps is for download (part of "gecko get") and indicates
...
func loadPackageData(ld *modload.Loader, ctx context.Context, path, parentPath, parentDir, parentRoot string, parentIsStd bool, mode int) (bp *build.Package, loaded bool, err error) {
	...
	// so that 'gecko list -e' reports something useful.
	...
	p, err := packageDataCache.Do(r.path, func() (*build.Package, error) {
		...
		Happy:
			...
			maybeExtendGOPATHForGecko()
			data.p, data.err = cfg.BuildContext.Import(r.path, parentDir, buildMode)
			if data.err != nil && !cfg.ModulesEnabled {
				// Gecko projects need no src/ directory: a local package may
				// live at the project root (as a directory or a single .gk
				// file) instead of only under <root>/src.
				if p, ok := geckoLookupProjectPackage(cfg.BuildContext, r.path, parentDir, buildMode); ok {
					data.p, data.err = p, nil
				}
			}
		}
...
// preloadWorkerCount is the number of concurrent tasks that can load
...
// to ensure preload tasks are no longer active. This is necessary
// because of global mutable state that cannot safely be read and written
// concurrently. In particular, packageDataCache may be cleared by "gecko get"
...
// or .gk (the source extensions gecko accepts).
// For a vendor check we must exclude directories that contain no source files.
// Otherwise it is not possible to vendor just a/b/c and still import the
// non-vendored a/b. See golang.org/issue/13832.
func hasGoFiles(dir string) bool {
	files, _ := os.ReadDir(dir)
	for _, f := range files {
		if !f.IsDir() && fsys.IsGoFileName(f.Name()) {
			return true
		}
	}
	return false
...
func InstallTargetDir(p *Package) TargetDir {
	if p.Goroot && strings.HasPrefix(p.ImportPath, "cmd/") && p.Name == "main" {
		switch p.ImportPath {
		case "cmd/gecko", "cmd/fmt", "cmd/gpm":
			return ToBin
		}
		return ToTool
	}
...
func (p *Package) exeFromFiles() string {
	...
	return strings.TrimSuffix(elem, filepath.Ext(elem))
}
...
func (p *Package) load(ld *modload.Loader, ctx context.Context, opts PackageOpts, path string, stk *ImportStack, importPos []token.Position, bp *build.Package, err error) {
	...
	// after encountering some errors so that 'gecko list -e' has more complete
	...
	if useBindir {
		...
		// gccgo has a preinstalled standard library that cmd/gecko cannot rebuild.
		p.Target = ""
	} else {
	...
	// src/cmd/gecko/internal/modcmd/vendor.go
	...
	if p.Error == nil && p.Name == "main" && !p.Internal.ForceLibrary && !p.Incomplete && !opts.SuppressBuildInfo {
		...
		// Consider starting this as a background task and retrieving the result
...
func PackagesAndErrors(ld *modload.Loader, ctx context.Context, opts PackageOpts, patterns []string) []*Package {
	...
	for _, p := range patterns {
		...
		if strings.HasSuffix(p, ".go") || strings.HasSuffix(p, ".gk") {
...
func mainPackagesOnly(pkgs []*Package, matches []*search.Match) []*Package {
	...
	for _, m := range matches {
		...
		if !foundMain {
			fmt.Fprintf(os.Stderr, "gecko: warning: %q matched only non-main packages\n", m.Pattern())
		}
	}
...
// isToolchainSourceFile reports whether f (which may be relative to the
// working directory) is a .go file belonging to the gecko toolchain's own
// tree (GOROOT/src standard library and GOROOT/test regression tests), which
// remains written in upstream Go source form. Gecko rejects explicit .go
// files from user code but must keep supporting its own internals.
func isToolchainSourceFile(f string) bool {
	if !strings.HasSuffix(f, ".go") {
		return false
	}
	abs, err := filepath.Abs(f)
	if err != nil {
		abs = filepath.Join(base.Cwd(), f)
	}
	return str.HasFilePathPrefix(abs, cfg.GOROOT)
}
	...
	for _, f := range gofiles {
		...
		if strings.HasSuffix(f, ".go") {
			...
			pkg.Error = &PackageError{
				Err: fmt.Errorf("gecko does not compile .go files; use the .gk extension: %s", pkg.Name),
			}
			pkg.Incomplete = true
			return pkg
		}
		if !strings.HasSuffix(f, ".gk") {
			pkg := new(Package)
			pkg.Internal.Local = true
			pkg.Internal.CmdlineFiles = true
			pkg.Name = f
			pkg.Error = &PackageError{
				Err: fmt.Errorf("named files must be .gk files: %s", pkg.Name),
			}
			pkg.Incomplete = true
			return pkg
		}
...
// parent directory, if there is one. This is used in the implementation of 'gecko
...
// PrepareForCoverageBuild is a helper invoked for "gecko install
// -cover", "gecko run -cover", and "gecko build -cover" (but not used by
// "gecko test -cover"). It walks through the packages being built (and
