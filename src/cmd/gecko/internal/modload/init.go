...
import (
	...
	"cmd/gecko/internal/fips140"
	"cmd/gecko/internal/fsys"
	"cmd/gecko/internal/gover"
	"cmd/gecko/internal/lockedfile"
	"cmd/gecko/internal/modfetch"
...
var (
	...
	// multiple times (for example, 'gecko get' calls LoadPackages multiple times) or
	// if it needs some other operation to be successful before go.mod and go.sum
	// can be written (for example, 'gecko mod download' must download modules before
...
func (mms *MainModuleSet) GoVersion(ld *Loader) string {
	...
	if mms != nil && len(mms.versions) == 1 {
		...
		if f == nil {
			// Special case: we are outside a module, like 'gecko run x.go'.
...
// for example 'gecko mod tidy', that don't operate in workspace mode.
...
func Init(ld *Loader) {
	...
	// Gecko projects never use go.mod; local packages resolve from the
	// project's src directory as if they were the first GOPATH entry.
	// Force GOPATH mode in such projects so directory-mode commands do not
	// fail with "go.mod file not found". The load package extends GOPATH
	// itself when it resolves packages.
	if geckoProjectRoot(base.Cwd()) != "" && findModuleRoot(base.Cwd()) == "" {
		return
	}
	...
	if ld.modRoots != nil {
		// modRoot set before Init was called ("gecko mod init" does this).
		// No need to search for go.mod.
	} else if ld.RootMode == NoRoot {
		if cfg.ModFile != "" && !base.InGOFLAGS("-modfile") {
			base.Fatalf("gecko: -modfile cannot be used with commands that ignore the current module")
		}
		ld.modRoots = nil
	} else if ld.workFilePath != "" {
		// We're in workspace mode, which implies module mode.
		if cfg.ModFile != "" {
			base.Fatalf("gecko: -modfile cannot be used in workspace mode")
		}
	} else {
		if modRoot := findModuleRoot(base.Cwd()); modRoot == "" {
			if cfg.ModFile != "" {
				base.Fatalf("gecko: cannot find main module, but -modfile was set.\n\t-modfile cannot be used to set the module root directory.")
			}
			...
			fmt.Fprintf(os.Stderr, "gecko: warning: ignoring go.mod in system temp root %v\n", os.TempDir())
			if ld.RootMode == NeedRoot {
				base.Fatal(NewNoMainModulesError(ld))
			}
			if !mustUseModules {
	...
	if cfg.ModFile != "" && !strings.HasSuffix(cfg.ModFile, ".mod") {
		base.Fatalf("gecko: -modfile=%s: file does not have .mod extension", cfg.ModFile)
	}
...
// of 'gecko get', but Init reads the -modfile flag in 'gecko get', so it shouldn't
...
func die(ld *Loader) {
	...
	if !ld.inWorkspaceMode() {
		if dir, name := findAltConfig(base.Cwd()); dir != "" {
			...
			base.Fatalf("gecko: cannot find main module, but found %s in %s\n\tto create a module there, run:\n\t%sgecko mod init", name, dir, cdCmd)
		}
	}
	base.Fatal(NewNoMainModulesError(ld))
}
...
func loadModFile(ld *Loader, ctx context.Context, opts *PackageOpts) (*Requirements, error) {
	...
	if ld.inWorkspaceMode() {
		...
		//
		// Commands like 'gecko build', 'gecko run', 'gecko list' have no go.mod file to
		...
		// For example, 'gecko get' does this, since it is expected to resolve paths.
		//
		...
		if err != nil {
			if ld.inWorkspaceMode() {
				if tooNew, ok := err.(*gover.TooNewError); ok && !strings.HasPrefix(cfg.CmdName, "work ") {
					...
					// Report this more specific error, unless we are a command like 'gecko work use'
					// or 'gecko work sync', which will fix the problem after the caller sees the TooNewError
	...
	if ld.MainModules.Index(mainModule).goVersion == "" && rs.pruning != workspace {
		...
		if cfg.BuildMod == "mod" && cfg.CmdName != "mod graph" && cfg.CmdName != "mod why" {
			...
			// We need to add a 'gecko' version to the go.mod file, but we must assume
...
func CreateModFile(ld *Loader, ctx context.Context, modPath string) {
	...
	if _, err := fsys.Stat(modFilePath); err == nil {
		base.Fatalf("gecko: %s already exists", modFilePath)
	}
	...
	if cfg.ModFile != "" {
		fmt.Fprintf(os.Stderr, "gecko: creating new go.mod (using -modfile path %s): module %s\n", base.ShortPath(modFilePath), modPath)
	} else {
		fmt.Fprintf(os.Stderr, "gecko: creating new go.mod: module %s\n", modPath)
	}
	...
	// Suggest running 'gecko mod tidy' unless the project is empty. Even if we
	...
	for _, f := range files {
		...
		if fsys.IsGoFileName(name) || f.IsDir() {
			empty = false
			break
		}
	}
	if !empty {
		fmt.Fprintf(os.Stderr, "gecko: to add module requirements and sums:\n\tgecko mod tidy\n")
	}
}
...
// like 'gecko get' are expected to do this.
//
...
func makeMainModules(ld *Loader, ms []module.Version, rootDirs []string, modFiles []*modfile.File, indices []*modFileIndex, workFile *modfile.WorkFile) *MainModuleSet {
	...
	for _, m := range ms {
		if mainModulePaths[m.Path] {
			base.Errorf("gecko: module %s appears multiple times in workspace", m.Path)
		}
	...
	for _, r := range workFileReplaces {
		if mainModulePaths[r.Old.Path] && r.Old.Version == "" {
			base.Errorf("gecko: workspace module %v is replaced at all versions in the go.work file. To fix, remove the replacement from the go.work file or specify the version at which to replace the module.", r.Old.Path)
		}
	...
	for i, m := range ms {
		...
		if rel := search.InDir(rootDirs[i], cfg.GOROOTsrc); rel != "" {
			...
			if m.Path == "std" {
				...
				// treatment, so it is possible to run 'gecko test .' in other GOROOTs to
		...
		if modFiles[i] != nil {
			...
			for _, r := range modFiles[i].Replace {
				...
				if prev, ok := replacements[r.Old]; ok && !curModuleReplaces[r.Old] && prev != newV {
					base.Fatalf("gecko: conflicting replacements for %v:\n\t%v\n\t%v\nuse \"gecko work edit -replace %v=[override]\" to resolve", r.Old, prev, newV, r.Old)
				}
...
func rootsFromModFile(ld *Loader, m module.Version, modFile *modfile.File, addToolchainRoot addToolchainRoot) (roots []module.Version, direct map[string]bool) {
	...
	for _, r := range modFile.Require {
		if index := ld.MainModules.Index(m); index != nil && index.exclude[r.Mod] {
			if cfg.BuildMod == "mod" {
				fmt.Fprintf(os.Stderr, "gecko: dropping requirement on excluded version %s %s\n", r.Mod.Path, r.Mod.Version)
			} else {
				fmt.Fprintf(os.Stderr, "gecko: ignoring requirement on excluded version %s %s\n", r.Mod.Path, r.Mod.Version)
			}
			continue
		}
...
func appendGoAndToolchainRoots(roots []module.Version, goVersion, toolchain string, direct map[string]bool) []module.Version {
	...
	if toolchain != "" {
		...
		// automatically if the 'gecko' version is changed so that it implies the exact
		// same toolchain.
	}
	return roots
}
		...
		if fi, err := fsys.Stat(vendorDir); err == nil && fi.IsDir() {
			if goVersion != "" {
				if gover.Compare(goVersion, "1.14") < 0 {
					...
					if err != nil {
						base.Fatalf("gecko: reading modules.txt for vendor directory: %v", err)
					}
...
// include one. The 'gecko' version added, if any, is the latest version supported
// by this toolchain.
...
func forceGoStmt(modFile *modfile.File, mod module.Version, v string) {
	if err := modFile.AddGoStmt(v); err != nil {
		base.Fatalf("gecko: internal error: %v", err)
	}
...
func findModulePath(dir string) (string, error) {
	...
	for _, info := range list {
		if info.Type().IsRegular() && fsys.IsGoFileName(info.Name()) {
	...
	for _, info1 := range list {
		if info1.IsDir() {
			...
			for _, info2 := range files {
				if info2.Type().IsRegular() && fsys.IsGoFileName(info2.Name()) {
...
type WriteOpts struct {
	...
	// TODO(bcmills): Make 'gecko mod tidy' update the go version in the Requirements
	// instead of writing directly to the modfile.File
	TidyWroteGo bool // Go.Version field already updated by 'gecko mod tidy'
}
...
func UpdateGoModFromReqs(ld *Loader, ctx context.Context, opts WriteOpts) (before, after []byte, modFile *modfile.File, err error) {
	...
	if goVersion == "" {
		base.Fatalf("gecko: internal error: missing go root module in WriteGoMod")
	}
	...
	if len(tools) > 0 && gover.Compare(goVersion, gover.GoModToolVersion) < 0 && cfg.CmdName == "get" {
		if opts.ExplicitToolchain {
			return nil, nil, nil, errors.New(gover.GoModToolVersion + " is required for tool directives in go.mod: gecko get go@" + gover.GoModToolVersion + ".0")
		}
	...
	if opts.DropToolchain || toolchain == "go"+goVersion || (gover.Compare(toolVers, gover.GoStrictVersion) < 0 && !opts.ExplicitToolchain) {
		...
		// TODO(#57001): 'gecko get' should reject explicit toolchains below GoStrictVersion.
...
func commitRequirements(ld *Loader, ctx context.Context, opts WriteOpts) (err error) {
	...
	if !dirty && cfg.CmdName != "mod tidy" {
		...
		// 'gecko mod init' shouldn't write go.sum, since it will be incomplete.
	...
	// previous versions of the 'gecko' command from making simultaneous edits.
...
func keepSums(ld *Loader, ctx context.Context, pld *packageLoader, rs *Requirements, which whichSums) map[module.Version]bool {
	...
	if pld == nil {
		...
		for _, pkg := range pld.pkgs {
			...
			// minor, so we maintain the previous (buggy) behavior in 'gecko mod tidy' to
...
// geckoProjectRoot returns the directory of the gecko project containing dir,
// climbing the tree until the filesystem root. A directory is a gecko project
// root when it contains a gecko.json manifest; a go.mod found first in the
// climb yields "" so module mode keeps control (gecko projects forbid go.mod).
