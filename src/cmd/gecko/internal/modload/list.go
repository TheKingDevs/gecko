...
func ListModules(ld *Loader, ctx context.Context, args []string, mode ListMode, reuseFile string) ([]*modinfo.ModulePublic, error) {
	...
	if err == nil {
		...
		// where "gecko mod tidy" and "gecko list -m -u all" fight over whether the go.sum
...
func listModules(ld *Loader, ctx context.Context, rs *Requirements, args []string, mode ListMode, reuse map[module.Version]*modinfo.ModulePublic) (_ *Requirements, mods []*modinfo.ModulePublic, mgErr error) {
	...
	for _, arg := range args {
		if strings.Contains(arg, `\`) {
			base.Fatalf("gecko: module paths never use backslash")
		}
		if search.IsRelativePath(arg) {
			base.Fatalf("gecko: cannot use relative path %s to specify module", arg)
		}
		if arg == "all" || strings.Contains(arg, "...") {
			needFullGraph = true
			if !ld.HasModRoot() {
				base.Fatalf("gecko: cannot match %q: %v", arg, NewNoMainModulesError(ld))
			}
			continue
		}
		...
		if _, ok := rs.rootSelected(ld, arg); !ok || rs.pruning == unpruned {
			...
			if mode&ListVersions == 0 && !ld.HasModRoot() {
				base.Fatalf("gecko: cannot match %q without -versions or an explicit version: %v", arg, NewNoMainModulesError(ld))
			}
		}
	}
	...
	for _, arg := range args {
		...
		if found {
			...
			if IsRevisionQuery(path, vers) || mode&ListRetracted != 0 {
				...
				// specific revision or used 'gecko list -retracted'.
				allowed = nil
			}
