...
// separate from the loading operation, such as during "gecko get"
// upgrades/downgrades or in "gecko mod" operations.
...
//
// Because "gecko mod vendor" prunes out the tests of vendored packages, the
...
// ("all" in Go 1.16+ and "gecko mod vendor" in Go 1.11+).
//
...
//
// If the main module is “tidy” (that is, if "gecko mod tidy" is a no-op for it)
...
type PackageOpts struct {
	...
	// cmd/gecko/internal/imports package).
	...
	// 'gecko' version listed in the go.mod file.
	...
	// loaded.
	//
	// AllowPackage may be invoked concurrently by multiple tasks,
	...
	// running "gecko mod vendor" (or building with "-mod=vendor").
	//
	// This is a no-op for modules that declare 'gecko 1.16' or higher, for which this
		...
		for _, m := range matches {
			...
			case strings.Contains(m.Pattern(), "..."):
				...
				if err != nil {
					...
					// modules. However, for cases like 'gecko list -e', the error should not
...
type packageLoader struct {
	...
	// for imported packages (as in 'gecko mod tidy' in Go 1.17–1.20).
...
func loadFromRoots(ld *Loader, ctx context.Context, params loaderParams) *packageLoader {
	...
	if pld.requirements.pruning == unpruned {
		...
		// behavior of the 'gecko' command and keeps the go.mod file more consistent in
...
func (pld *packageLoader) resolveMissingImports(ld *Loader, ctx context.Context) (modAddedBy map[module.Version]*loadPkg, err error) {
	...
	if maxTooNew != nil {
		fmt.Fprintf(os.Stderr, "gecko: toolchain upgrade needed to resolve %s\n", maxTooNewPkg.path)
	...
	for _, pm := range pkgMods {
		...
		fmt.Fprintf(os.Stderr, "gecko: found %s in %s %s\n", pkg.path, mod.Path, mod.Version)
...
func (pld *packageLoader) stdVendor(ld *Loader, parentPath, path string) string {
	...
	if str.HasPathPrefix(parentPath, "cmd") {
		...
		// globally from GOROOT/src/vendor (and are listed as part of 'gecko list std')
		...
		// 'gecko mod tidy' and similar module commands when working within std.)
...
func (pld *packageLoader) checkTidyCompatibility(ld *Loader, ctx context.Context, rs *Requirements, compatVersion string) {
	...
	mismatches := <-mismatchMu
	if len(mismatches) == 0 {
		// Since we're running as part of 'gecko mod tidy', the roots of the module
		...
		for _, m := range pld.requirements.rootModules {
			if v := mg.Selected(m.Path); v != m.Version {
				fmt.Fprintln(os.Stderr)
				base.Fatalf("gecko: internal error: failed to diagnose selected-version mismatch for module %s: go %s selects %s, but go %s selects %s\n\tPlease report this at https://go.dev/issue.", m.Path, goVersion, m.Version, compatVersion, v)
			}
		}
		return
	}
	...
	for _, pkg := range pld.pkgs {
		...
		if pkg.isTest() {
			...
			if _, ok := mismatches[pkg.testOf]; !ok {
				base.Fatalf("gecko: internal error: mismatch recorded for test %s, but not its non-test package", pkg.path)
			}
			continue
		}
		...
		default:
			base.Fatalf("gecko: internal error: mismatch recorded for package %s, but no differences found", pkg.path)
		}
	}
...
// These are so old that they predate "gecko get" and did not use URL-like paths.
...
// during "gecko vendor", we look into "// +build appengine" files and
...
// why returns the text to use in "gecko mod why" output about the given package.
...
// Why returns the "gecko mod why" output stanza for the given package,
