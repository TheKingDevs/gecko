...
type Requirements struct {
	...
	// 'gecko mod tidy' or 'gecko mod vendor'.
	//
...
func mustHaveGoRoot(roots []module.Version) {
	...
	panic("gecko: internal error: missing go root module")
}
		...
		if rs.pruning == pruned {
			...
			for _, m := range vendorList {
				if v, ok := rs.rootSelected(ld, m.Path); !ok || v != m.Version {
					base.Errorf("gecko: vendored module %v should be required explicitly in go.mod", m)
					inconsistent = true
				}
			}
			if inconsistent {
	...
	if pruning == pruned {
		...
		readModGraphDebugOnce.Do(func() {
			for f := range strings.SplitSeq(os.Getenv("GODEBUG"), ",") {
				...
				case "lazymod=log":
					debug.PrintStack()
					fmt.Fprintf(os.Stderr, "gecko: read full module graph.\n")
				case "lazymod=strict":
					debug.PrintStack()
					base.Fatalf("gecko: read full module graph (forbidden by GODEBUG=lazymod=strict).")
				}
			}
		})
	}
...
func updatePrunedRoots(ld *Loader, ctx context.Context, direct map[string]bool, rs *Requirements, pkgs []*loadPkg, add []module.Version, rootsImported bool) (*Requirements, error) {
	...
	for _, pkg := range pkgs {
		...
		case rootsImported && pkg.flags.has(pkgFromRoot):
			...
			// roots as if they are imported by the main module (as in 'gecko get').
		...
		case pkg.flags.has(pkgIsRoot):
			...
			// 'gecko' command — such as 'gecko test' on the same package — to continue to
	...
	for {
		...
		for _, m := range rs.rootModules {
			if inRootPaths[m.Path] {
				...
				// 'require' line and then runs 'gecko mod tidy' to try to make everything
...
func tidyUnprunedRoots(ld *Loader, ctx context.Context, mainModule module.Version, old *Requirements, pkgs []*loadPkg) (*Requirements, error) {
	var (
		...
		// example.com/m/v2, then runs 'gecko mod tidy'. Tidy may delete the
