	...
	for _, r := range modFile.Require {
		if meta, ok := i.require[r.Mod]; !ok {
			...
			if cfg.BuildMod == "readonly" {
				...
				// after a "gecko mod tidy" — it's a good idea to run those before
...
func rawGoModSummary(ld *Loader, m module.Version) (*modFileSummary, error) {
	...
	if m.Version == "" && ld.inWorkspaceMode() && m.Path == "command-line-arguments" {
		// "gecko work sync" calls LoadModGraph to make sure the module graph is valid.
...
func rawGoModData(ld *Loader, m module.Version) (name string, data []byte, err error) {
	if m.Version == "" {
		...
		if !gover.ModIsValid(m.Path, m.Version) {
			...
			base.Fatalf("gecko: internal error: %s@%s: unexpected invalid semantic version", m.Path, m.Version)
		}
		name = "go.mod"
