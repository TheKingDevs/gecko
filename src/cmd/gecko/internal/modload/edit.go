...
// package within the module matching an explicit command line argument to 'gecko
// get', and the versions in tryUpgrade are transitive dependencies that are
// either being upgraded by 'gecko get -u' or being added to satisfy some
...
func editRequirements(ld *Loader, ctx context.Context, rs *Requirements, tryUpgrade, mustSelect []module.Version) (edited *Requirements, changed bool, err error) {
	...
	for _, m := range mustSelect {
		if m.Path == "go" {
			...
			//
			// TODO(bcmills): 'gecko get' ought to reject explicit toolchain versions
	...
	for _, r := range tryUpgrade {
		...
		if cfg.BuildV {
			fmt.Fprintf(os.Stderr, "gecko: trying upgrade to %v\n", r)
		}
	...
	//
	// For 'gecko mod tidy' we would do that using expandGraph, which upgrades the
	...
	//
	// 	1. If the user runs 'gecko get' with a set of mutually-compatible module
	...
	for rootsDirty && len(conflicts) == 0 {
		...
		for _, m := range roots {
			...
			if v, ok := mustSelectVersion[m.Path]; ok && v == m.Version {
				...
				// get graph pruning to take effect, but (a) it is likely that 'gecko mod
			...
			if conflict.Err != nil && origV == m.Version {
				...
				// 'gecko get -u ./...' to incidentally downgrade some dependency whose
		...
		if rootsDirty {
			if cfg.BuildV {
				...
				for _, m := range upgradedFrom {
					fmt.Fprintf(os.Stderr, "gecko: accepting indirect upgrade from %v to %s\n", m, selectedRoot[m.Path])
				}
			}
			continue
		}
	...
	// 'gecko mod tidy' to clean up the direct/indirect annotations.)
	//
	...
	if rootPruning == unpruned {
		if v, ok := edited.rootSelected(ld, "go"); ok && pruningForGoVersion(v) == pruned {
			...
			// the graph. 'gecko get' will check for that sort of transition and log a
