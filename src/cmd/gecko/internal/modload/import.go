...
func (e *ImportMissingError) Error() string {
	if e.Module.Path == "" {
		...
		if e.replaced.Path != "" {
			...
			return fmt.Sprintf("module %s provides package %s and is replaced but not required; to add it:\n\tgecko get %s", e.replaced.Path, e.Path, suggestArg)
		}
		...
		if e.ImportingMainModule.Path != "" && e.ImportingMainModule != e.modContainingCWD {
			return fmt.Sprintf("%s; to add it:\n\tcd %s\n\tgecko get %s", message, e.modRoot, e.Path)
		}
		return fmt.Sprintf("%s; to add it:\n\tgecko get %s", message, e.Path)
	}
...
// that suggests a 'gecko get' command for root packages that transitively import
...
func (e *ImportMissingSumError) Error() string {
	...
	if e.importer == "" {
		...
		// command line. Recommend 'gecko mod download' for the modules that could
		...
		if len(e.mods) > 0 {
			...
			hint = fmt.Sprintf("; to add:\n\tgecko mod download %s", strings.Join(args, " "))
		}
	} else {
		// Importing package is known (common case). Recommend 'gecko get' on the
		...
		hint = fmt.Sprintf("; to add:\n\tgecko get%s %s%s", tFlag, e.importer, version)
	}
	return message + hint
}
...
// allows 'gecko mod tidy' to preserve a minor checksum-preservation bug
// (https://go.dev/issue/56222) for modules with 'gecko' versions between 1.17 and
...
func importFromModules(ld *Loader, ctx context.Context, path string, rs *Requirements, mg *ModuleGraph, skipModFile bool) (m module.Version, modroot, dir string, altMods []module.Version, err error) {
	...
	if filepath.IsAbs(path) {
		return invalidf("%q is not a package path; see 'gecko help packages'", path)
	}
	if search.IsMetaPackage(path) {
		return invalidf("%q is not an importable package; see 'gecko help packages'", path)
	}
		...
		if ld.HasModRoot() {
			...
			if inVendorDir {
				...
				if ok || (gover.Compare(ld.MainModules.GoVersion(ld), gover.ExplicitModulesTxtImportVersion) < 0) {
					...
					fmt.Fprintf(os.Stderr, "gecko: ignoring package %s which exists in the vendor directory but is missing from vendor/modules.txt. To sync the vendor directory run gecko %s vendor.\n", path, subCommand)
				}
			}
		}
...
func queryImport(ld *Loader, ctx context.Context, path string, rs *Requirements) (module.Version, error) {
	...
	fmt.Fprintf(os.Stderr, "gecko: finding module for package %s\n", path)
