...
// Package vet implements the “gecko vet” and “gecko fix” commands.
package vet
...
var CmdVet = &base.Command{
	CustomFlags: true,
	UsageLine:   "gecko vet [build flags] [-vettool prog] [vet flags] [packages]",
...
can be built and run using these commands:
  ...
  gecko install golang.org/x/tools/go/analysis/passes/shadow/cmd/shadow@latest
  gecko vet -vettool=$(which shadow)

Alternative vet tools should be built atop golang.org/x/tools/go/analysis/unitchecker,
which handles the interaction with gecko vet.

The default vet tool is 'gecko tool vet' or cmd/vet.
For help on its checkers and their flags, run 'gecko tool vet help'.
For details of a specific checker such as 'printf', see 'gecko tool vet help printf'.

For more about specifying packages, see 'gecko help packages'.

The build flags supported by gecko vet are those that control package resolution
and execution, such as -C, -n, -x, -v, -tags, and -toolexec.
For more about these flags, see 'gecko help build'.

See also: gecko fmt, gecko fix.
	`,
}

var CmdFix = &base.Command{
	CustomFlags: true,
	UsageLine:   "gecko fix [build flags] [-fixtool prog] [fix flags] [packages]",
...
alternative or additional fixers; see the documentation for gecko vet's
-vettool flag for details.

The default fix tool is 'gecko tool fix' or cmd/fix.
For help on its fixers and their flags, run 'gecko tool fix help'.
For details of a specific fixer such as 'hostport', see 'gecko tool fix help hostport'.

For more about specifying packages, see 'gecko help packages'.

The build flags supported by gecko fix are those that control package resolution
and execution, such as -C, -n, -x, -v, -tags, and -toolexec.
For more about these flags, see 'gecko help build'.

See also: gecko fmt, gecko vet.
	`,
}
...
func run(ctx context.Context, cmd *base.Command, args []string) {
	...
	// gecko vet               =>      cmd/vet -json           Parse stdout, print diagnostics to stderr.
	// gecko vet -json         =>      cmd/vet -json           Pass stdout through.
	// gecko vet -fix [-diff]  =>      cmd/vet -fix [-diff]    Pass stdout through (and exit 1 if diffs).
	// gecko fix [-diff]       =>      cmd/fix -fix [-diff]    Pass stdout through (and exit 1 if diffs).
	// gecko fix -json         =>      cmd/fix -json           Pass stdout through.
	//
	// Notes:
	// * -diff requires "gecko vet -fix" or "gecko fix", and no -json.
	...
	if cmd.Name() == "fix" || *vetFixFlag {
		// fix mode: 'gecko fix' or 'gecko vet -fix'
		if jsonFlag {
			if diffFlag {
		...
		// vet mode: 'gecko vet' without -fix
		if !jsonFlag {
		...
		if diffFlag {
			base.Fatalf("gecko vet -diff flag requires -fix")
		}
	}

	// Implement legacy "gecko fix -fix=name,..." flag.
	if *fixFixFlag != "" {
		fmt.Fprintf(os.Stderr, "gecko %s: the -fix=%s flag is obsolete and has no effect\n", cmd.Name(), *fixFixFlag)

		// The buildtag fixer is now implemented by cmd/fix.
		if slices.Contains(strings.Split(*fixFixFlag, ","), "buildtag") {
			fmt.Fprintf(os.Stderr, "gecko %s: to enable the buildtag check, use -buildtag\n", cmd.Name())
		}
	}
	...
	root := &work.Action{Mode: "gecko " + cmd.Name()}
	...
	for _, p := range pkgs {
		...
		if len(ptest.GoFiles) == 0 && len(ptest.CgoFiles) == 0 && pxtest == nil {
			base.Errorf("gecko: can't %s %s: no Go files in %s", cmd.Name(), p.ImportPath, p.Dir)
			continue
		}
