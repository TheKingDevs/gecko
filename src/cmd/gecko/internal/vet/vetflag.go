...
// gecko vet/fix flag processing
var (
	// We query the flags of the tool specified by -{vet,fix}tool
	// and accept any of those flags plus any flag valid for 'gecko
	...
	//
	// The default behavior ("") runs 'gecko tool {vet,fix}'.
	//
...
func parseToolFlag(cmd *base.Command, args []string) string {
	...
	return base.Tool(cmd.Name()) // default to 'gecko tool vet|fix'
}
...
func toolFlags(cmd *base.Command, args []string) (passToTool, packageNames []string) {
	...
	if err := toolcmd.Run(); err != nil {
		fmt.Fprintf(os.Stderr, "gecko: %s -flags failed: %v\n", tool, err)
	...
	if err := json.Unmarshal(out.Bytes(), &analysisFlags); err != nil {
		fmt.Fprintf(os.Stderr, "gecko: can't unmarshal JSON from %s -flags: %v", tool, err)
		base.SetExitStatus(2)
		base.Exit()
	}
