...
// Package tool implements the “gecko tool” command.
package tool
...
var CmdTool = &base.Command{
	Run:       runTool,
	UsageLine: "gecko tool [-n] command [args...]",
	Short:     "run specified gecko tool",
	Long: `
Tool runs the gecko tool command identified by the arguments.

Go ships with a number of builtin tools, and additional tools
may be defined in the go.mod of the current module. 'gecko get -tool'
can be used to define additional tools in the current module's
go.mod file. See 'gecko help get' for more information.
...
For more about each builtin tool command, see 'gecko doc cmd/<command>'.
`,
}

var toolN bool

// Return whether tool can be expected in the gccgecko tool directory.
// Other binaries could be in the same directory so don't
// show those with the 'gecko tool' command.
...
func runTool(ctx context.Context, cmd *base.Command, args []string) {
	...
	if err != nil {
		if toolName == "dist" && len(args) > 1 && args[1] == "list" {
			...
			// place. However, 'gecko tool dist list' is useful for listing all supported
			// platforms.
			//
...
func listTools(ld *modload.Loader, ctx context.Context) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "gecko: no tool directory: %s\n", err)
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "gecko: can't read tool directory: %s\n", err)
		base.SetExitStatus(2)
		return
	}
	...
	for _, name := range names {
		...
		// in addition to gecko tools. Only display gecko tools here.
...
func impersonateDistList(args []string) (handled bool) {
	fs := flag.NewFlagSet("gecko tool dist list", flag.ContinueOnError)
	jsonFlag := fs.Bool("json", false, "produce JSON output")
	brokenFlag := fs.Bool("broken", false, "include broken ports")

	// The usage for 'gecko tool dist' claims that
	...
	if err := fs.Parse(args); err != nil || len(fs.Args()) > 0 {
		...
		// Force fallback to the real 'gecko tool dist'.
		return false
	}
...
func builtTool(runAction *work.Action) string {
	...
	if toolN {
		...
		// before this invocation of the gecko command: if the binary
...
func buildAndRunBuiltinTool(ld *modload.Loader, ctx context.Context, toolName, tool string, args []string) {
	...
	// the same GOOS and GOARCH as this gecko command.
	cfg.ForceHost()
...
func buildAndRunModtool(ld *modload.Loader, ctx context.Context, toolName, tool string, args []string) {
	runFunc := func(b *work.Builder, ctx context.Context, a *work.Action) error {
		// Use the ExecCmd to run the binary, as gecko run does. ExecCmd allows users
		...
		// Use same environment gecko run uses to start the executable:
...
func buildAndRunTool(ld *modload.Loader, ctx context.Context, tool string, args []string, runTool work.ActorFunc) {
	...
	a := &work.Action{Mode: "gecko tool", Actor: runTool, Args: args, Deps: []*work.Action{a1}}
	b.Do(ctx, a)
}
...
func runBuiltTool(toolName string, env, cmdline []string) error {
	...
	// and (*runTestActor).Act in cmd/gecko/internal/test. See #22220, #22315, #78204.
	...
	if err != nil {
		...
		if !ok || !e.Exited() || cfg.BuildX {
			fmt.Fprintf(os.Stderr, "gecko tool %s: %s\n", toolName, err)
		}
		if ok {
			n := e.ExitCode()
			if n == -1 {
