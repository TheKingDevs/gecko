...
var (
	...
	list        = flag.Bool("l", false, "list files whose formatting differs from fmt's")
	...
	// errors
	errFormattingDiffers = fmt.Errorf("formatting differs from fmt's")
)
...
const (
	...
	// and exponents while printing. See https://golang.org/doc/go1.13#fmt.
	//
	// This value is defined in go/printer specifically for go/format and cmd/fmt.
...
//
// f may run concurrently in a task, but its output to the passed-in
...
func processFile(filename string, info fs.FileInfo, in io.Reader, r *reporter) error {
	...
	mode := printerMode
	if strings.HasSuffix(filename, ".gk") {
		mode |= printer.GeckoParens | printer.GeckoColons // preserve mandatory gecko clause parens and ':' annotations
	}
...
func main() {
	...
	// call fmtMain in a separate function
...
func fmtMain(s *sequencer) {
	counter.Open()
	flag.Usage = usage
	flag.Parse()
	counter.Inc("fmt/invocations")
	counter.CountFlags("fmt/flag:", *flag.CommandLine)
...
func writeFile(filename string, orig, formatted []byte, perm fs.FileMode) error {
	...
	restoreFail := func(err error) {
		fmt.Fprintf(os.Stderr, "fmt: %s: error restoring file to original: %v; backup in %s\n", filename, err, bakname)
	}
