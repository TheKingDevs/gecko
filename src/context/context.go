...
//
// The same Context may be passed to functions running in different tasks;
// Contexts are safe for simultaneous use by multiple tasks.
//
...
// API boundaries.
//
// Context's methods may be called by multiple tasks simultaneously.
...
// A CancelFunc may be called by multiple tasks simultaneously.
...
// AfterFunc arranges to call f in its own task after ctx is canceled.
// If ctx is already canceled, AfterFunc calls f immediately in its own task.
//
...
// either the context is canceled and f has been started in its own task;
...
// tasks counts the number of tasks ever created; for testing.
