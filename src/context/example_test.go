...
// task leak. By the end of the example function, the task started
// by gen will return without leaking.
func ExampleWithCancel() {
	// gen generates integers in a separate task and
	// sends them to the returned channel.
	// The callers of gen need to cancel the context once
	// they are done consuming generated integers not to leak
	// the internal task started by gen.
	gen := func(ctx context.Context) <-chan int {
		...
		go func() {
			for {
				select {
				case <-ctx.Done():
					return // returning not to leak the task
				case dst <- n:
					n++
				}
			}
...
func ExampleAfterFunc_cond() {
	waitOnCond := func(ctx context.Context, cond *sync.Cond, conditionMet func() bool) error {
		stopf := context.AfterFunc(ctx, func() {
			...
			// If multiple tasks are waiting on cond simultaneously,
			// we need to make sure we wake up exactly this one.
			// That means that we need to Broadcast to all of the tasks,
			// which will wake them all up.
			//
			// If there are N concurrent calls to waitOnCond, each of the tasks
			// will spuriously wake up O(N) other tasks that aren't ready yet,
		...
		// Wait may unblock due to some other task's context being canceled,
