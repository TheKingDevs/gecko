...
func (check *Checker) stmt(ctxt stmtContext, s ast.Stmt) {
	...
	case *ast.AssignStmt:
		...
		case token.ASSIGN, token.DEFINE:
			...
			} else if check.geckoDynamicFile(s.Pos()) && check.geckoNewVar(s.Lhs) {
				// gecko declare-or-assign: `x = e` declares x when x is not
				// visible anywhere in the environment (and assigns otherwise).
				check.geckoVarDecl(inNode(s, s.TokPos), s.Lhs, s.Rhs)
			} else {
				// regular assignment
	...
	case *ast.TypeSwitchStmt:
		...
		case *ast.ExprStmt:
			rhs = guard.X
		case *ast.AssignStmt:
			if len(guard.Lhs) != 1 || (guard.Tok != token.DEFINE && !check.geckoFile(s.Pos())) || len(guard.Rhs) != 1 {
	...
	case *ast.WhileStmt:
		// gecko while loop; supports break and continue and defines
		// an implicit scope for the condition.
		inner |= breakOk | continueOk
