...
func (check *Checker) rangeStmt(inner stmtContext, rangeStmt *ast.RangeStmt, noNewVarPos positioner, sKey, sValue, sExtra, rangeVar ast.Expr, isDef bool) {
	...
	if isDef {
		...
		if check.geckoDynamicFile(sKey.Pos()) && check.geckoNewVar([]ast.Expr{sKey, sValue}) {
...
func (check *Checker) geckoRangeVars(rangeStmt *ast.RangeStmt, noNewVarPos positioner, sKey, sValue ast.Expr, rangeOverInt bool, x *operand, key, val Type) {
	lhs := [2]ast.Expr{sKey, sValue}
	rhs := [2]Type{key, val}
	...
	for i, l := range lhs {
		...
		if ident, ok := ast.Unparen(l).(*ast.Ident); ok && ident.Name != "_" && check.lookup(ident.Name) == nil {
			...
			obj := newVar(LocalVar, ident.Pos(), check.pkg, ident.Name, nil)
			check.recordDef(ident, obj)
			vars = append(vars, obj)
			assert(obj.typ == nil)
			if rangeOverInt {
