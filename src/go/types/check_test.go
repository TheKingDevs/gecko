...
// TestGeckoDeclareOrAssign verifies the gecko `x = e` declare-or-assign
// rule on .gk files: `=` instanceof existing visible name assigns to it,
// otherwise it declares a new variable in the current block.
func TestGeckoDeclareOrAssign(t *testing.T) {
	const src = `package p

func f() {
	x = 1        // declare x in the current block
	x = x + 1    // assign to the already-declared x
	if (true) {
		x = 10    // assign to the outer x (no shadowing)
		var y: int = 2
		y = 3
		_ = y
	}
	items = []int{1, 2}
	for (i = range items) { // declare i (range iteration var)
		_ = i
	}
	_ = x
}
`
	testFiles(t, []string{"gecko.gk"}, [][]byte{[]byte(src)}, true)
}

// TestGeckoImplicitPointers verifies that gecko accepts implicit pointer
// address-of and dereference on .gk files: a value of type E is
// assignable to a variable of type *E (implicit address-of, when
// addressable), and a value of type *E is assignable to a variable of
// type E (implicit dereference).
func TestGeckoImplicitPointers(t *testing.T) {
	const src = `package p

func describe(p: *int): string {
	var d: int = p      // auto deref RHS
	return string(d)
}

func getPtr(): *int {
	var v: int = 5
	return v            // auto address-of on return
}
...
func getVal(): int {
	...
	var p: *int = null
	p = v               // auto address-of
	return p            // auto deref on return
}

func setTo(p: *int, val: int) {
	*p = val
}

func f() {
	var x: int = 100
	var p: *int = x          // auto address-of in var init
	var y: int = p           // auto deref in var init
	_ = y
	_ = getPtr()
	_ = getVal()
	var q: *int = x
	setTo(q, 7)
	setTo(x, 9)              // auto address-of of int argument
	var r: *int = null
	_ = r
}
`
...
// TestGeckoTypeMode verifies the "type" field of gecko.json: in dynamic mode
// the gecko dynamic-typing features (inferred composite literals,
// declare-or-assign, implicit pointer address-of/dereference) are accepted;
// in typed mode (Config.GeckoTyped) they are rejected, forcing explicit type
// annotations and explicit & and * operators.
func TestGeckoTypeMode(t *testing.T) {
	const dynamicSrc = `package p
...
`
	// Dynamic mode: the source above type-checks cleanly.
	testFiles(t, []string{"gecko.gk"}, [][]byte{[]byte(dynamicSrc)}, true)

	// Typed mode: dynamic-typing features become errors.
	got := checkGeckoSource(t, dynamicSrc, true)
	for _, want := range []string{
		...
		if !found {
			t.Errorf("typed mode: no error matching %q; got:\n%s", want, strings.Join(got, "\n"))
		}
	}

	// Typed mode: fully annotated code still type-checks (explicit & and *
	// replace the implicit conversions).
...
`
	if errs := checkGeckoSource(t, typedSrc, true); len(errs) > 0 {
		t.Errorf("typed mode: fully annotated code must type-check, got:\n%s", strings.Join(errs, "\n"))
	}
}

// checkGeckoSource type-checks a gecko source string with the given typed
// mode and returns the reported error messages.
func checkGeckoSource(t *testing.T, src string, typed bool) []string {
	t.Helper()
	files, errlist := parseFiles(t, []string{"gecko.gk"}, [][]byte{[]byte(src)}, parser.AllErrors)
	if len(errlist) > 0 {
		t.Fatalf("parse errors:\n%s", errlist)
	}
	var conf Config
	conf.GeckoTyped = typed
	conf.Importer = defaultImporter(fset)
	var msgs []string
	conf.Error = func(err error) { msgs = append(msgs, err.Error()) }
	conf.Check("p", fset, files, nil)
	return msgs
}
