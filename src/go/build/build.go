...
// isGoFileName reports whether name is a Go source file accepted by the gecko
// build. Gecko compiles user sources with the ".gk" extension, while the Go
// standard library it is built on remains written in ".go"; both contain Go
// syntax and must be classified the same way.
func isGoFileName(name string) bool {
	return strings.HasSuffix(name, ".go") || strings.HasSuffix(name, ".gk")
}

// isGoTestFileName reports whether name is a Go test file, in either of the
// source extensions recognized by gecko (".go" and ".gk").
func isGoTestFileName(name string) bool {
	return strings.HasSuffix(name, "_test.go") || strings.HasSuffix(name, "_test.gk")
}
...
// to that path.
//
// In the directory containing the package, .go, .gk, .c, .h, and .s files are
// considered part of the package (gecko accepts .gk files as Go syntax in
// addition to the standard .go files) except for:
//
...
func hasGoFiles(ctxt *Context, dir string) bool {
	...
	for _, ent := range ents {
		if !ent.IsDir() && isGoFileName(ent.Name()) {
			return true
		}
	}
	return false
...
func (ctxt *Context) matchFile(dir, name string, allTags map[string]bool, binaryOnly *bool, fset *token.FileSet) (*fileInfo, error) {
	...
	if !isGoFileName(name) && fileListForExt(&dummyPkg, ext) == nil {
		// skip
		return nil, nil
	}
	...
	if isGoFileName(name) {
		err = readGoInfo(f, info)
		if isGoTestFileName(name) {
