...
// skipTSImportTail consumes the gecko TypeScript-style tail of an import
// declaration so that readString can read the final quoted path. The forms are:
//
//	import "p"                       (no tail)
//	import . "p"                     (no tail)
//	import x "p"                     (no tail)
//	import x from "p"                (gecko TS default import)
//	import { a, b } from "p"         (gecko TS named imports)
//	import x, { a, b } from "p"      (gecko TS mixed import)
//
// The tail past the optional leading name is "from", ", { ... }", or
// ", { ... } from"; none of those contain a quote, so we simply consume
// characters until the opening quote of the path string (or a newline/EOF,
// which readString will report).
func (r *importReader) skipTSImportTail() {
	r.peekByte(true)
	for r.peek != '"' && r.peek != '`' && r.peek != '\n' && r.peek != 0 {
		r.peek = 0
		r.peekByte(true)
	}
}
