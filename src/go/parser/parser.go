...
type parser struct {
	...
	// gecko is set for source files with the .gk extension. It enables
	// the gecko syntax: mandatory parentheses around if/for/switch clauses
	// and the while loop keyword.
	gecko bool
}

func (p *parser) init(file *token.File, src []byte, mode Mode) {
	p.file = file
	p.gecko = strings.HasSuffix(file.Name(), ".gk")
	eh := func(pos token.Position, msg string) { p.errors.Add(pos, msg) }
	p.scanner.Init(p.file, src, eh, scanner.ScanComments)
	p.scanner.Gecko = p.gecko
...
// atGeckoColon consumes and reports an explicit type annotation colon
// ('x: Type') in gecko files. It returns false when the next token is
// not a colon, in which case the caller reports the missing ':'.
func (p *parser) atGeckoColon() bool {
...
func (p *parser) parseParamDecl(name *ast.Ident, typeSetsOK bool) (f field) {
	...
	case token.IDENT:
		...
		// between the name and its type (type parameter lists keep the Go
		// syntax).
		geckoColon := false
		if p.gecko && !typeSetsOK && p.tok == token.COLON {
			p.next()
			geckoColon = true
		}
		...
		case token.IDENT, token.MUL, token.ARROW, token.FUNC, token.CHAN, token.MAP, token.STRUCT, token.INTERFACE, token.LPAREN:
			// name type
			if p.gecko && !typeSetsOK && !geckoColon {
				p.error(p.pos, "missing ':' between the parameter name and its type (gecko)")
			}
			f.typ = p.parseType()
...
// parseResults parses the (optional) result list of a function header.
// In gecko files the result type is prefixed by a colon, e.g.
// `func Login(...): bool` or `func f(): (x: int, err: error)`.
func (p *parser) parseResults() *ast.FieldList {
	if !p.gecko {
		return p.parseParameters(true)
	}
	colon := p.atGeckoColon()
	results := p.parseParameters(true)
	if !colon && results != nil {
		p.error(results.Pos(), "missing ':' before the function result type (gecko)")
	}
	return results
}
...
func (p *parser) parseOperand() ast.Expr {
	...
	case token.LBRACK:
		if p.gecko {
			...
			p.next() // consume '['
			if p.tok != token.RBRACK {
				// gecko dynamic-slice literal [a, b, c]
				return p.parseGeckoDynamicSlice(lbrack)
			}
			// []T{...} composite literal or []T slice type;
			// "[" has already been consumed.
			return p.parseArrayType(lbrack, nil)
		}

	case token.NEW:
		// gecko: new Class(args)
...
// parseNewExpr parses a gecko new expression, "new Class(args)".
func (p *parser) parseNewExpr() *ast.NewExpr {
	...
	x := &ast.NewExpr{New: pos, Ellipsis: token.NoPos}

	switch p.tok {
	case token.IDENT:
		x.Type = p.parseQualifiedIdent(nil)
	default:
		p.errorExpected(pos, "class name after new (gecko)")
		p.advance(exprEnd)
		return x
	}
	...
	if p.tok == token.LPAREN {
		...
		p.exprLev++
		for p.tok != token.RPAREN && p.tok != token.EOF && !x.Ellipsis.IsValid() {
			x.Args = append(x.Args, p.parseRhs())
			if p.tok == token.ELLIPSIS {
				x.Ellipsis = p.pos
				p.next()
			}
		...
		// gecko new requires an argument list
		p.error(pos, "expected ( after class name in new expression (gecko)")
		x.Rparen = p.pos
	}
...
// parseGeckoDynamicSlice parses a gecko dynamic-slice literal, "[List]",
// with the opening "[" already consumed at position lbrack. The element
// type is inferred by the compiler from the elements.
func (p *parser) parseGeckoDynamicSlice(lbrack token.Pos) ast.Expr {
	if p.trace {
		defer un(trace(p, "GeckoDynamicSlice"))
	}

	lit := &ast.CompositeLit{Lbrace: lbrack, GeckoBrackets: true}
	p.exprLev++
	for p.tok != token.RBRACK && p.tok != token.EOF {
		lit.Elts = append(lit.Elts, p.parseElement())
		if !p.atComma("dynamic slice literal", token.RBRACK) {
			break
		}
		p.next()
	}
	p.exprLev--
	lit.Rbrace = p.expect(token.RBRACK)
	return lit
}
...
func (p *parser) parseSimpleStmt(mode int) (ast.Stmt, bool) {
	...
	case
		...
		pos, tok := p.pos, p.tok
		if p.gecko && tok == token.DEFINE {
			p.error(pos, "':=' is not allowed in gecko; use '=' instead")
		}
		p.next()
		var y []ast.Expr
		isRange := false
...
func (p *parser) parseIfHeader() (init ast.Stmt, cond ast.Expr) {
	// gecko: the clause must be wrapped in parentheses, if (cond) or
	// if (init; cond), and is terminated by the closing parenthesis.
	end := token.LBRACE
	if p.gecko {
		if p.tok == token.LPAREN {
			end = token.RPAREN
			...
			p.error(p.pos, "missing ( after if (mandatory in gecko)")
		}
	}
...
func (p *parser) isTypeSwitchGuard(s ast.Stmt) bool {
	...
	case *ast.AssignStmt:
		// v := x.(type)  (or v = x.(type) in gecko)
		if len(t.Lhs) == 1 && len(t.Rhs) == 1 && isTypeSwitchAssert(t.Rhs[0]) {
			switch t.Tok {
			case token.ASSIGN:
				// permit v = x.(type) but complain unless gecko
				if p.gecko {
					return true
				}
...
func (p *parser) parseSwitchStmt() ast.Stmt {
	...
	// gecko: the clause must be parenthesized, switch (tag) or
	// switch (init; tag), and is terminated by the closing parenthesis.
	end := token.LBRACE
	paren := false
	if p.gecko {
		if p.tok == token.LPAREN {
			...
			p.error(p.pos, "missing ( after switch (mandatory in gecko)")
		}
	}
...
func (p *parser) parseForStmt() ast.Stmt {
	...
	// gecko: the clause must be parenthesized, for (init; cond; post) or
	// for (range), and is terminated by the closing parenthesis.
	end := token.LBRACE
	paren := false
	if p.gecko {
		if p.tok == token.LPAREN {
			...
			p.error(p.pos, "missing ( after for (mandatory in gecko)")
		}
	}
...
// parseWhileStmt parses a gecko while statement, while (cond) body.
// It is only reached for source files with the .gk extension (the
// scanner maps "while" to token.WHILE only in that case).
func (p *parser) parseWhileStmt() *ast.WhileStmt {
	...
	if p.tok == token.LPAREN {
		...
		p.error(p.pos, "missing ( after while (mandatory in gecko)")
		cond = &ast.BadExpr{From: p.pos, To: p.pos}
	}
...
func (p *parser) parseImportSpec(doc *ast.CommentGroup, _ token.Token, _ int) ast.Spec {
	...
	// Gecko (.gk) files support the TypeScript-style import forms
	// "import x from \"p\"", "import { a, b.x } from \"p\"" and
	// "import x, { a, b.x } from \"p\"". The member list and "from"
	// keyword are preserved so the import can be reprinted faithfully.
	var tsNames []ast.Expr
	var tsPos token.Pos
	tsFrom := false
	if p.gecko && p.tok != token.STRING {
		if p.tok == token.COMMA {
			p.next()
		}
		if p.tok == token.LBRACE {
			tsPos = p.expect(token.LBRACE)
			for p.tok != token.RBRACE && p.tok != token.EOF && p.tok != token.SEMICOLON {
				tsNames = append(tsNames, p.parseTsMember())
				if p.tok != token.COMMA {
					break
				}
				p.next()
...
// parseTsMember parses one member of a gecko TypeScript-style import list,
// either a plain name ("a") or a selector-qualified name ("b.x").
func (p *parser) parseTsMember() ast.Expr {
	x := p.parseOperand()
	if p.tok != token.PERIOD {
		return x
	}
	// ignore a trailing qualified name tail: only the leading name
	// and a single selector nest are preserved for reprinting
	for p.tok == token.PERIOD && x != nil {
		p.next()
		if p.tok != token.IDENT {
			p.errorExpected(p.pos, "name after . in import member")
			break
		}
...
func (p *parser) parseValueSpec(doc *ast.CommentGroup, keyword token.Token, iota int) ast.Spec {
	...
	case token.CONST:
		...
		if p.tok != token.EOF && p.tok != token.SEMICOLON && p.tok != token.RPAREN {
			...
			colon := p.atGeckoColon()
			typ = p.tryIdentOrType()
			if p.gecko && !colon && typ != nil {
				p.error(typ.Pos(), "missing ':' between the constant name and its type (gecko)")
			}
	...
	case token.VAR:
		if p.tok != token.ASSIGN {
			...
			typ = p.parseType()
			if p.gecko && !colon {
				p.error(typ.Pos(), "missing ':' between the variable name and its type (gecko)")
			}
		}
		if p.tok == token.ASSIGN {
			p.next()
...
// parseExportDecl parses a gecko `export` declaration. The current token is
// `export`. It either prefixes the following top-level declaration
// (export func/var/const/class) or marks already declared names as exported
...
func (p *parser) parseExportDecl() ast.Decl {
	...
	case token.CONST, token.VAR:
		d := p.parseGenDecl(p.tok, p.parseValueSpec)
		d.Export = true
		return d
	...
	case token.LBRACE:
		...
		p.expect(token.RBRACE)
		p.expectSemi()
		return &ast.ExportDecl{Doc: doc, ExportPos: pos, Names: names}

	case token.IDENT:
		// export X  (shorthand for export { X })
		name := p.parseIdent()
		p.expectSemi()
		return &ast.ExportDecl{Doc: doc, ExportPos: pos, Names: []*ast.Ident{name}}
...
// parseClassDecl parses a gecko class declaration,
// "class Name() { classMember₁; classMember₂; ... }". Class members are
// methods, field declarations (`var x = v`, `const X = v`, or the bare
// `x = v` shorthand). Field initializers run before the constructor body;
// a class with fields but no explicit constructor gets a synthetic one.
func (p *parser) parseClassDecl(export bool) ast.Decl {
	if p.trace {
	...
	if p.tok == token.LBRACE {
		...
		for p.tok != token.RBRACE && p.tok != token.EOF {
			...
			case token.VAR, token.CONST:
				tok := p.tok
				p.next()
				spec := p.parseValueSpec(nil, tok, 0).(*ast.ValueSpec)
				if tok == token.CONST {
					decl.Consts = append(decl.Consts, spec)
				} else {
			...
			case token.IDENT:
				...
				if p.tok == token.ASSIGN {
					...
					spec := &ast.ValueSpec{Names: []*ast.Ident{name}, Values: values}
					decl.Fields = append(decl.Fields, spec)
					decl.Inits = append(decl.Inits, p.fieldInitStmts(spec.Names, spec.Values)...)
				} else {
					decl.Methods = append(decl.Methods, p.parseClassMethodTail(name))
				}
			default:
				p.errorExpected(p.pos, "class member declaration")
				p.advance(declStart)
				break
			}
		}
	...
	if len(decl.Inits) > 0 {
		...
		for i, m := range decl.Methods {
			if m.Name.Name == "constructor" {
				ctorIdx = i
				break
			}
		}
		if ctorIdx >= 0 {
			body := decl.Methods[ctorIdx].Body
			body.List = append(body.List[:0], append(decl.Inits, body.List...)...)
		} else {
			ctor := &ast.FuncDecl{
				Name:           ast.NewIdent("constructor"),
				Type:           &ast.FuncType{Func: name.Pos()},
				Body:           &ast.BlockStmt{Lbrace: pos, List: decl.Inits, Rbrace: pos},
				GeckoSynthCtor: true,
			}
			decl.Methods = append([]*ast.FuncDecl{ctor}, decl.Methods...)
		}
	}
...
func (p *parser) fieldInitStmts(names []*ast.Ident, values []ast.Expr) []ast.Stmt {
	if len(values) == 0 {
		return nil
	}
...
// parseClassMethod parses one gecko class method
// "Name Parameters [ ":" Result ] "{" Body "}".
func (p *parser) parseClassMethod() *ast.FuncDecl {
	name := p.parseIdent()
	return p.parseClassMethodTail(name)
}

// parseClassMethodTail parses the signature and body of a gecko class
// method whose name has already been read.
func (p *parser) parseClassMethodTail(name *ast.Ident) *ast.FuncDecl {
	if p.trace {
		defer un(trace(p, "ClassMethod"))
	}
	...
	decl := &ast.FuncDecl{
		Doc:  doc,
		Name: name,
		Type: &ast.FuncType{Func: name.Pos(), Params: params, Results: results},
	}
	if p.tok == token.LBRACE {
