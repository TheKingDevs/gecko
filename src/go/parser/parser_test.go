...
// TestGeckoStatements verifies that .gk files accept the gecko syntax:
// mandatory parentheses around if/for/switch clauses, the while keyword,
// ':=' rejected in favor of '=', and ':' between a name and its type.
func TestGeckoStatements(t *testing.T) {
	src := `package gecko
...
`
	...
	ast.Inspect(f, func(n ast.Node) bool {
		if _, ok := n.(*ast.WhileStmt); ok {
			gotWhile = true
		}
		return true
	})
	if !gotWhile {
		t.Error("expected a while statement in gecko source")
	}

	// "while" must remain a valid identifier in regular .go files.
	if _, err := ParseFile(token.NewFileSet(), "a.go", "package p\nvar while = 1\nfunc f() { _ = while }\n", 0); err != nil {
		t.Errorf("while must be a valid identifier in .go files: %v", err)
	}
}

// TestGeckoColonEqualsError verifies that ':=' is rejected in gecko files
// with a migration hint, and that a missing ':' before an explicit type is
// reported.
func TestGeckoColonEqualsError(t *testing.T) {
	for _, src := range []string{
		"package p\nfunc f() { x := 1 }\n",
		"package p\nfunc f() { for (i := 0; i < 3; i++) {} }\n",
		"package p\nfunc f() { for (i := range x) {} }\n",
		"package p\nfunc f() { switch (t := x.(type)) { case int: } }\n",
	} {
		_, err := ParseFile(token.NewFileSet(), "gecko.gk", src, 0)
		if err == nil || !strings.Contains(err.Error(), "':=' is not allowed in gecko; use '=' instead") {
			t.Errorf("ParseFile(gk) %q: got %v, want ':=' gecko error", src, err)
		}
	}

	// explicit types require a ':' in gecko.
	for _, src := range []string{
		"package p\nvar x int\n",
		"package p\nfunc f(a int) {}\n",
		...
		_, err := ParseFile(token.NewFileSet(), "gecko.gk", src, 0)
		if err == nil || !strings.Contains(err.Error(), "gecko)") {
			t.Errorf("ParseFile(gk) %q: got %v, want missing ':' error", src, err)
		}
	}
}

// TestGeckoMissingParens verifies that .gk files reject unparenthesized
// if/for/switch clauses and that the parenthesized forms are required.
func TestGeckoMissingParens(t *testing.T) {
	for _, src := range []string{
		"package p\nfunc f() { if 1 == 1 {} }\n",
		"package p\nfunc f() { for i := 0; i < 3; i++ {} }\n",
		"package p\nfunc f() { switch 1 { case 1: } }\n",
		"package p\nfunc f() { while true {} }\n",
	} {
		_, err := ParseFile(token.NewFileSet(), "gecko.gk", src, 0)
		if err == nil || !strings.Contains(err.Error(), "mandatory in gecko") {
			t.Errorf("ParseFile(gk): %s got %v, want missing ( error", src, err)
		}
	}
}

// TestGeckoDecls verifies that .gk files accept the gecko declaration forms:
// export, class with implicit-this methods, new expressions, dynamic slices,
// and TypeScript-style member imports.
func TestGeckoDecls(t *testing.T) {
	src := `package gecko

import { fmt } from "fmt"
import x from "strings"
import y, { a, b.e } from "math"
...
`
	...
	ast.Inspect(f, func(n ast.Node) bool {
		...
		case *ast.NewExpr:
			...
			case *ast.Ident, *ast.SelectorExpr:
				// new App("gecko"), new pkg.Type(x, ...)
			default:
				t.Errorf("new expression type = %v, want a name", n.Type)
			}
	...
	if !gotClass || !gotNew || !gotSlice || !gotExport || !gotTsMember {
