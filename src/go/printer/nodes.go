	...
	if len(fields.List) > 0 {
		...
		for i, par := range fields.List {
			...
			if len(par.Names) > 0 {
				...
				if p.Config.Mode&GeckoColons != 0 && mode == funcParam {
					// gecko: func f(name: Type)
					p.print(token.COLON)
				}
				p.print(blank)
			}
...
func (p *printer) signature(sig *ast.FuncType) {
	...
	if n > 0 {
		// res != nil
		if p.Config.Mode&GeckoColons != 0 {
			// gecko: func f(...): Type or func f(...): (x: Type, ...)
			p.print(token.COLON)
		}
		p.print(blank)
...
func (p *printer) expr1(expr ast.Expr, prec1, depth int) {
	...
	case *ast.CompositeLit:
		...
		// gecko dynamic-slice literals are written with "[" and "]" instead
		// of "{" and "}": [1, 2, 3]
...
func (p *printer) controlClause(isForStmt bool, init ast.Stmt, expr ast.Expr, post ast.Stmt) {
	p.print(blank)
	needsBlank := false
	if p.Config.Mode&GeckoParens != 0 {
		// gecko: the clause is always wrapped in parentheses, e.g. if (x),
		// for (init; cond; post), switch (tag).
		p.print(token.LPAREN)
		if init == nil && post == nil && isForStmt && expr == nil {
			// for (;;)
			p.print(token.SEMICOLON, token.SEMICOLON)
			needsBlank = true
		} else if init == nil && post == nil {
...
func (p *printer) stmt(stmt ast.Stmt, nextIsRBrace bool) {
	...
	case *ast.WhileStmt:
		// gecko while loop; the condition is always parenthesized.
		p.print(token.WHILE, blank)
		p.print(token.LPAREN)
		if s.Cond != nil {
			p.expr(s.Cond)
		}
...
func (p *printer) spec(spec ast.Spec, n int, doIndent bool) {
	...
	case *ast.ImportSpec:
		...
		if s.Name != nil {
			p.expr(s.Name)
			if len(s.TsNames) > 0 {
				// gecko: import Name, { A, B.x } from "p"
				p.print(token.COMMA, blank)
			}
		}
		if len(s.TsNames) > 0 {
			// gecko member-import list "import { A, B.x } [from] \"p\""
			p.setPos(s.TsPos)
			p.print(token.LBRACE)
			if n := len(s.TsNames); n > 0 {
				p.print(blank)
				for i, x := range s.TsNames {
					if i > 0 {
						p.print(token.COMMA, blank)
					}
					p.expr(x)
		...
		if s.TsFrom {
			// gecko: the "from" keyword of a TypeScript-style import
			p.setPos(s.Path.Pos())
			p.print(blank)
			p.expr(ast.NewIdent("from"))
		}
		if s.Name != nil || len(s.TsNames) > 0 || s.TsFrom {
			p.print(blank)
		}
...
// classDecl prints a gecko class declaration,"class Name() { method₁; ... }".
// Class methods use an implicit "this" receiver and are printed without the
// "func" keyword and without an explicit receiver.
func (p *printer) classDecl(d *ast.ClassDecl) {
	...
	if d.Rbrace.IsValid() {
		...
		members := make([]ast.Node, 0, len(d.Fields)+len(d.Consts)+len(d.Methods))
		for _, s := range d.Fields {
			members = append(members, &classField{spec: s, tok: token.VAR})
		}
		for _, s := range d.Consts {
			members = append(members, &classField{spec: s, tok: token.CONST})
		}
		...
		if len(members) > 0 {
			p.print(indent, formfeed)
			var line int
			prevClassInits := p.classInits
			p.classInits = d.Inits
			for i, m := range members {
				...
				switch m := m.(type) {
				case *classField:
					p.setComment(m.spec.Doc)
					p.setPos(m.spec.Pos())
					p.print(m.tok, blank)
					p.valueSpec(m.spec, false)
				case *ast.FuncDecl:
					p.setComment(m.Doc)
					p.setPos(m.Pos())
					if m.Async {
...
func (c *classField) Pos() token.Pos { return c.spec.Pos() }
func (c *classField) End() token.Pos { return c.spec.End() }

// classMethodBodyPruned returns the body to print for a class method,
// dropping the field-initializer prefix that the parser merged into a
// `constructor` body: those initializers are printed separately as their
// `var x = v` / `const X = v` declarations. It returns nil when nothing
// should be printed.
func (p *printer) classMethodBodyPruned(m *ast.FuncDecl) *ast.BlockStmt {
	if len(p.classInits) == 0 || m.Name == nil || m.Name.Name != "constructor" || m.Body == nil {
		return m.Body
	}
	k := 0
	for k < len(m.Body.List) && k < len(p.classInits) && m.Body.List[k] == p.classInits[k] {
		k++
	}
	if k == 0 {
		return m.Body
	}
	return &ast.BlockStmt{Lbrace: m.Body.Lbrace, List: m.Body.List[k:], Rbrace: m.Body.Rbrace}
}
