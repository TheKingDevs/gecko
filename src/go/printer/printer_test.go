...
func format(src []byte, mode checkMode) ([]byte, error) {
	...
	f, err := parser.ParseFile(fset, fname, src, parser.ParseComments|parser.SkipObjectResolution)
	if err != nil {
	...
	if mode&gecko != 0 {
		cfg.Mode |= GeckoParens
		cfg.Mode |= GeckoColons
	}
	...
	res := buf.Bytes()
	if _, err := parser.ParseFile(fset, fname, res, parser.ParseComments|parser.SkipObjectResolution); err != nil {
...
var data = []entry{
	...
	{"gecko.gk", "gecko.golden", idempotent | gecko},
}
