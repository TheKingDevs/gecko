...
type printer struct {
	...
	// classInits holds the field-initializer statements of the gecko class
	// currently being printed, so a `constructor` body can drop the merged
	// initializer prefix (they are printed separately as declarations).
	classInits []ast.Stmt
...
const (
	...
	GeckoParens                  // always parenthesize if/for/switch clauses (gecko, .gk source files)
	GeckoColons                  // annotate types with ':' (var x: int, params, results; gecko, .gk source files)
)
