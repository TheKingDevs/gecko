...
type (
	...
	CompositeLit struct {
		...
		GeckoBrackets bool      // true if written with "[...]" (gecko dynamic-slice literal)
	}

	// A NewExpr node represents a gecko "new Class(ArgList)" expression,
	// which instantiates a class and runs its constructor.
	NewExpr struct {
		New      token.Pos // position of "new" keyword
		Type     Expr      // class type
...
type (
	...
	// A WhileStmt represents a gecko while statement, while (cond) body.
	// It is produced only for source files with the .gk extension.
	WhileStmt struct {
		While token.Pos // position of "while" keyword
		Cond  Expr      // condition; or nil for while (); infinite loop
		Body  *BlockStmt
	}
...
type (
	...
	// explicitly with `var x = v` / `const X = v` (or the bare `x = v`
	// shorthand) in the class body. Explicit declarations are recorded in
	// Fields (var) and Consts (const); their initializers are merged into
	// the constructor body and also recorded in Inits. Class methods are
...
func (d *ExportDecl) Pos() token.Pos {
	if d.ExportPos.IsValid() {
		return d.ExportPos
	}
...
func (d *ClassDecl) End() token.Pos {
	...
	if n := len(d.Consts); n > 0 {
		return d.Consts[n-1].End()
	}
...
func (d *ExportDecl) End() token.Pos {
	...
	return token.Pos(int(d.ExportPos) + len("export"))
}
