...
// tasks and even the same task as it's rescheduled on
...
// Ensure current working directory seen by this task matches
// the most recent [Chdir] called in any task. It's called internally
// before executing any syscall which uses a relative pathname. Must
// be called with the task locked to the OS thread, to prevent
