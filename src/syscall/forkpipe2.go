...
var (
	...
	// Number of tasks currently forking, and thus the
	// number of tasks holding a conceptual write lock
	// on ForkLock.
	forking int
)

// hasWaitingReaders reports whether any task is waiting
...
func acquireForkLock() {
	...
	if hasWaitingReaders(&ForkLock) {
		...
		// task is waiting to read from it.
