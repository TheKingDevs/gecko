...
// startProcess starts a new task, tied to the OS
// thread, which runs the process and subsequently waits
// for it to finish, communicating the process stats back
// to any tasks that may have been waiting on it.
//
// Such a dedicated task is needed because on
// Plan 9, only the parent thread can wait for a child,
// whereas tasks tend to jump OS threads (e.g.,
// between starting a process and running Wait(), the
// task may have been rescheduled).
