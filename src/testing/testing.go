...
// directly. TestMain runs in the main task and can do whatever setup
...
// the [T.Parallel] method, must be called only from the task running the
// Test function.
//
// The other reporting methods, such as the variations of [T.Log] and [T.Error],
// may be called simultaneously from multiple tasks.
type T struct {
	common
...
// FailNow must be called from the task running the
// test or benchmark function, not from other tasks
...
func (c *common) FailNow() {
	...
	// Calling runtime.Goexit will exit the task, which
	// will run the deferred functions in this task,
	...
	// tRunner no matter what), but worse the task teardown
...
// SkipNow must be called from the task running the test, not from
// other tasks created during the test. Calling SkipNow does not stop
// those other tasks.
...
// Helper may be called simultaneously from multiple tasks.
...
func (c *common) checkRaces() (raceErrors int64) {
	...
	if c.raceErrorLogged.CompareAndSwap(false, true) {
		...
		// (Note that the race detector itself will still write a task
...
func (t *T) Parallel() {
	...
	// although it can be wrong if the test spawns a task that races in the
	...
	// task was blocked, such as in the parent test or in other parallel
	// subtests.
	//
...
func tRunner(t *T, fn func(t *T)) {
	...
	// When this task is done, either because fn(t)
	...
	defer func() {
		...
		// tRunner is called in its own task, so this terminates the process.
		//
		...
		// the user does not appropriately synchronize a task.
		t.done = true
...
// Run runs f as a subtest of t called name. It runs f in a separate task
// and blocks until f returns or calls t.Parallel to become a parallel test.
// Run reports whether f succeeded (or at least did not fail before calling t.Parallel).
//
// Run may be called simultaneously from multiple tasks, but all such calls
...
func (t *T) Run(name string, f func(t *T)) bool {
	...
	// The parent task will block until the subtest either finishes or calls
	// Parallel, but in general we don't know whether the parent task is the
	// top-level test function or some other task it has spawned.
