...
// Those methods must be called only from the task running the Benchmark function.
...
type B struct {
	...
	parallelism      int // RunParallel creates parallelism*GOMAXPROCS tasks
...
// run executes the benchmark in a separate task, including all of its
...
// launch is run by the doBench function as a separate task.
...
type PB struct {
	globalN *atomic.Uint64 // shared between all worker tasks iteration counter
...
// It creates multiple tasks and distributes b.N iterations among them.
// The number of tasks defaults to GOMAXPROCS. To increase parallelism for
// non-CPU-bound benchmarks, call [B.SetParallelism] before RunParallel.
// RunParallel is usually used with the go test -cpu flag.
//
// The body function will be run in each task. It should set up any
// task-local state and then iterate until pb.Next returns false.
...
// not the sum of wall time or CPU time over each parallel task.
...
// SetParallelism sets the number of tasks used by [B.RunParallel] to p*GOMAXPROCS.
