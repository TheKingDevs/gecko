...
//
// Time in a bubble only advances when every task in the
...
//			}()
//			time.Sleep(2 * time.Second) // the task above will run before this Sleep returns
//			t.Log(time.Since(start))    // always logs "2s"
//		})
//	}
//
// Time stops advancing when the root task of the bubble exits.
//
// # Blocking
//
// A task in a bubble is "durably blocked" when it is blocked
// and can only be unblocked by another task in the same bubble.
// A task which can be unblocked by an event from outside its
// bubble is not durably blocked.
//
// The [Wait] function blocks until all other tasks in the
...
//				done = true
//			}()
//			// Wait will block until the task above has finished.
//			synctest.Wait()
//			t.Log(done) // always logs "true"
//		})
//	}
//
// When every task in a bubble is durably blocked:
//
//   - [Wait] returns, if it has been called.
//   - Otherwise, time advances to the next time that will
//     unblock at least one task, if there is such a time
//     and the root task of the bubble has not exited.
//   - Otherwise, there is a deadlock and [Test] panics.
//
// The following operations durably block a task:
//
...
// In particular, the following operations may block a task,
// but are not durably blocking because the task can be unblocked
...
//
// [sync.Cond.Wait] is durably blocking. Waking a task in a bubble
...
//
// AfterFunc registers a function to execute in a new task
...
//			// because tasks blocked on network I/O prevent a synctest
...
//			// Send it in a new task, since it won't complete until the end of the test.
//			body := "request body"
//			go func() {
...
//			}
//
//			// Start a new task copying the body sent by the client into a buffer.
//			// Wait for all tasks in the bubble to block and verify that we haven't
...
//
//			// We started several tasks during the test.
...
//
// Test waits for all tasks in the bubble to exit before returning.
// If the tasks in the bubble become deadlocked, the test fails.
//
...
// Wait blocks until every task within the current bubble,
// other than the current task, is durably blocked.
//
// Wait must not be called from outside a bubble.
// Wait must not be called concurrently by multiple tasks
// in the same bubble.
func Wait() {
	synctest.Wait()
}
...
// by the duration of d and every task within the current bubble,
// other than the current task, is durably blocked.
//
...
// If the test itself and another task running the system under test
// sleeps for the exact same amount of time, it's unpredictable which
// of the two tasks will run first. The test itself usually wants
