...
func initFuzzFlags() {
	...
	fuzzCacheDir = flag.String("test.fuzzcachedir", "", "directory where interesting fuzzing inputs are stored (for use only by cmd/gecko)")
	isFuzzWorker = flag.Bool("test.fuzzworker", false, "coordinate with the parent process to fuzz random values (for use only by cmd/gecko)")
}
...
// task. To wait for its completion, receive from f.signal.
//
...
func fRunner(f *F, fn func(*F)) {
	// When this task is done, either because runtime.Goexit was called, a
